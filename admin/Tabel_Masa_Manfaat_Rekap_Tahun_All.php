		...
		while ($mRo = mysql_fetch_array($nRs, MYSQL_BOTH))
		{
			...
			else
			{
				if ($TbL=='1.5.4')
				{
					...
					$Col03 = fGlobalNEW("IfNull(sum(Debet),0)","ta_kib_post_108".$mts,"Kd_UPB:Kd_Aset_108:Tanggal:extracom:Crit",$uNT."%:".$Col01."%:".$rTH."-12-31:".$ExtR.":SLD","LIKE:LIKE:<=:=:=","",DatabaseSB,$ConSB,"");
			...
			while ($mRoD = mysql_fetch_array($nRsD, MYSQL_BOTH))
			{
				...
				else
				{
					if ($TbL=='1.5.4')
					{
						...
						#$Col03 = $Col03-$Col04;
						$Col03 = fGlobalNEW("IfNull(sum(debet),0)","ta_kib_post_108".$mts,"Kd_UPB:Kd_Aset_108:Tanggal:extracom:Crit",$uNT."%:".$Col01.":".$rTH."-12-31:".$ExtR.":SLD","LIKE:=:<=:=:=","",$DatabaseSB,$ConSB,"");
						$Col04 = fGlobalNEW("IfNull(sum(debet),0)","ta_kib_post_108".$mts,"Kd_UPB:Kd_Aset_108:Tanggal:extracom:Crit",$uNT."%:".$Col01.":".$rTH."-12-31:".$ExtR.":INV","LIKE:=:<=:=:=","",$DatabaseSB,$ConSB,"");
					}
					else
					{

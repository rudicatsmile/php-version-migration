...
if ($fSave=='Save')
{
	...
	{
		$gNeW = 1;
		$rMax = fGlobal("max(Referensi)","ta_usulan","Referensi","PHM.".fGetDate('year')."%","LIKE","","");
		if ($rMax)
		{
		...
		$gNeW = 1;
		$rMax = fGlobal("max(Nomor)","ta_usulan","Nomor","%/PHM/".fGetDate('year'),"LIKE","","");
		if ($rMax)
		{
		...
		$IdT = fGlobal("max(IDT)","ta_usulan","IDT","%","LIKE","","");
	}

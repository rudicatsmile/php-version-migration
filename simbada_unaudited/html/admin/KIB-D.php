...
<form name="myfrm" method="post" action="<?php echo "KIB-D_.php?FrmG=".$_GET['FrmG']."&IdL=".$_GET['IdL'] ?>">
  ...
  <table border="0" align="center" style="width:99%">
    ...
    <tr> 
      ...
      <td>&nbsp;</td>
      <td align="right"><input type="button" name="B37" value="EXPORT DATA" disabled onclick="P_ToExcel('800','400','center','ToExcel/KIB_D_ToExcel')" style="width: 90px; height: 24px; padding-left: 0px; padding-right: 0px; padding-top: 0px; padding-bottom: 0px" /></td>
    </tr>
    <tr> 
      <td>TAHUN</td>
      <td valign="middle"> <table border="0" width="100%" cellpadding="0" style="border-collapse: collapse">
          <tr> 
            ...
            <td width="8%">FIND</td>
            <td width="36%"> <input class="text" type="text" name="fFind" id="fFind" value="<? echo $gFin?>" placeholder='Search' style="width:200px; font-family: Calibri; font-size: 10pt; padding-left: 4px; padding-right: 4px; padding-top: 1px; padding-bottom: 1px; background-color: #E1F986" /></td>
            <td width="1%">&nbsp;</td>
            <td width="24%"> <input type="button" name="B39" value="GO" onclick="P_Find()" style="width: 50px; height: 21px; padding-left: 0px; padding-right: 0px; padding-top: 0px; padding-bottom: 0px" />
            <? if ($Lev=="0" || $Lev=="100") {?>
    ...
    <tr>
      ...
      <td><select class="boxs" name="fExt" style="width: 120px" tabindex="0" onchange="this.form.submit()">
        ...
        <option <? if ($gExt=="Y") {echo "selected";} ?> value="Y">EXTRACOM</option>
      </select></td>
      <td>&nbsp;</td>
      <td><label style="float:right; color:#FF0000"><input type="checkbox" name="fSdhMutasi" <? if ($gMuT!="") {echo "checked";}?> onclick="P_Find()" value="ON" />Sudah Mutasi</label></td>
    </tr>
    <tr> 
      <td>&nbsp;</td>
      <td>	  </td>
      <td><div id="loadingImg" style="width:40px;height:10px;display:none"><img src="Images/loading3.gif" alt="" width="40" height="40"></div>
	  </td>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
      <td></td>
  ...
  <table border="0" align="center" id="tbViewData" style="width:99%">
	...
	<table align="center" class="table-list" border="0" cellspacing="0" cellpadding="0" style="width:99%">
	  <tr> 
        ...
        <th width="100" align="left">Referensi</th>
        <th width="58" class="ac">Register</th>
        <th width="268" style="padding-left:2px">Nama Barang</th>
        <th width="210" style="padding-left:2px">Uraian</th>
        <th width="71" class="ac">Perolehan</th>
        <th width="62" class="ac">Tgl.Mutasi</th>
        <th width="105" class="ar">Nilai Perolehan </th>
        <th width="111" class="ar">Nilai Akhir </th>
        <th width="301" class="ac">Actions</th>
      </tr>
         <?
		if ($gMuT!=""){
			$tMuTZ="_mutasi";
		...
		*/
		...
		if ($gFin!="")
		{$fFindSy = "AND (No_Pengadaan LIKE '%".$gFin."%' OR Ref_KdpToAset LIKE '%".$gFin."%' OR Nm_Aset LIKE '%".$gFin."%' OR No_Register LIKE '%".$gFin."%' OR Harga LIKE '%".$gFin."%' OR Referensi LIKE '%".$gFin."%' OR Kd_Aset_108 LIKE '%".$gFin."%' OR Lokasi LIKE '%".$gFin."%' OR Keterangan LIKE '%".$gFin."%' OR Pencatat LIKE '%".$gFin."%')";}
		else
		{$fFindSy = "";}
		...
		$nSQL= "SELECT * FROM ta_kib_108".$tMuTZ." WHERE referensi LIKE '$gRf%' AND extracom LIKE '".$zExt."' AND Kd_UPB LIKE '".$zUpb."' AND Kd_Aset_108 LIKE '".$rBid.".".$rKel.".".$rOBJ.".".$rRin."' AND Tgl_Perolehan LIKE '".$rThn."-__-__' ".$fFindSy." ORDER BY Referensi LIMIT $Offset, $DataPerPage";
		#echo $nSQL;
		...
		do
			...
			$eCK="";
			$gNId = fGlobal("IfNull(sum(Debet),0)","ta_kib_post_108","Referensi:Tanggal",$mRo['Referensi'].":2019-12-31","=:<=","","");
			...
			$NeN = fGlobal("count(*)","ta_kib_post_108".$tMuTZ,"Referensi:HasilMerger:Kd_UPB",$mRo['Referensi'].":Y:".substr($mRo['Kd_UPB'],0,11)."%","=:=:LIKE","","");
			...
			if ($ImPORT=='YA')
			{
				$aTR = fGlobal("count(*)","atribusi_d","kode_skpd:register_new",$gSK.":".$mRo['No_Register'],"=:=","","");
				$aTP = fGlobal("count(*)","ta_kib_post_108","referensi:kd_upb",$mRo['Referensi'].":".$mRo['Kd_UPB'],"=:=","","");
				$aHR = fGlobal("IfNull(sum(Harga_Bertambah),0)","atribusi_d","kode_skpd:register_new",$gSK.":".$mRo['No_Register'],"=:=","","");
				
				if ($aTR > 1 && $aTR > $aTP && $aHR==$gNIL)
				{
					callImportAtribusi_CDE('atribusi_d',$mRo['No_Register'],$mRo['Referensi'],$mRo['Kd_Aset_108'],$mRo['Kd_UPB'],$Dipla,$Exec);
				}
			}
			*/
			...
			if ($mRo['kode_bar']!="")
			{
				...
				$dtScan = fGlobal("Recorded_map:userName","ta_kib_108_barcode","kode_bar",$mRo['kode_bar'],"=","","dd");
				if ($dtScan)
				{
			...
			<tr height="40" style="cursor: pointer <?=$Blnk?>" onmouseover="this.style.cursor=&#39;pointer&#39" <?=fBackCLR($iG)?>> 
              ...
              <td style="border-bottom: #999999 dotted 1px; border-right:#999999 dotted 1px; text-align:center"><?=$mRo['Referensi'].$eCK.$kdUPB.$RfPTA?></td>
              ...
              <td style="border-bottom: #999999 dotted 1px; border-right:#999999 dotted 1px; text-align:center"><? if ($mRo['Tgl_Mutasi']!="0000-00-00") {echo fConvertDateShort($mRo['Tgl_Mutasi']);}?><?=$gAda?></td>
              ...
              <td style="border-bottom: #999999 dotted 1px; border-right:#999999 dotted 1px; padding-right:2px <?=$ClR?>" align="right" title="<?=$aTR?> record"><? echo fConvertToRupiah($gNIL)?></td>
			  ...
			  <? if ($Lev <= 1) {?>
			  |&nbsp;<a href="#" class="ico <?=$del?>" onclick="P_DeleteR('<?=$NeN?>','<?=$CeK?>','<?=$ThN?>','<?=$KiB?>','<?=$mRo['IDT']?>','<?=$RegGrp?>','<?=$ReO?>'); return false">Delete</a>			  </td>
			  <? } ?>
			  <? } ?>
            </tr>
            <?
		...
		?>
          ...
          <?
			...
			$fUlrR= "FrmG=".$_GET['FrmG']."&IdL=".$_GET['IdL']."&gExt=".$gExt."&gFin=".$gFin."&gUnt=".$gUnt."&gSub=".$gSub."&gUpb=".$gUpb."&gThn=".$gThn."&gBid=".$gBid."&gKel=".$gKel."&gOBJ=".$gOBJ."&gRin=".$gRin."&";
...
<script language="javascript">
			...
			if (win!=null)
    			{
      				win.window.document.open()       			
					<?
						$URL_Top = "Form_Asset_D_Top.php?FrmG=".$_GET['FrmG']."&IdL=".$_GET['IdL'];
						$URL_Mid = "Form_Asset_D_Mid.php?gUnt=".$zUnt."&gSub=".$zSub."&gUpb=".$zUpb."&gBid=".$zBid."&gKel=".$zKel."&gOBJ=".$zOBJ."&gRin=".$zRin."&IdL=".$_GET['IdL'];
						$URL_Bot = "Form_Asset_D_Bot.php";
					?>       			
			...
			if (win!=null)
    			{
      				win.window.document.open()       			
					<?
						$URL_Top = "Find_Acc_Top.php?CrAcc=KIB-D&FrmG=".$_GET['FrmG']."&gUnt=".$zUnt."&gSub=".$zSub."&gUpb=".$zUpb."&IdL=".$_GET['IdL'];
						$URL_Mid = "Find_Acc_Mid.php?CrAcc=KIB-D&FrmG=".$_GET['FrmG']."&gUnt=".$zUnt."&gSub=".$zSub."&gUpb=".$zUpb."&IdL=".$_GET['IdL'];
	...
	function EditData(w,h,pos,IDT)
	{
		...
		{
			win.window.document.open()       			
			URL_Top = "Form_Asset_D_Top.php?"+"<?="FrmG=".$_GET['FrmG']?>";
			URL_Mid = "Form_Asset_D_Mid.php?rIDT="+IDT+"<?="&IdL=".$_GET['IdL']?>";
			URL_Bot = "Form_Asset_D_Bot.php";
	...
	function MergerData(ReO,NeN,KeY,ThN,KiB,w,h,pos,IDT,IdL)
	{
		...
		if (win!=null)
		{
			...
			URL_Mid = "Merger_Mid.php?CrT=d&rIDT="+IDT+"&IdL="+IdL;
...
function P_OpenDoc(w,h,IdL)
{
	...
	LeftPosition=(screen.width)?(screen.width-w)/2:100; 
	TopPosition=(screen.height)?(screen.height-h)/2:100;
	URL= 'KIB_D_Dokumen.php?CriT=BPK&gUnt='+gUnt+'&gSub='+gSub+'&gUpb='+gUpb+'&gBid='+gBid+'&gKel='+gKel+'&gOBJ='+gOBJ+'&gRin='+gRin+'&gExt='+gExt+'&gFin='+gFin+'&gThn='+gThn+'&IdL='+IdL;
	settings='width='+w+',height='+h+',top='+TopPosition+',left='+LeftPosition+',location=no,directories=no,status=no,menubar=yes,toolbar=yes,resizable=ya,maximize=yes,scrollbars=yes,navigation=no';
	window.open(URL,'',settings);
}

...
if ($Smp=="Add")
	{
		$gKD = fGlobal("Kd_Aset","Ref_Rek_Aset4","IDT",$rIDT,"=","","");
		$SQ  = "INSERT INTO ta_masa_manfaat SET 
		Kode='$gKD', tA='0', tB='0', tUmur='0'";
		$rs = mysql_query($SQ) or die(mysql_error());		
		
		$URL="Form_Kode_Aset4_Mid.php?FrmG=".$FrmG."&IdL=".$IdL."&rIDT=".$rIDT;
		header("Location: ".$URL);
	}
else if ($Smp=="DelItem")
	{
...
else if ($Smp=="Save")
	{
		if ($rIDT!="")
		{
			$SQL = "UPDATE ref_rek_aset4 SET 
			Nm_Aset='$gNmA', Ms_Manfaat='$gUMR', nilaiExtracom='$gEXT' WHERE IDT='".$rIDT."'";
			$rst = mysql_query($SQL) or die(mysql_error());	
			
			$gCod= fGlobal("Kd_Aset","Ref_Rek_Aset4","IDT",$rIDT,"=","","");
		...
		{
			//CARI KODE TERAKHIR
			$nSQL = "SELECT IFNULL(MAX(Kd_Aset),0) AS LasKd FROM ref_rek_aset4 WHERE Kd_Aset LIKE '".$gJNS.".__'";
			...
			$SQL = "INSERT INTO ref_rek_aset4 SET 
			Kd_Aset='$NewKode', Nm_Aset='$gNmA', Ms_Manfaat='$gUMR', nilaiExtracom='$gEXT'";
			$rst = mysql_query($SQL) or die(mysql_error());		
			
			$rIDT = fGlobal("IDT","Ref_Rek_Aset4","Kd_Aset",$gJNS.".__","LIKE","IDT desc LIMIT 1","");
		}
		$URL="Form_Kode_Aset4_Mid.php?FrmG=".$FrmG."&IdL=".$IdL."&rIDT=".$rIDT;
		header("Location: ".$URL);
	}

...
<table border='0' width='526' cellspacing='1' style='font-family: Calibri; font-size: 10pt; font-weight: bold; border-collapse: collapse'>
  ...
  <?
		...
		for ($i=0; $i<=$gMax; $i=$i+5000)
		{
			...
			<td>
			<a href='#' class="ico docu" onClick="P_OpenDoc('Report_LHI_III_B_14','800','400','<?=$ReC?>','<?=$TtD?>','<?=$HeA?>','<?=$LoadEND?>','<?=$_GET['fHR']?>','<?=$_GET['fBL']?>','<?=$_GET['fTH']?>','<?=$_GET['AsT']?>','<?=$_GET['UpB']?>','<?=$_GET['SuB']?>','<?=$_GET['UnT']?>','<?=$_GET['IdL']?>'); return false">&nbsp;&nbsp;&nbsp;LHI ( III.B.14 ) - <? echo "( Record &nbsp;&nbsp; => &nbsp;&nbsp; ".fConvertToRupiahBulat($gA)." &nbsp; s/d &nbsp; ".fConvertToRupiahBulat($gB)." )"?></a>

...
<?php
extract($_GET);
$gREF= "RPB.".(fGetDate('year')+1).".XXXXXXXX";
...
if ($IdT)
{
	$nSQL= "SELECT Referensi, Kd_Unit, Tahun, Uraian FROM ta_rpbmd_new WHERE IDT='$IdT'";
	...
	$stLOCK = fGlobalNEW("pemindahtangananBMD","ta_unit_lock","kdUnit:tahunBMD",$gUNT.":".$gTHN,"=:=","",DatabaseSB,$ConSB,"");
	//echo $stLOCK;
}
...
<form name="myfrm" method="POST" action="<?="RPBMD_New_Frm_.php?FrmG=".$_GET['FrmG']."&IdT=".$IdT."&IdL=".$_GET['IdL']?>" enctype="multipart/form-data">
...
<table border="0" class="table-link" cellspacing="0" cellpadding="0" align="center" style="width:99%; background:#CCCCCC">
  <tr>
    <td width="16">&nbsp;</td>
    <td width="96">&nbsp;</td>
    <td width="25">&nbsp;</td>
  ...
  <tr height="25">
	...
	<input name="dUNT" type="text" value="<?=$dUNT?>" <?php if (!$IdT) {?> onClick="showUNIT('<?=$stLOCK?>','','<?=$_GET['IdL']?>')" onkeypress="if (event.keyCode==13) {showUNIT('<?=$stLOCK?>','find','<?=$_GET['IdL']?>'); return false;} else if (event.keyCode==27) {closeCLICK('unit'); return false;}" <?php } else {echo "readonly";}?> style="padding-left:5px; width:372px; border: 1px solid #C0C0C0; text-transform:uppercase"/>
  ...
  <tr height="25">
    ...
    <td align="center">:</td>
    <td>
	<select class="boxs" name="fTHN" tabindex="0" style="width:65px">
    <?php
	for ($i=2019; $i<=2030; $i++)
	{
		$sel ="";
...
<table border="0" class="table-link" cellspacing="0" cellpadding="0" align="center" style="width:99%; height:30px; background: #EAE6E6">
  <tr>
    ...
    <td><input type="button" name="B39" <?=$DisA?> value="SAVE" onclick="Save('<?=$stLOCK?>')" style="width: 80px; height: 21px" />
	  ...
	  <!--input type="button" name="B12" value="ADD ASET" onclick="showPROG('','<?=$IdT?>','<?=$_GET['IdL']?>')" style="width: 130px; height: 21px; color:#0000FF" /-->
	  <input type="button" name="B12" value="ADD ASET" onclick="showASET('<?=$stLOCK?>','','<?=$IdT?>','<?=$_GET['IdL']?>')" style="width: 130px; height: 21px; color:#0000FF" />
	  
	  <?=str_repeat("&nbsp;",10)?>
	  <input type="button" name="B13" value="DAFTAR USULAN" onclick="showDATANew('<?=$_GET['IdL']?>')" style="width: 130px; height: 21px" />
	  	  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	  <input type="button" name="B14" <?php if ($IdT=="") {echo "disabled";}?> value="CETAK RINCI" onclick="P_Dokumen('800','400','<?=$IdT?>','<?=$_GET['IdL']?>')" style="width: 120px; height: 21px; color:#0000FF" />
...
<script languange="javascript">
	...
	function showUNIT(stLOCK,CrT,IdL)
	{
		if (CrT=='find')
		{
			...
			{
				$("#unitDiv2Cri").load('RPBMD_New_Frm_Find_Unit_Mid.php?gFnD='+gFnD+'&stLOCK='+stLOCK+'&IdL='+IdL);
			});
		}
		else
		{
			...
			$(document).ready(function()
			{
				$("#unitDiv1Cri").load('RPBMD_New_Frm_Find_Unit_Top.php?stLOCK='+stLOCK+'&IdL='+IdL);
			});
			
			$(document).ready(function()
			{
				$("#unitDiv2Cri").load('RPBMD_New_Frm_Find_Unit_Mid.php?stLOCK='+stLOCK+'&IdL='+IdL);
			});
	...
	*/
	...
	function proslNIL(stLOCK,crt,IdT,gTBL,IdL)
	{
		...
		{
			$("#loadingImg").show();
			$("#ViewDATA").load('RPBMD_New_Frm_Find_PKRK_Aset_Mid_CheckList.php?stLOCK='+stLOCK+'&IdT='+IdT+'&gTBL='+gTBL+'&gCrID='+gCrID+'&IdL='+IdL);
	...
	function showCLICK(stLOCK,crt,mesg,refs,kde,nma,IdL)
	{
		...
		if (crt=='aset') 
		{
			$(document).ready(function()
			{
				$("#ViewDATA").load('RPBMD_New_Frm_Find_PKRK_Aset_Add.php?stLOCK='+stLOCK+'&IdT='+kde+'&rDTA='+nma+'&IdL='+IdL);
			});
		}
		document.getElementById(crt+'MstCri').style.display = "none";
	}
	...
	function RefreshDATA(stLOCK,IdL)
	{
		...
		$(document).ready(function()
		{
			//$("#ViewDATA").load('RPBMD_New_Frm_Data.php?gREF='+gREF+'&IdL='+IdL);
		});
		
		$(document).ready(function()
		{
			$.ajax({
				url:"RPBMD_New_Frm_Data.php",
				data: {stLOCK:stLOCK,gREF:gREF,IdL:IdL},
				type:"get",
				beforeSend:function()
				{
					$("#loadingImg").show();
	...
	*/
	function showDATANew(IdL)
	{
		...
		window.open('RPBMD_New.php?FrmG=DAFTAR RPBMD&gUNT='+gUNT+'&IdL='+IdL,'_self');
	}
	...
	*/
	...
	function showPROG(CrT,IdT,IdL)
	{
		...
		if (CrT=='find')
		{
			...
			$(document).ready(function()
			{
				$("#progDiv2Cri").load('RPBMD_New_Frm_Find_PKRK_Mid.php?gFrm=prog&gFnD='+gFnD+'&IdT='+IdT+'&gUNT='+gUNT+'&IdL='+IdL);
			});
		}
		else
		{
			...
			$(document).ready(function()
			{
				$("#progDiv1Cri").load('RPBMD_New_Frm_Find_PKRK_Top.php?gFrm=prog&IdT='+IdT+'&IdL='+IdL);
			});
			
			$(document).ready(function()
			{
				$("#progDiv2Cri").load('RPBMD_New_Frm_Find_PKRK_Mid.php?gFrm=prog&IdT='+IdT+'&gUNT='+gUNT+'&IdL='+IdL);
			});
	...
	*/
	function showPKRK_add(crt,iD,IdT,IdL)
	{
		document.getElementById(crt+'MstCri').style.display = "none";
		$(document).ready(function()
		{
			//$("#ViewDELL").load('RPBMD_New_Frm_Find_PKRK_Add.php?crt='+crt+'&iD='+iD+'&IdT='+IdT+'&IdL='+IdL);
			$("#ViewDATA").load('RPBMD_New_Frm_Find_PKRK_Add.php?crt='+crt+'&iD='+iD+'&IdT='+IdT+'&IdL='+IdL);
		});
	}
	
	function remoPKRK(stLOCK,rIdT,IdL)
	{
		if (stLOCK=='1'){alert('Access denied, data sudah terkunci..!!'); return false;}
		...
		if (AN)
		{
			$(document).ready(function()
			{
				$("#ViewDELL").load('RPBMD_New_Frm_Find_PKRK_Rem.php?stLOCK='+stLOCK+'&rIdT='+rIdT+'&IdL='+IdL);
			});
		}
	}
	
	function showKEGI(CrT,rIdT,IdL)
	{
		document.getElementById('progMstCri').style.display = "none";
		document.getElementById('reknMstCri').style.display = "none";
		var gUNT = objfrm.fUNT.value;
		if (CrT=='find')
		{
			...
			$(document).ready(function()
			{
				$("#kegiDiv2Cri").load('RPBMD_New_Frm_Find_PKRK_Mid.php?gFrm=kegi&gFnD='+gFnD+'&rIdT='+rIdT+'&gUNT='+gUNT+'&IdL='+IdL);
			});
		}
		else
		{
			...
			$(document).ready(function()
			{
				$("#kegiDiv1Cri").load('RPBMD_New_Frm_Find_PKRK_Top.php?gFrm=kegi&rIdT='+rIdT+'&IdL='+IdL);
			});
			
			$(document).ready(function()
			{
				$("#kegiDiv2Cri").load('RPBMD_New_Frm_Find_PKRK_Mid.php?gFrm=kegi&rIdT='+rIdT+'&gUNT='+gUNT+'&IdL='+IdL);
			});
	...
	function showREKN(CrT,rIdT,IdL)
	{
		document.getElementById('progMstCri').style.display = "none";
		document.getElementById('kegiMstCri').style.display = "none";
		var gUNT = objfrm.fUNT.value;
		if (CrT=='find')
		{
			...
			$(document).ready(function()
			{
				$("#reknDiv2Cri").load('RPBMD_New_Frm_Find_PKRK_Mid.php?gFrm=rekn&gFnD='+gFnD+'&rIdT='+rIdT+'&gUNT='+gUNT+'&IdL='+IdL);
			});
		}
		else
		{
			...
			$(document).ready(function()
			{
				$("#reknDiv1Cri").load('RPBMD_New_Frm_Find_PKRK_Top.php?gFrm=rekn&rIdT='+rIdT+'&IdL='+IdL);
			});
			
			$(document).ready(function()
			{
				$("#reknDiv2Cri").load('RPBMD_New_Frm_Find_PKRK_Mid.php?gFrm=rekn&rIdT='+rIdT+'&gUNT='+gUNT+'&IdL='+IdL);
			});
	...
	function saveRECO(rb,crt,fld,rIdT,IdL)
	{
		if (rb=='rb'){
			fld = fld;
		}
		...
		//alert(rIdT); return false;
		$(document).ready(function()
		{
			$("#ViewDELL").load('RPBMD_New_Frm_Find_PKRK_Save.php?crt='+crt+'&fld='+fld+'&rIdT='+rIdT+'&IdL='+IdL);
		});
	}
	
	function showASET(stLOCK,CrT,IdT,IdL)
	{
		if (stLOCK=='1'){alert('Access denied, data sudah terkunci..!!'); return false;}
		var fUNT = objfrm.fUNT.value;
		if (CrT=='find')
		{
			...
			$(document).ready(function()
			{
				$("#asetDiv2Cri").load('RPBMD_New_Frm_Find_PKRK_Aset_Mid.php?gTBL='+gTBL+'&gFnD='+gFnD+'&stLOCK='+stLOCK+'&IdT='+IdT+'&gUNT='+fUNT+'&IdL='+IdL);
			});
		}
		else
		{
			...
			$(document).ready(function()
			{
				$("#asetDiv1Cri").load('RPBMD_New_Frm_Find_PKRK_Aset_Top.php?stLOCK='+stLOCK+'&IdT='+IdT+'&IdL='+IdL);
			});
			
			$(document).ready(function()
			{
				$("#asetDiv2Cri").load('RPBMD_New_Frm_Find_PKRK_Aset_Mid.php?gTBL='+gTBL+'&stLOCK='+stLOCK+'&IdT='+IdT+'&gUNT='+fUNT+'&IdL='+IdL);
			});

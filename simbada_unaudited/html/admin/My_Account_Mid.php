...
?>
<body topmargin="0">
<form name="myfrm" method="POST" action="<? "My_Account_Mid.php?IdL=".$IdL."&fFind=".$gFin ?>">
	...
	if ($tRo > 0)
	{
		do
		{
						...
						if (win!=null)
							{
								...
								?>       			
							txtHTML="<html><head><title>Simbada Kab. Hulu Sungai Tengah</title></head><frameset framespacing='0' border='0' rows='45,*,30' frameborder='0'><frame name='WinFormUser_Top' noresize src='<? echo $URL_Top?>' scrolling='no'><frame name='WinFormUser_Mid' src='<? echo $URL_Mid?>' scrolling='auto'><frame name='WinFormUser_Bot' src= '<? echo $URL_Bot?>' scrolling='no'><noframes><body><p>=>.............??!</p></body></noframes></frameset></html>"            
							win.focus()
			...
			<tr style="cursor: pointer" <?=fBackCLR($iG)?>>
			  <td width="147" height="5"><a href="#" class="<? echo $fIco ?>" onClick="EditData<? echo $mRo['IDT']?>('800','450','center'); return false">&nbsp;&nbsp;<? echo $mRo['User_ID']?></a></td>
			  <td width="180"><? echo $mRo['Full_Name']?></td>
			  <td width="161"><? echo $tAdm?></td>
			  <td width="397"><? echo $NmSkpD?></td>
			  <td width="35" class="ac"><? echo $Act?></td>
			  <td width="91" class="ac"><a href="#" class="ico del" onClick="P_Delete('<? echo $mRo['IDT']?>','<?=$ReO?>')">Delete</a>&nbsp;&nbsp;&nbsp;<a href="#" class="ico edit" onClick="EditData<? echo $mRo['IDT']?>('800','450','center'); return false">Edit</a></td>
			</tr>
			<? 
			$iG++;
		}

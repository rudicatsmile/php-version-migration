...
{
	#<=2019
	$CrT ="WHERE (kdRekening LIKE '%$FnD%' OR nmRekening LIKE '%$FnD%')";
	#<=2020
...
<table align="center" cellpadding="0" class="table-listpop" cellspacing="0" width="100%" height="240" border="0" style="font-family:calibri; font-size:9pt">
	...
	$nSQ = "SELECT kdRekening, nmRekening FROM ta_apbd_rekening_skpd $CrT GROUP BY kdRekening";
	#echo $nSQ;
	$nRs = mysql_query($nSQ);
	while ($mRo = mysql_fetch_array($nRs, MYSQL_BOTH))
	{
		...
		<tr height="20">
			...
			<td valign="top" width="100" <?=fBackCLR($iG)?> style="border-bottom:1px dotted #ccc; border-right:1px dotted #ccc; text-align:center"><?=$gKD?></td>
			...
			<td valign="top" width="60" <?=fBackCLR($iG)?> style="border-bottom:1px dotted #ccc; text-align:center"><a href="#" class="ico add" onclick="showCLICK('rekn','<?=$gKD?>','<?=$gNM?>','<?=$IdL?>'); return false;">Add</a></td>
		</tr>
		<?
		$iG++;
	}

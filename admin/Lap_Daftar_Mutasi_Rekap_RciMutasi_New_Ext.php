...
$nSQL= "SELECT Kd_Aset FROM ta_kib_post_108_mutasi WHERE Kd_UPB LIKE '$gUnt%' GROUP BY Kd_Aset ORDER BY Kd_Aset";
$nRs = mysql_query($nSQL);
while ($mRo = mysql_fetch_array($nRs, MYSQL_BOTH))
{
	$tID = fGlobal("IDT","ref_rek_aset108_7_temp","Kd_Aset:Kd_Unit",$mRo[0].":".$gUnt,"=:=","","");
...
while ($mRo = mysql_fetch_array($nRs, MYSQL_BOTH))
{
	$rCeK = fGlobal("IDT","ref_rek_aset108_7","Kd_Aset",$mRo[0],"=","","");	
	if (!$rCeK){
		$wSQL="INSERT INTO ref_rek_aset108_7_temp SET Kd_Aset='".$mRo[0]."', Nm_Aset='..........????????'";
		$nRW = mysql_query($wSQL);
	}
}
...
<table border="0" align="center" width="4000" cellspacing="1" style="font-family: Calibri; font-size: 8pt; border-collapse: collapse" id="table1">
	...
	<tr>
		...
		<tr>
			<td width="85">SKPD</td>
			<td width="31">:</td>
			<td width="74"><?php if ($xUnt!="") {echo $xUnt;}?></td>
			<td width="3797"><?php if ($xUnt=="") {echo "<i>SEMUA</i>";} else {echo strtoupper(fGlobal("Nm_Unit","Ref_Unit","Kd_Unit",$xUnt,"=","",""));}?></td>
		  </tr>
		<!--tr>
	...
	<tr>
	  ...
	  <table border="1" width="4000" cellspacing="1" style="font-family: arial; font-size: 8pt; border-collapse: collapse" bordercolor="#000000">
        ...
        <?php
			...
			?>
        <?php function ViewRincian($x1,$x2,$x3,$x4,$x5,$x6,$x7,$x8,$x9,$x10,$x11,$x12,$x13,$x14,$x15,$x16,$x17,$x18,$x19,$x20,$x21,$x22,$x23,$x24,$x25,$x26,$x27,$x28,$x29,$x30,$x31,$x32,$x33,$x34,$x35,$x36,$x37,$x38,$x39,$x40,$x41,$x42,$x43,$x44,$x45,$x46,$x47,$x48,$x49,$x50,$x51,$x52,$x53,$x54,$x55,$x56,$x57,$xR,$mUnt,$Thn,$Slh,$Wrn,$UID,$xB) {?>
        <tr <?=$xR?>>
          ...
          <td><?php if ($Slh!=0 && $UID=="creator"){echo "selisih ".$Slh;}?><?=$x57?></td>
        </tr>
        <?php } ?>
...
function ItemTambahThnN($iGR,$KdAsTR,$ThnR,$gUpbR,$LoadExtrac,$LoD)
{
	...
	#$SdhMtsi = fGlobal("IfNull(count(*),0)", "ta_kib_108_mutasi","Kd_Aset_108:Tgl_Perolehan:Tgl_Mutasi:Kd_UPB:extracom",$KdAsTR."%:".$ThnR."-%-%:".$ThnR."-%-%:".$gUpbR."%:".$LoadExtrac,"LIKE:LIKE:LIKE:LIKE:LIKE","",$LoD);
	$SdhMtsi = fGlobal("IfNull(count(*),0)", "ta_kib_108_mutasi","Kd_Aset_108:Tgl_Perolehan:Tgl_Mutasi_Masuk:Kd_UPB:extracom",$KdAsTR."%:".$ThnR."-%-%:0000-00-00:".$gUpbR."%:".$LoadExtrac,"LIKE:LIKE:=:LIKE:LIKE","",$LoD);
...
{
	$MashAda = 0;//fGlobal("IfNull(count(*),0)", "ta_kib_108","Kd_Aset_108:Tgl_Perolehan:Ref_Mutasi:Kd_Upb:extracom",$KdAsTR."%:".$ThnR."-%-%"."::".$gUpbR."%:".$LoadExtrac,"LIKE:LIKE:=:LIKE:LIKE","",$LoD);
	#$SdhMtsi = fGlobal("IfNull(count(*),0)", "ta_kib_108_mutasi","Kd_Aset_108:Tgl_Perolehan:Tgl_Mutasi:Kd_UPB:extracom",$KdAsTR."%:".$ThnR."-%-%:".$ThnR."-%-%:".$gUpbR."%:".$LoadExtrac,"LIKE:LIKE:LIKE:LIKE:LIKE","",$LoD);
	$SdhMtsi = fGlobal("IfNull(count(*),0)", "ta_kib_108_mutasi","Kd_Aset_108:Tgl_Perolehan:Tgl_Mutasi_Masuk:Kd_UPB:extracom",$KdAsTR."%:".$ThnR."-%-%:0000-00-00:".$gUpbR."%:".$LoadExtrac,"LIKE:LIKE:=:LIKE:LIKE","",$LoD);
	return $MashAda+$SdhMtsi;
}
...
{
	$MashAda = 0;//fGlobal("IfNull(sum(Debet),0)", "ta_kib_post_108","Kd_Aset_108:Tanggal:Ref_Mutasi:Kd_Upb:extracom",$KdAsTR."%:".$ThnR."-%-%::".$gUpbR."%:".$LoadExtrac,"LIKE:LIKE:=:LIKE:LIKE","",$LoD);
	#$SdhMtsi = fGlobal("IfNull(sum(Debet),0)","ta_kib_post_108_mutasi","Kd_Aset_108:Tanggal:Tgl_Mutasi:Kd_Upb:extracom",$KdAsTR."%:".$ThnR."-%-%:".$ThnR."-%-%:".$gUpbR."%:".$LoadExtrac,"LIKE:LIKE:LIKE:LIKE:LIKE","",$LoD);
	$SdhMtsi = fGlobal("IfNull(sum(Debet),0)","ta_kib_post_108_mutasi","Kd_Aset_108:Tanggal:Tgl_Mutasi_Masuk:Kd_Upb:extracom",$KdAsTR."%:".$ThnR."-%-%:0000-00-00:".$gUpbR."%:".$LoadExtrac,"LIKE:LIKE:=:LIKE:LIKE","",$LoD);
	return $MashAda+$SdhMtsi;
}

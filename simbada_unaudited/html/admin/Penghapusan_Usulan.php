...
<table border="0" class="table-link" cellspacing="0" cellpadding="0" align="center" style="width:99%; color:#fff; background:#79a86a">
  ...
  <tr height="25">
    ...
    <td>FIND</td>
    <td style="padding-right:5px"><input name="fFnD" type="text" value="" onkeypress="if (event.keyCode==13) {RefreshDATA('<?=$FrmG?>','<?=$IdL?>'); return false;}" style="padding-left:5px; width:140px; border: 1px solid #C0C0C0"/>
...
<table border="0" cellspacing="0" class="table-link" cellpadding="0" align="center" style="width:99%; height:20px; background: #C9DCD8; font-weight:bold">
  <tr>
    ...
    <td width="115" align="left">REF. / NOMOR</td>
    <td width="225" align="left">SKPD</td>
    <td width="210" align="left">NO.DOKUMEN</td>
    <td width="72" align="left">TGL.DOC</td>
    <td width="274" align="left">KETERANGAN</td>
    <td width="115" align="left">N I L A I</td>
    <td width="155" align="left">ACTION</td>
...
<script languange="javascript">
	...
	function RefreshDATA(FrmG,IdL)
	{
		...
		//{
		//	$("#ViewDATA").load('Penghapusan_Usulan_Data.php?FrmG='+FrmG+'&gUnT='+gUnT+'&gHR='+gHR+'&gBL='+gBL+'&gTH='+gTH+'&gHRd='+gHRd+'&gBLd='+gBLd+'&gTHd='+gTHd+'&gFnD='+gFnD+'&gJNS='+gJNS+'&IdL='+IdL);
		//});
		
		$(document).ready(function()
		{
			$.ajax({
				url:"Penghapusan_Usulan_Data.php",
	...
	{
		window.open('Penghapusan_Usulan_Frm.php?IdT='+IdT+'&FrmG='+FrmG+'&IdL='+IdL,'_self');
	}
	...
	function showDOC(pil,w,h,IdL)
	{
		...
		LeftPosition=(screen.width)?(screen.width-w)/2:100; 
		TopPosition=(screen.height)?(screen.height-h)/2:100;
		URL='Penghapusan_Usulan_Frm_Data_Dok_'+pil+'.php?gUNT='+gUNT+'&JeNS='+JeNS+'&HR1='+HR1+'&HR2='+HR2+'&HR3='+HR3+'&BL1='+BL1+'&BL2='+BL2+'&BL3='+BL3+'&TH1='+TH1+'&TH2='+TH2+'&TH3='+TH3+'&IdL='+IdL;
		settings='width='+w+',height='+h+',top='+TopPosition+',left='+LeftPosition+',location=no,directories=no,status=no,menubar=no,toolbar=no,resizable=no,maximize=no,scrollbars=yes,navigation=no';
		window.open(URL,'',settings);
		//closeCLICK('choise');
	}
	...
	function showDELE(FrmG,ReO,IdT,Cek,IdL)
	{
		...
		if (AN)
		{
			...
			{
				$.ajax({
					url:"Penghapusan_Usulan_Data_Dell.php",
	...
	function showDELEAll(FrmG,IdT,Cek,IdL)
	{
		...
		if (AN)
		{
			...
			{
				$.ajax({
					url:"Penghapusan_Usulan_Data_Dell_All.php",
					data: {FrmG:FrmG,IdT:IdT,IdL:IdL},
					type:"get",
					beforeSend:function()
					{
	...
	function showFORM(crt,IdT,gIdL)
	{
		if (crt=='refr')
		{
			$(document).ready(function()
			{
				$("#formDiv2Veri").load('Penghapusan_Usulan_Veri_Frm_Veri_Mid.php?IdT='+IdT+'&IdL='+gIdL);
			});
		}
		else
		{
			...
			$(document).ready(function()
			{
				$("#formDiv1Veri").load('Penghapusan_Usulan_Veri_Frm_Veri_Top.php?IdL='+gIdL);
			});
	...
	*/
	...
	function showEXEC_RefR(crt,IdT,gIdL)
	{
		$(document).ready(function()
		{
			$("#formDiv2Exec").load('Penghapusan_Usulan_Exec_Frm_Mid.php?IdT='+IdT+'&IdL='+gIdL);
		});
	}
	...
	function showEXEC(crt,IdT,gIdL)
	{
		...
		$(document).ready(function()
		{
			$("#formDiv1Exec").load('Penghapusan_Usulan_Exec_Frm_Top.php?crt='+crt+'&IdL='+gIdL);
		});
	...
	function unexecFORM(tID,IdT,IdL)
	{
		...
		if (AN)
		{
			//$(document).ready(function()
			//{
			//	$("#ViewDATA").load('Penghapusan_Usulan_Exec_Frm_Save.php?rBatal='+rBatal+'&tID='+tID+'&IdT='+IdT+'&IdL='+IdL);
			//	//$("#formDiv2Exec").load('Penghapusan_Usulan_Exec_Frm_Save.php?tID='+tID+'&IdT='+IdT+'&IdL='+IdL);
			//});
	...
	function execFORM(KdA,JenS,tbl,wrn,exc,tID,IdT,IdL)
	{
		...
		if (AN)
		{
			//$(document).ready(function()
			//{
			//	$("#ViewDATA").load('Penghapusan_Usulan_Exec_Frm_Save.php?tID='+tID+'&IdT='+IdT+'&IdL='+IdL);
			//	//$("#formDiv2Exec").load('Penghapusan_Usulan_Exec_Frm_Save.php?tID='+tID+'&IdT='+IdT+'&IdL='+IdL);
			//});

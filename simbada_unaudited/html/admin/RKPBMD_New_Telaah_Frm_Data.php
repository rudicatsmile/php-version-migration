...
function RowList($stLOCK,$CrT,$iG,$mRo1,$mRo2,$mRo3,$rIdT,$gTHN,$gUBH,$IdL,$pad,$DatabaseSB,$ConSB,$xB,$Link,$eCeK)
{
	...
	?>
	<tr height="25"> 
	  <td width="36" rowspan="<?=$span?>" style="border-bottom:1px #999999 dotted; text-align:center" <?=$gBG?>><? if ($iG){echo $iG.".";}?></td>
	  ...
	  <td width="415" rowspan="<?=$span?>" style="border-bottom:1px #999999 dotted; padding-left:<?=$pad?>px; border-left:1px #ccc solid" <?=$gBG?>><? if ($CrT=='rekn') {echo $xB."<b>".$mRo3."</b>".$gJnSR;} else {echo strtoupper($xB.$mRo3);}?></td>
	  ...
	  <?
	   ...
	   if ($CrT=='rekn'){
			...
			<tr>
			  ...
			  <td>&nbsp;</td>
			  <td>
			  <label><input name="fRadio<?=$rIdT?>" <?=$eDis?> type="radio" onclick="saveRECOx('<?=$stLOCK?>','rb','kondisi_barang','B','<?=$rIdT?>','<?=$_GET['IdL']?>'); return false;" value="B" <? if ($gKonD=='B'){echo "checked";}?> />B</label>
			  <label><input name="fRadio<?=$rIdT?>" <?=$eDis?> type="radio" onclick="saveRECOx('<?=$stLOCK?>','rb','kondisi_barang','RR','<?=$rIdT?>','<?=$_GET['IdL']?>'); return false;" value="RR" <? if ($gKonD=='RR'){echo "checked";}?> />RR</label>
			  <label><input name="fRadio<?=$rIdT?>" <?=$eDis?> type="radio" onclick="saveRECOx('<?=$stLOCK?>','rb','kondisi_barang','RB','<?=$rIdT?>','<?=$_GET['IdL']?>'); return false;" value="RB" <? if ($gKonD=='RB'){echo "checked";}?> />RB</label>			  </td>

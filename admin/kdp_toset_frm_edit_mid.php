...
<table align="center" class="table-list" cellpadding="0" cellspacing="0" width="100%" height="100%" border="0" style="border:0px"> 
  <?php 
	...
	{ 
		...
		?> 
          <tr height="90" style="cursor:pointer  <?=$CoL?>">  
			...
			<input name="fInduk<?=$IdR?>" id="fInduk<?=$IdR?>" type="radio" value="Y" <?=$YeS?> <?php if ($ExT=='Y'){echo 'disabled';}?> onclick="IndKDP('kdpt','<?=$IdR?>','<?=$IdT?>','<?=$IdL?>'); return false;" />
            </td>
    ...
    ?> 
  <tr height="100%">  
    <td colspan="9" style="border-bottom:1px dotted #999; border-right:1px solid #ccc; text-align:center">&nbsp;&nbsp;<?php if ($iG==1){echo "Data tidak ditemukan..!";}?></td> 
  </tr> 
</table>

...
?>
<body onload="javascript:myfrm.fUnt.focus()">
  ...
  <table border="0" align="center" width="1858">
    ...
    <tr>
      ...
      <option value="">ALL</option>
		...
		$nSQ = "SELECT idProgram, nmProgram FROM ta_apbd_program_skpd WHERE kdUnit = '".$mSKD."' and periode='$tTbl' and apbd='$uTbl' ORDER BY idProgram";
		$nRs = mysql_query($nSQ) or die(mysql_error());
    ...
    <tr>
      ...
      <option value="ALL">ALL</option>
		...
		{
			$zKG = "";
			$nSQ = "SELECT idKegiatan, nmKegiatan FROM ta_apbd_kegiatan_skpd WHERE kdUnit = '".$mSKD."' AND idKegiatan LIKE '".$zPR."%' and periode='$tTbl' and apbd='$uTbl' GROUP BY idKegiatan";
			$nRs = mysql_query($nSQ) or die(mysql_error());
			while ($mRo = mysql_fetch_array($nRs, MYSQL_BOTH))
			{
				$sel = "";
    ...
    <tr>
      ...
      <option value="ALL">ALL</option>
		...
		{
			$zSB = "";
			$nSQ = "SELECT idSubKegiatan, nmSubKegiatan FROM ta_apbd_kegiatan_sub_skpd WHERE kdUnit = '".$mSKD."' AND idSubKegiatan LIKE '".$zKG."%' and periode='$tTbl' and apbd='$uTbl' ORDER BY idSubKegiatan";
			$nRs = mysql_query($nSQ) or die(mysql_error());
			while ($mRo = mysql_fetch_array($nRs, MYSQL_BOTH))
			{
				$sel = "";
    ...
    <tr>
	  ...
	  <select name="fRK" style="width:600px; border: 1px solid #C0C0C0; padding-left: 1px; padding-right: 1px; padding-top: 1px; padding-bottom: 1px" onchange="this.form.submit()">
		...
		if ($zKG!="")
		{
			...
			$nSQ = "SELECT kdRekening, nmRekening FROM ta_apbd_rekening_skpd WHERE kdUnit = '".$mSKD."' AND idSubKegiatan = '".$zSB."' and periode='$tTbl' GROUP BY kdRekening";
			$nRs = mysql_query($nSQ) or die(mysql_error());
			while ($mRo = mysql_fetch_array($nRs, MYSQL_BOTH))
			{
				$sel = "";
    ...
    <tr> 
      ...
      <td valign="middle"> <table border="0" width="100%" cellpadding="0" style="border-collapse: collapse">
          <tr> 
            <td width="230"><input class="text" type="text" name="fFind" value="<?php echo $gFin?>" style="font-family: Calibri; font-size: 10pt; border: 1px solid #C0C0C0; padding-left: 4px; padding-right: 4px; padding-top: 1px; padding-bottom: 1px; background-color: #E1F986; width:200px" /></td>
            ...
            <td>
			<input type="button" name="B39" value="GO" onclick="P_Find()" style="width: 50px; height: 21px; padding-left: 0px; padding-right: 0px; padding-top: 0px; padding-bottom: 0px" />
            <input type="button" name="B35" value="INPUT DATA" onclick="InputData('950','520','center')" style="width: 100px; height: 21px; padding-left: 0px; padding-right: 0px; padding-top: 0px; padding-bottom: 0px" />			</td>
          </tr>
        </table></td>
    </tr>
    <tr> 
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr> 
        ...
        <th width="80" class="ac" style="border-right: #fff dotted 1px; padding-left:0px; padding-right:0px">PEMBAYARAN</th>
        <th width="70" class="ac" style="border-right: #fff dotted 1px; padding-left:0px">POSTING KE</th>
	  ...
	  if ($gUpb=="All") {$zUpb = $zSub.".%";}
		...
		$nSQL= "SELECT * FROM ta_penerimaan_berkas WHERE Kd_SubKegiatan LIKE '$SyTKG' AND Kd_Rek13 LIKE '$SyTRK' AND Tanggal LIKE '".$gThn."-%-%' AND Kd_Unit LIKE '".$zUnt."' ".$fFindSy." ORDER BY Tanggal, Nomor LIMIT $Offset, $DataPerPage";
		#if ($Lev == 0){echo $nSQL;}
		...
		do
		  ...
		  <tr <?=fBackCLR($iG)?>> 
			...
			<td valign="top" style="border-bottom: #999 dotted 1px; border-right: #999 dotted 1px; padding-left:3px <?=$Wrn?>; padding-right:3px; text-align:center; font-style:italic"><?=$CrTB?></td>
			<td valign="top" style="border-bottom: #999 dotted 1px; border-right: #999 dotted 1px; padding-left:3px <?=$Wrn?>; text-align:center; font-style:italic"><?=str_replace("N","No",str_replace("Y","Yes",$mRo['CritBayar_PostingKe']))?></td>
			...
			<a href="#" class="ico <?=$edit?>" onclick="EditData('950','520','center','<?=$mRo['IDT']?>'); return false">Edit</a>&nbsp;&nbsp;
			<a href="#" class="ico <?=$dele?>" onclick="P_DeleteR('<?=$DelT?>','<?=$mRo['IDT']?>','<?=$mRo['Proses']?>'); return false">Delete</a>&nbsp;&nbsp;
			<a href="#" class="ico reff" onclick="P_Repair('<?=$mRo['IDT']?>','<?=$_GET['IdL']?>'); return false;">Repair</a>
			</td>
      </tr>
      <?php
			  $iG++;
...
<script language="javascript">
			...
			if (win!=null)
    			{
					...
					<?php
						...
						$URL_Mid = "Penerimaan_Berkas_Mid.php?gUnt=".$zUnt."&gSub=".$zSub."&gUpb=".$zUpb."&gBid=".$zBid."&IdL=".$_GET['IdL'];
	...
	function EditData(w,h,pos,IDT)
	{
		...
		{
			win.window.document.open()       			
			URL_Top = "Penerimaan_Berkas_Top.php?"+"<?="FrmG=".$_GET['FrmG']?>";
			URL_Mid = "Penerimaan_Berkas_Mid.php?gIdT="+IDT+"<?="&IdL=".$_GET['IdL']?>";
			URL_Bot = "Penerimaan_Berkas_Bot.php";

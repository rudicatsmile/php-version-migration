...
if ($rIDT!="")
{
	...
	if ($tRo > 0)
	{
		...
		$gMSM  = fGlobal("Ms_Manfaat","Ref_Rek_Aset108_7","Kd_Aset",$mRo['Kd_Aset_108'],"=","","");
...
?>
<body>
<form name="myfrm" method="post" action="<?php echo "Form_Asset_G_Mid_.php?IdL=".$_GET['IdL']."&rIDT=".$rIDT ?>">
<input type="hidden" name="Simpan">
  <table border="0" align="center" width="1165">
    ...
    <tr> 
      <td>TGL. DOKUMEN</td>
      <td>
	  <select name="fHriD" class="boxs" id="fHriD" tabindex="0" style="width:50px">
	  <option value="00"></option>
          <?
		for($nHri=1; $nHri<=31; $nHri++)
		{
	  ...
	  <tr>
		...
		while ($mRo = mysql_fetch_array($nRs, MYSQL_BOTH))
		{
			...
			<td width="18"><a href="#" onClick="P_PRE('g','800','400','<?=$gIDT?>','<?=$_GET['IdL']?>'); return false" class="ico prev"></a></td>
			<td width="10" style="border-right:1px solid #CCCCCC"><a href="#" onClick="P_PDF('<?=$ReO?>','g','600','300','<?=$rIDT?>','<?=$gIDT?>','<?=$_GET['IdL']?>'); return false" class="ico reff"></a></td>
			<?
			$iG++;
		}
		?>
	  <td><a href="#" onClick="P_PDF('<?=$ReO?>','g','600','300','<?=$rIDT?>','','<?=$_GET['IdL']?>'); return false" class="ico pdf">&nbsp;&nbsp;Upl.PDF</a></td>
	  </tr>
	  </table>	  </td>
    </tr>
    <tr> 
      ...
      <td colspan="2" rowspan="9"> <table border="0" width="430" style="border-collapse: collapse; color: #CC0000; border: 1px solid #8FA908; padding-left: 4px; padding-right: 4px; padding-top: 1px; padding-bottom: 1px">
          ...
          <tr>
			...
			[&nbsp;<a href="#" class="ico doc" onclick="P_OpenDoc('800','400','KIB_J_Dokumen'); return false">DOKUMEN KIB</a>&nbsp;]			</td>
          </tr>
          <tr>
            <td>&nbsp;</td>
            <td>&nbsp;</td>
    ...
    <tr> 
      ...
      <td valign="top"><textarea name="fKeterangan" cols="57" rows="3" id="fKeterangan" style="border: 1px solid #C0C0C0"><? echo $gKTR ?></textarea></td>
      <td>&nbsp;</td>
    </tr>
    <tr> 
      <td>&nbsp;</td>
...
<script language="javascript">
			...
			if (win!=null)
    			{
					...
					<?
						$URL_Top = "Find_Acc_Top.php?CrAcc=Form_Asset_J_Mid&IdL=".$_GET['IdL']."&gUnt=".$zUnt."&gSub=".$zSub."&gUpb=".$zUpb;
						$URL_Mid = "Find_Acc_Mid.php?CrAcc=Form_Asset_J_Mid&IdL=".$_GET['IdL']."&gUnt=".$zUnt."&gSub=".$zSub."&gUpb=".$zUpb;
			...
			if (win!=null)
    			{
      				win.window.document.open()       			
					<?
					$URL_Top = "Form_Invent_Asset_Top.php?FrmG=INVENTARISASI -> PERUBAHAN NILAI -> KIB J (ATB)";
					$URL_Mid = "Form_Invent_Asset_Ubh.php?rKib=Kib_J&IdL=".$_GET['IdL']."&rIDT=".$rIDT;
	...
	function P_ViewHistory(w,h,pos)
	{
		...
		<?
			{
			$URL="KIB_J_History.php?rIDT=".$rIDT."&IdL=".$_GET['IdL'];
			}
		?>
	...
	function P_MoveAccount(ReO,KeY,w,h,rIDT,IdL)
	{
		...
		{
			win.window.document.open()       			
			URL_Top = 'Move_Account_Top.php?FrmG=PEMINDAHAN REKENING ASET -> KIB J (ATB)';
			URL_Mid = 'Move_Account_Mid.php?rKib=Kib_G&IdL='+IdL+'&rIDT='+rIDT;
	...
	function P_MoveUPB(ReO,KeY,w,h,rIDT,IdL)
	{
		...
		{
			win.window.document.open()       			
			URL_Top = 'Move_Upb_Kib_Top.php?FrmG=PEMINDAHAN KODE UPB -> KIB J (ATB)';
			URL_Mid = 'Move_Upb_Kib_Mid.php?rKib=Kib_J&IdL='+IdL+'&rIDT='+rIDT;

<?php
	function ViewObjek($KdOBJ,$mUpb,$mThn,$nThn,$mG,$LoadMutasi,$UID,$LoadExtrac,$rPil)  #****#
	{
		$iGD  = 1;
		$nSQD = "SELECT left(P1.Kd_Aset,14) as Kd_Aset, P2.Nm_Aset 
		FROM ref_rek_aset108_7_temp P1 
		JOIN ref_rek_aset108_6 P2 ON P2.Kd_Aset=left(P1.Kd_Aset,14) 
		WHERE P1.Kd_Aset LIKE '".$KdOBJ.".%' AND P1.Kd_Unit = '".substr($mUpb,0,11)."' GROUP BY left(P1.Kd_Aset,14)";
		...
		while ($mRoD = mysql_fetch_array($nRsD, MYSQL_BOTH))
		{
			...
			#}
				...
				if ($nThn<=2026) {
					...
					$Col[6] = $Col[6] + fGlobal("IfNull(sum(Debet),0)", "ta_kib_post_mutasi","Kd_Aset_108:Tanggal:Tgl_Mutasi:Tgl_Mutasi:Kd_UPB:extracom",$KdAsT."%:".$mThn."-12-31:".$nThn."-01-01:".($nThn+1)."-12-31:".$mUpb."%:".$LoadExtrac,"LIKE:<=:>=:<=:LIKE:LIKE","","");
				}
				else{
			...
			if ($tView!=0) {
				...
				if ($rPil==4) {ViewRciObjek($KdAsT,$mUpb,$mThn,$nThn,$mG,$LoadMutasi,$UID,$LoadExtrac);}
			}
			$iGD++;
		}
	}

	  ...
	  <table width="100%" border="0" cellspacing="0" class="table-list" cellpadding="0" style="border-collapse:collapse; font-size: 9pt; font-family: Calibri">
		...
		do
            ...
            <tr height="20" style="cursor: pointer" <?=fBackCLR($iG)?>> 
              ...
              <td valign="top" style="border-bottom:1px dotted #ccc; border-right:1px solid #ccc; padding-left:3px <?=$fCl?>"><?=$BV.$mRo['Uraian']?><? if ($mRo['Keterangan']!='') {echo "<br><i>".$mRo['Keterangan']."</i>";}?><? if ($mRo['No_Pengadaan']!="" && $mRo['No_Pengadaan']!="-") {echo "<br>No. Pengadaan: ".$mRo['No_Pengadaan']."<br>";}?><? if ($mRo['HasilMerger']=="Y") {echo "<br>".$mRo['Mrg_Ref_History'];}?><? if ($gAsL!='') {echo "<br>Asal-usul : ".$gAsL;}?></td>
              <td valign="top" style="border-bottom:1px dotted #ccc; border-right:1px solid #ccc; text-align:right; padding-right:3px"><? if ($mRo['Debet']!=0) {echo fConvertToRupiah($mRo['Debet']);} else {echo "-";}?></td>

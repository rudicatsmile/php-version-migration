...
<table align="center" class="table-list" cellpadding="0" cellspacing="0" width="100%" height="100%" border="0" style="border:0px"> 
  <?php 
    if ($FnD)  
    { 
		$SyT = "AND (Kd_UPB LIKE '%$FnD%' OR Nm_UPB LIKE '%$FnD%')"; 
        $LmT = ""; 
    } 
    else 
    { 
	...
	$nSQ = "SELECT Kd_UPB as A0, Nm_UPB 
	FROM ref_upb
	WHERE Kd_UPB LIKE '".$SkP."%' $SyT ORDER BY Kd_UPB";
	#echo $nSQ;
	$nRs = mysql_query($nSQ); 
    ...
    ?> 
  <tr height="100%">  
    <td colspan="2" valign="top">&nbsp;&nbsp;<?php if ($iG==1){echo "Data tidak ditemukan..!";}?></td> 
  </tr> 
</table>

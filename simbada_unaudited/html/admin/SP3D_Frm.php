...
<table border="0" class="table-link" cellspacing="0" cellpadding="0" align="center" style="width:1000px">
  ...
  <tr height="27">
	...
	<input type="button" name="B13" value="..." <? if ($IdT=='' && $Lev<=1){?> onclick="findUPB('','<?=$_GET['IdL']?>')" <? } ?> style="width: 27px; height:21px" />
...
<table border="0" class="table-link" cellspacing="0" cellpadding="0" align="center" style="width:1000px; height:30px; background: #EAE6E6">
  <tr>
	...
	<input type="button" name="B12" <?=$DisA?> value="ADD REKENING" <? if ($Lock=='Y'){echo "disabled";}?> onclick="showReknP90('','<?=$IdT?>','<?=$_GET['IdL']?>')" style="width:100px; height: 21px; color:#0000FF" />
	<?=str_repeat("&nbsp;",10)?>
	<input type="button" name="B13" value="DAFTAR " onclick="showDATANew('<?=$_GET['IdL']?>')" style="width: 130px; height: 21px" />
	
	<div style="float:right">
	<input type="button" name="B14" <? if ($IdT=="") {echo "disabled";}?> value="POSISI SALDO AKHIR" onclick="editSALDO('','<?=$Lock?>','<?=$IdT?>','<?=$IdL?>')" style="width:120px; height:21px" />
	<input type="button" name="B14" <? if ($IdT=="") {echo "disabled";}?> value="CETAK" onclick="P_Dokumen('800','400','<?=$IdT?>','<?=$IdL?>')" style="width:80px; height: 21px" />
	</div>
	</td>
    <td width="5">&nbsp;</td>
  </tr>

...
<form name="myfrm" method="post" action="<?php echo "KIB-J_.php?FrmG=".$_GET['FrmG']."&IdL=".$_GET['IdL'] ?>">
  ...
  <table border="0" align="center" style="width:99%">
    ...
    <tr> 
	  ...
	  <select class="boxs" name="fBid" tabindex="0" style="width: 480px" onchange="P_Change()">
		...
		$nSQ = "SELECT Kd_Aset, Nm_Aset FROM ref_rek_aset108_4 WHERE Kd_Aset LIKE '1.5.3.__' ORDER BY Kd_Aset";
		$nRs = mysql_query($nSQ) or die(mysql_error());
		$mRo = mysql_fetch_assoc($nRs);
		$tRo = mysql_num_rows($nRs);
		if ($tRo > 0)
    ...
    <tr> 
      ...
      <td align="right"><input type="button" name="B37" value="EXPORT DATA" disabled onclick="P_ToExcel('800','400','center','ToExcel/KIB_G_ToExcel')" style="width: 90px; height: 24px; padding-left: 0px; padding-right: 0px; padding-top: 0px; padding-bottom: 0px" /></td>
    </tr>
    <tr>
      <td>RUANG</td>
      <td valign="middle">
    ...
    <tr> 
      ...
      <td valign="middle"> <table border="0" width="100%" cellpadding="0" style="border-collapse: collapse">
          <tr> 
            <td width="46%"> <input class="text" type="text" name="fFind" value="<? echo $gFin?>" placeholder='Search' style="width: 200px; font-family: Calibri; font-size: 10pt; padding-left: 4px; padding-right: 4px; padding-top: 1px; padding-bottom: 1px; background-color: #E1F986" /></td>
      ...
      <td width="119" align="right"><input type="hidden" name="B362" value="DOKUMEN KIR" onclick="P_KIR('650','350','<?=$_GET['IdL']?>')"  style="width: 90px; height: 24px; padding-left: 0px; padding-right: 0px; padding-top: 0px; padding-bottom: 0px" /></td>
    </tr>
    <tr> 
      <td>&nbsp;</td>
      <td>&nbsp;</td>
    ...
    <tr> 
	  ...
	  <table align="center" border="0" class="table-list" cellspacing="0" cellpadding="0" style="width:99%">
		<tr>
		  ...
		  <th width="280" class="ac">Actions</th>
	    </tr>
		...
		if ($gFin!="")
		{$fFindSy = "AND (Kd_Aset_108 LIKE '%".$gFin."%' OR Ref_Temp LIKE '%".$gFin."%' OR Nm_Aset LIKE '%".$gFin."%' OR Harga LIKE '%".$gFin."%' OR Referensi LIKE '%".$gFin."%' OR Nomor_Polisi LIKE '%".$gFin."%' OR Nomor_Rangka LIKE '%".$gFin."%' OR Nomor_BPKB LIKE '%".$gFin."%' OR Nomor_Mesin LIKE '%".$gFin."%' OR Keterangan LIKE '%".$gFin."%' OR Merk LIKE '%".$gFin."%')";}
		else
		{$fFindSy = "";}
		
		$nSQA= "SELECT kd_aset FROM ta_kib_108 WHERE referensi LIKE 'ATB%' AND kd_aset_108 ='' GROUP BY kd_aset";
		#echo $nSQA."<br>";
		$nRsA= mysql_query($nSQA) or die(mysql_error());
		while ($mRoA = mysql_fetch_array($nRsA, MYSQL_BOTH))
		{
			$x08A  = fGlobal("kd_aset108","ref_rek_aset5_maping","Kd_Aset17",$mRoA[0],"=","","");
			#echo $mRoA[0]." : ".$x08A."<br>";
			if ($x08A){
				$eU = "UPDATE ta_kib_108 SET kd_aset_108='".$x08A."' WHERE Kd_Aset='".$mRoA[0]."' AND Referensi LIKE 'ATB%'";
				echo $eU."<br>";
				$nR = mysql_query($eU);
				
				$eU = "UPDATE ta_kib_post_108 SET kd_aset_108='".$x08A."' WHERE Kd_Aset='".$mRoA[0]."' AND Referensi LIKE 'ATB%'";
				echo $eU."<br>";
				$nR = mysql_query($eU);
			}
		}
		...
		FROM ta_kib_108 WHERE Kd_UPB LIKE '".substr($zUpb,0,11)."%' AND Tgl_Perolehan LIKE '2017-%-%' ORDER BY Kd_Aset, No_Register";
		$nRsA= mysql_query($nSQA) or die(mysql_error());
		while ($mRoA = mysql_fetch_array($nRsA, MYSQL_BOTH))
		{
			$mRoRf = $mRoA[0];
		...
		*/
		...
		AND Tgl_Perolehan >='".$gThnA."-01-01' AND Tgl_Perolehan <='".$gThn."-12-31' ".$fFindSy.$tMuT."ORDER BY Kd_Aset_108, Tgl_Perolehan, No_Register LIMIT $Offset, $DataPerPage";
		#echo $nSQL;
...
<script language="javascript">
			...
			if (win!=null)
    			{
					...
					<?
						$URL_Top = "Form_Asset_J_Top.php?FrmG=".$_GET['FrmG']."&IdL=".$_GET['IdL'];
						$URL_Mid = "Form_Asset_J_Mid.php?gUnt=".$zUnt."&gSub=".$zSub."&gUpb=".$zUpb."&gBid=".$zBid."&gKel=".$zKel."&gOBJ=".$zOBJ."&gRin=".$zRin."&IdL=".$_GET['IdL'];
						$URL_Bot = "Form_Asset_J_Bot.php";
					?>       			
			...
			if (win!=null)
    			{
					...
					<?
						...
						$URL_Mid = "Open_KIB_Mid.php?CrKIB=KIB_G&gUnt=".$zUnt."&gSub=".$zSub."&gUpb=".$zUpb."&IdL=".$_GET['IdL'];
			...
			if (win!=null)
    			{
      				win.window.document.open()       			
					<?
						$URL_Top = "Find_Acc_Top.php?CrAcc=KIB-J&FrmG=".$_GET['FrmG']."&gUnt=".$zUnt."&gSub=".$zSub."&gUpb=".$zUpb."&IdL=".$_GET['IdL'];
						$URL_Mid = "Find_Acc_Mid.php?CrAcc=KIB-J&FrmG=".$_GET['FrmG']."&gUnt=".$zUnt."&gSub=".$zSub."&gUpb=".$zUpb."&IdL=".$_GET['IdL'];
	...
	function EditData(w,h,pos,IDT)
	{
		...
		{
			win.window.document.open()       			
			URL_Top = "Form_Asset_J_Top.php?"+"<?="FrmG=".$_GET['FrmG']?>";
			URL_Mid = "Form_Asset_J_Mid.php?rIDT="+IDT+"<?="&IdL=".$_GET['IdL']?>";
			URL_Bot = "Form_Asset_J_Bot.php";
	...
	function P_KIR(w,h,IdL)
	{
		...
		TopPosition=(screen.height)?(screen.height-h)/2:100;
		settings='width='+w+',height='+h+',top='+TopPosition+',left='+LeftPosition+',location=no,directories=no,status=no,menubar=yes,toolbar=yes,resizable=ya,maximize=yes,scrollbars=yes,navigation=no';
		window.open('KIB_G_Dokumen_KIR.php?gUpb='+fUpb+'&gRua='+fRua+'&IdL='+IdL,'',settings);
	}
	...
	function P_OpenDoc(w,h,IdL)
	{
		...
		LeftPosition=(screen.width)?(screen.width-w)/2:100; 
		TopPosition=(screen.height)?(screen.height-h)/2:100;
		URL= 'KIB_J_Dokumen.php?CriT=BPK&gUnt='+gUnt+'&gSub='+gSub+'&gUpb='+gUpb+'&gBid='+gBid+'&gKel='+gKel+'&gOBJ='+gOBJ+'&gRin='+gRin+'&gExt='+gExt+'&gFin='+gFin+'&gThn='+gThn+'&gThnA='+gThnA+'&IdL='+IdL;
		settings='width='+w+',height='+h+',top='+TopPosition+',left='+LeftPosition+',location=no,directories=no,status=no,menubar=yes,toolbar=yes,resizable=ya,maximize=yes,scrollbars=yes,navigation=no';
		window.open(URL,'',settings);
	}

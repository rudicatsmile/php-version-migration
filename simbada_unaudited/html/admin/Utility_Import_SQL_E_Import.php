...
while($mRo = sqlsrv_fetch_array($rst))
{
	...
	$SQb = "SELECT harga FROM ta_fn_kib_e WHERE IDPemda='".$mRo[0]."' AND Tahun='2025'";
...
{
	$CekK = fGlobal("IDT","ta_kib_108","IdTabelMaster:Referensi",$gIDT.":ATL%","=:LIKE","","");
	if ($CekK=='')
	{
		$NewK = fGlobal("count_referensi","ta_kib_108_count_referensi","ref_crit","ATL","=","","");
		if ($NewK==0)
		{
			...
			$NewC = fGlobal("IfNull(max(referensi),0)","ta_kib_108_merger_his","referensi","ATL%","LIKE","","");
		...
		$SQ="UPDATE ta_kib_108_count_referensi SET count_referensi='".$NewK."' WHERE ref_crit='ATL'";
		mysql_query($SQ);

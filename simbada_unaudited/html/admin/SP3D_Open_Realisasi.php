...
<table border="0" class="table-link" cellspacing="0" cellpadding="0" align="center" style="width:670px">
  ...
  <tr height="27">
	...
	<input name="dUPB" id="dUPB" type="text" value="<?=$dUPB?>" <? if (!$IdT) {?> onclick="showUPB('','<?=$_GET['IdL']?>')" onkeypress="if (event.keyCode==13){ showUPB('find','<?=$_GET['IdL']?>'); return false;} else if (event.keyCode==27) {closeCLICK('upb'); return false;}" <? } else {echo "readonly";}?> style="padding-left:5px; width:291px; border: 1px solid #C0C0C0"/>
	<input type="button" name="B13" value="..." <? if (!$IdT) {?> onclick="findUPB('','<?=$_GET['IdL']?>')" <? } ?> style="width: 27px; height:21px" />
...
<script languange="javascript">
	...
	function P_Dokumen(w,h,IdL)
	{
		...
		URL='SP3D_Open_Realisasi_Dok.php?eHR='+eHR+'&eBL='+eBL+'&eTH='+eTH+'&fTH='+fTH+'&UpB='+UpB+'&SmS='+SmS+'&IdL='+IdL;
	...
	function findUPB(CrT,IdL)
	{
		if (CrT=='find'){
			...
			$(document).ready(function()
			{
				$("#findupbDiv2Cri").load('SP3D_Open_Realisasi_Find_All_Upb_Mid.php?FnD='+FnD+'&IdL='+IdL);
			});
		}
		else
		{
			...
			$(document).ready(function()
			{
				$("#findupbDiv1Cri").load('SP3D_Open_Realisasi_Find_All_Upb_Top.php?IdL='+IdL);
			});
			
			$(document).ready(function()
			{
				$("#findupbDiv2Cri").load('SP3D_Open_Realisasi_Find_All_Upb_Mid.php?IdL='+IdL);
			});

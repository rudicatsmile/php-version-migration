<?
		...
		while ($mRoE = mysql_fetch_array($nRsE, MYSQL_BOTH))
		{
			...
			if (fNmHuruf($mG)=="f"){
				...
				if ($nThn<=2016) {
					...
					$Col[5] = $Col[5] + fGlobal("IfNull(count(*),0)", "ta_kib_".fNmHuruf($mG)."_mutasi","Kd_Aset:Tgl_Perolehan:Tgl_Mutasi:Tgl_Mutasi:Tgl_Mutasi_Masuk:Kd_UPB:extracom",$KdAsT."%:".$mThn."-12-31:".$nThn."-01-01:".($nThn+1)."-12-31:".$mThn."-12-31:".$mUpb."%:".$LoadExtrac,"LIKE:<=:>=:<=:<=:LIKE:LIKE","","");
					...
					#$Col[6] = $Col[6] + fGlobal("IfNull(sum(Debet),0)", "ta_kib_post_mutasi","Kd_Aset:Tanggal:Tgl_Mutasi:Tgl_Mutasi:Kd_UPB:extracom",$KdAsT."%:".$mThn."-12-31:".$nThn."-01-01:".($nThn+1)."-12-31:".$mUpb."%:".$LoadExtrac,"LIKE:<=:>=:<=:LIKE:LIKE","","adf");
					$Col[6] = $Col[6] + fGlobal("IfNull(sum(Debet),0)", "ta_kib_post_mutasi","Kd_Aset:Tanggal:Tgl_Mutasi:Tgl_Mutasi:Tgl_Mutasi_Masuk:Kd_UPB:extracom",$KdAsT."%:".$mThn."-12-31:".$nThn."-01-01:".($nThn+1)."-12-31:".$mThn."-12-31:".$mUpb."%:".$LoadExtrac,"LIKE:<=:>=:<=:<=:LIKE:LIKE","","");
				}
				else{
					$Col[5] = fGlobal("unitAkhir", "ta_kib_post_saldo_mutasi","Kd_Unit:Kd_Aset:Tahun",$mUpb.":".$KdAsT.":".$mThn,"=:=:=","","");
					$Col[6] = fGlobal("saldoAkhir", "ta_kib_post_saldo_mutasi","Kd_Unit:Kd_Aset:Tahun",$mUpb.":".$KdAsT.":".$mThn,"=:=:=","","");
			...
			if (fNmHuruf($mG)=="f"){
				...
				$Col[54]   = round(fGlobal("IfNull(sum(Debet),0)", "ta_kib_post","Kd_Aset:Tanggal:Tanggal:Kd_UPB",$KdAsT."%:".$nThn."-01-01".":".$nThn."-12-31:".$mUpb."%","LIKE:>=:<=:LIKE","",""),2);
			}
			else{
	...
	{
		$sldKB = fGlobal("IfNull(sum(debet),0)", "ta_kib_post","Kd_Aset:Kd_Upb:Tanggal:extracom",$KdAsT."%:".$mUpb."%:".$nThn."-12-31:N","LIKE:LIKE:<=:=","",$LoD);
		return $sldKB;
	}
?>
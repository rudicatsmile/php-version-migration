<?php
	function ViewJenis($KdJNS,$mUpb,$mThn,$nThn,$mG,$LoadMutasi,$UID,$LoadExtrac,$rPil)  #***#
	{
		$iGC  = 1;
		$nSQC = "SELECT left(P1.Kd_Aset,11) as Kd_Aset, P2.Nm_Aset 
		FROM ref_rek_aset108_7_temp P1 
		JOIN ref_rek_aset108_5 P2 ON P2.Kd_Aset=left(P1.Kd_Aset,11) 
		WHERE P1.Kd_Aset LIKE '".$KdJNS.".%' AND P1.Kd_Unit = '".substr($mUpb,0,11)."' GROUP BY left(P1.Kd_Aset,11)";
		...
		while ($mRoC = mysql_fetch_array($nRsC, MYSQL_BOTH))
		{
			...
			#if (fNmHuruf($mG)=="f"){
			#	if ($nThn<=2016) {
			#		$Col[5] = fGlobal("IfNull(count(*),0)", "ta_kib_108","Kd_Aset_108:Kd_Upb:KdpToAset:Tgl_Perolehan",$KdAsT."%:".$mUpb."%:N:".$mThn."-12-31","LIKE:LIKE:=:<=","","");
			#		$Col[6] = round(fGlobal("IfNull(sum(Debet),0)", "ta_kib_post_108","Kd_Aset_108:Kd_UPB:KdpToAset:Tanggal",$KdAsT."%:".$mUpb."%:N:".$mThn."-12-31","LIKE:LIKE:=:<=","",""),2);
			#	}
			#	else{
			#		$Col[5] = fGlobal("IfNull(sum(unitAkhir),0)", "ta_kib_post_saldo_mutasi","Kd_Unit:Kd_Aset_108:Tahun",$mUpb.":".$KdAsT.":".$mThn,"=:=:=","","");
			#		$Col[6]  = fGlobal("IfNull(sum(saldoAkhir),0)", "ta_kib_post_saldo_mutasi","Kd_Unit:Kd_Aset_108:Tahun",$mUpb.":".$KdAsT.":".$mThn,"=:=:=","","");
			#	}
			#}
			#else{
				if ($nThn<=2026) {
					...
					$Col[5] = $Col[5] + fGlobal("IfNull(count(*),0)", "ta_kib_108_mutasi","Kd_Aset_108:Tgl_Perolehan:Tgl_Mutasi:Tgl_Mutasi:Kd_UPB:extracom",$KdAsT."%:".$mThn."-12-31:".$nThn."-01-01:".($nThn+1)."-12-31:".$mUpb."%:".$LoadExtrac,"LIKE:<=:>=:<=:LIKE:LIKE","","");
					...
					$Col[6] = $Col[6] + fGlobal("IfNull(sum(Debet),0)", "ta_kib_post_108_mutasi","Kd_Aset_108:Tanggal:Tgl_Mutasi:Tgl_Mutasi:Kd_UPB:extracom",$KdAsT."%:".$mThn."-12-31:".$nThn."-01-01:".($nThn+1)."-12-31:".$mUpb."%:".$LoadExtrac,"LIKE:<=:>=:<=:LIKE:LIKE","","");
				}
				else{
					$Col[5] = fGlobal("IfNull(sum(unitAkhir),0)","ta_kib_post_saldo_mutasi","Kd_Unit:Kd_Aset_108:Tahun",$mUpb.":".$KdAsT."%:".$mThn,"=:LIKE:=","","");
					$Col[6] = fGlobal("IfNull(sum(saldoAkhir),0)","ta_kib_post_saldo_mutasi","Kd_Unit:Kd_Aset_108:Tahun",$mUpb.":".$KdAsT."%:".$mThn,"=:LIKE:=","","");
				}
			#}
			...
			//BERKURANG
			if (fNmHuruf($mG)=="f"){
				$Col[51] = fGlobal("IfNull(count(*),0)", "ta_kib_108","Kd_Aset_108:Tgl_Perolehan:Tgl_Perolehan:Kd_Upb:KdpToAset",$KdAsT."%:".$nThn."-01-01".":".$nThn."-12-31:".$mUpb."%:Y","LIKE:>=:<=:LIKE:=","","");
			...
			//BERTAMBAH
			if (fNmHuruf($mG)=="f"){
				$Col[53] = fGlobal("IfNull(count(*),0)", "ta_kib_108","Kd_Aset_108:Tgl_Perolehan:Tgl_Perolehan:Kd_Upb",$KdAsT."%:".$nThn."-01-01".":".$nThn."-12-31:".$mUpb."%","LIKE:>=:<=:LIKE","","");
				$Col[54] = fGlobal("IfNull(sum(Debet),0)", "ta_kib_post_108","Kd_Aset_108:Tanggal:Tanggal:Kd_Upb",$KdAsT."%:".$nThn."-01-01".":".$nThn."-12-31:".$mUpb."%","LIKE:>=:<=:LIKE","","");
			}
			else{
			...
			if ($tView!=0) {
				ViewRincian($Col[1],$Col[2],$Col[3],$Col[4],$Col[5],$Col[6],$Col[7],$Col[8],$Col[9],$Col[10],$Col[11],$Col[12],$Col[13],$Col[14],$Col[15],$Col[16],$Col[17],$Col[18],$Col[19],$Col[20],$Col[21],$Col[22],$Col[23],$Col[24],$Col[25],$Col[26],$Col[27],$Col[28],$Col[29],$Col[30],$Col[31],$Col[32],$Col[33],$Col[34],$Col[35],$Col[36],$Col[37],$Col[38],$Col[39],$Col[40],$Col[41],$Col[42],$Col[43],$Col[44],$Col[45],$Col[46],$Col[47],$Col[48],$Col[49],$Col[50],$Col[51],$Col[52],$Col[53],$Col[54],$Col[55],$Col[56],$Col[57],"",$mUpb,$nThn,"","",$UID,$xB);
				if ($rPil==3 || $rPil==4) {ViewObjek($KdAsT,$mUpb,$mThn,$nThn,$mG,$LoadMutasi,$UID,$LoadExtrac,$rPil);}
			}
			$iGC++;
		}
	}	

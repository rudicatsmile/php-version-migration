...
$gREF= "SKB.".fGetDate('year').".XXXXXXXX";
$gHR = fGetDate('mday');
$gBL = fGetDate('mon');
$gTH = fGetDate('year');
$gPR = fGetDate('year')+1;
...
if ($IdT)
{
	$nSQL= "SELECT referensi,kd_unit,nomor,tanggal,uraian,tahun,'' 
	FROM ta_rkbmd_standar_kebutuhan WHERE IDT='$IdT'";
...
<form name="myfrm" method="POST" action="<?="rkbmd_standar_kebutuhan_frm_.php?Frm=".$Frm."&IdT=".$IdT."&IdL=".$_GET['IdL']."&FrmG=".$_GET['FrmG']?>" enctype="multipart/form-data">
...
<table border="0" class="table-link" cellspacing="0" cellpadding="0" align="center" style="width:99%">
  ...
  <tr height="25">
	...
	<div id="asetMstCri" class="findrekn0CriByPage">
		<div id="asetDiv1Cri" class="findrekn1CriByPage"></div>
		<div id="asetDiv2Cri" class="findrekn2CriByPage"></div>
		<div id="asetDiv3Cri" class="findrekn3CriByPage"></div>
	</div>	
	</td>
    <td align="right">&nbsp;</td>
    <td align="center">&nbsp;</td>
  ...
  <tr height="25">
	...
	<input name="dUNT" id="dUNT" type="text" value="<?=$dUNT?>" <?php if ($IdT=='' && $Lev <= 1) {?> onClick="showUNIT('','<?=$_GET['IdL']?>')" onkeypress="if (event.keyCode==13) {showUNIT('find','<?=$_GET['IdL']?>'); return false;} else if (event.keyCode==27) {closeCLICK('unit'); return false;}" <?php } else {echo "readonly";}?> style="padding-left:5px; width:400px; border: 1px solid #C0C0C0"/>
...
<table border="0" class="table-link" cellspacing="0" cellpadding="0" align="center" style="width:99%; height:30px; background: #EAE6E6">
  <tr>
    ...
    <td><input type="button" name="B39" <?=$DisA?> value="SAVE" onclick="Save()" style="width: 80px; height: 21px" />
	  ...
	  <input type="button" name="B12" id="B12" <?=$DisA?> value="ADD ITEM" onclick="showASET('','0','<?=$IdT?>','<?=$_GET['IdL']?>')" style="width: 80px; height: 21px; color:#0000FF" /><?=str_repeat("&nbsp;",10)?>
	  <input type="button" name="B13" id="B13" value="DAFTAR TRANSAKSI" onclick="showDATANew('<?=$Frm?>','<?=$_GET['IdL']?>')" style="width: 130px; height: 21px" />
	  &nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;&nbsp;
	  <input type="button" name="B15" id="B15" <?php if ($IdT=="") {echo "disabled";}?> value="CETAK" onclick="P_Dokumen('800','400','<?=$Frm?>','<?=$IdT?>','<?=$_GET['IdL']?>')" style="width: 120px; height: 21px; color:#0000FF" />
	  </td>
    <td width="5">&nbsp;</td>
  </tr>
</table>
<table border="0" cellspacing="0" class="table-link" cellpadding="0" align="center" style="width:99%; height:20px; background: #CEFBE3; font-weight:bold">
  <tr style="text-align:center">
    ...
    <td width="402" style="border-right:1px #ccc solid">Nama Rekening</td>
    <td width="60" style="border-right:1px #ccc solid">Aset</td>
    <td width="75" style="border-right:1px #ccc solid">Jumlah</td>
    <td width="95" style="border-right:1px #ccc solid">Satuan</td>
    <td width="450" style="border-right:1px #ccc solid">Uraian</td>
...
<script languange="javascript">
	...
	function showUNIT(CrT,gIdL)
	{
		if (CrT=='find')
		{
			...
			{
				$("#unitDiv2Cri").load('rkbmd_standar_kebutuhan_frm_find_unit_mid.php?gFnD='+gFnD+'&IdL='+gIdL);
			});
		}
		else
		{
			...
			$(document).ready(function()
			{
				$("#unitDiv1Cri").load('rkbmd_standar_kebutuhan_frm_find_unit_top.php?IdL='+gIdL);
			});
			
			$(document).ready(function()
			{
				$("#unitDiv2Cri").load('rkbmd_standar_kebutuhan_frm_find_unit_mid.php?IdL='+gIdL);
			});
	...
	function showSUB(CrT,gIdL)
	{
		...
		if (CrT=='find')
		{
			...
			$(document).ready(function()
			{
				$("#subDiv2Cri").load('rkbmd_standar_kebutuhan_frm_find_sub_mid.php?gFnD='+gFnD+'&gUNT='+fUNT+'&IdL='+gIdL);
			});
		}
		else
		{
			...
			$(document).ready(function()
			{
				$("#subDiv1Cri").load('rkbmd_standar_kebutuhan_frm_find_sub_top.php?IdL='+gIdL);
			});
			
			$(document).ready(function()
			{
				$("#subDiv2Cri").load('rkbmd_standar_kebutuhan_frm_find_sub_mid.php?gUNT='+fUNT+'&IdL='+gIdL);
			});
	...
	function showUPB(CrT,gIdL)
	{
		...
		if (CrT=='find')
		{
			...
			$(document).ready(function()
			{
				$("#upbDiv2Cri").load('rkbmd_standar_kebutuhan_frm_find_upb_mid.php?gFnD='+gFnD+'&gSUB='+fSUB+'&IdL='+gIdL);
			});
		}
		else
		{
			...
			$(document).ready(function()
			{
				$("#upbDiv1Cri").load('rkbmd_standar_kebutuhan_frm_find_upb_top.php?IdL='+gIdL);
			});
			
			$(document).ready(function()
			{
				$("#upbDiv2Cri").load('rkbmd_standar_kebutuhan_frm_find_upb_mid.php?gSUB='+fSUB+'&IdL='+gIdL);
			});
	...
	function proslNIL(crt,PgE,IdT,gAsT,gLhI,gExT,IdL,AmBiL)
	{
		if (AmBiL=="ALL")
		{
			...
			{
				$("#loadingImg").show();
				$("#ViewDATA").load('rkbmd_standar_kebutuhan_frm_find_aset_mid_checklist.php?rUpb='+rUpb+'&gExT='+gExT+'&gLhI='+gLhI+'&PgE='+PgE+'&IdT='+IdT+'&gAsT='+gAsT+'&gUNT='+gUNT+'&AmBiL='+AmBiL+'&gFnD='+gFnD+'&gTHN='+gTHN+'&gLST='+gLST+'&IdL='+IdL);
			});
		}
		else
		{
			...
			{
				$("#loadingImg").show();
				//$("#ViewDELL").load('rkbmd_standar_kebutuhan_frm_find_aset_mid_checklist.php?AmBiL='+AmBiL+'&PgE='+PgE+'&IdT='+IdT+'&gCrID='+gCrID+'&IdL='+IdL);
				$("#ViewDATA").load('rkbmd_standar_kebutuhan_frm_find_aset_mid_checklist.php?AmBiL='+AmBiL+'&PgE='+PgE+'&IdT='+IdT+'&gCrID='+gCrID+'&IdL='+IdL);
			});
		}
		document.getElementById(crt+'MstCri').style.display = "none";
	}
	
	function showCLICK(crt,idt,kde,nma,IdL)
	{
		if (crt=='unit') 
		{
			$("#fUNT").val(kde);
		...
		if (crt=='aset') 
		{
			//alert(idt+':'+kde); return false;
			$(document).ready(function()
			{
				$("#ViewDATA").load('rkbmd_standar_kebutuhan_frm_find_aset_add.php?IdT='+idt+'&kde='+kde+'&IdL='+IdL);
			});
		}
		document.getElementById(crt+'MstCri').style.display = "none";
	}
	...
	function showASET(CrT,PgE,IdT,IdL)
	{
		...
		if (CrT=='find')
		{
			...
			$(document).ready(function()
			{
				$("#asetDiv2Cri").load('rkbmd_standar_kebutuhan_frm_find_aset_mid.php?PgE='+PgE+'&gEXT='+gEXT+'&gLST='+gLST+'&gAsT='+gAsT+'&gFnD='+gFnD+'&IdT='+IdT+'&IdL='+IdL);
				$("#asetDiv3Cri").load('rkbmd_standar_kebutuhan_frm_find_aset_page.php?PgE='+PgE+'&gEXT='+gEXT+'&gLST='+gLST+'&gAsT='+gAsT+'&gFnD='+gFnD+'&IdT='+IdT+'&IdL='+IdL);
			});
		}
		else
		{
			...
			$(document).ready(function()
			{
				$("#asetDiv1Cri").load('rkbmd_standar_kebutuhan_frm_find_aset_top.php?PgE='+PgE+'&IdT='+IdT+'&gAsT='+gAsT+'&IdL='+IdL);
				$("#asetDiv2Cri").load('rkbmd_standar_kebutuhan_frm_find_aset_mid.php?PgE='+PgE+'&gEXT='+gEXT+'&gLST='+gLST+'&gAsT='+gAsT+'&IdT='+IdT+'&IdL='+IdL);
				$("#asetDiv3Cri").load('rkbmd_standar_kebutuhan_frm_find_aset_page.php?PgE='+PgE+'&gEXT='+gEXT+'&gLST='+gLST+'&gAsT='+gAsT+'&IdT='+IdT+'&IdL='+IdL);
			});
	...
	function RefreshDATA(IdL,PgE)
	{
		...
		{
			$.ajax({
				url:"rkbmd_standar_kebutuhan_frm_data.php",
			...
			$("#ViewDATAaX").load('rkbmd_standar_kebutuhan_frm_data_pages.php?PgE='+PgE+'&gREF='+gREF+'&IdL='+IdL);
		});
	}
	...
	function P_Remove(PgE,rIdT,DeL,IdL)
	{
		if (DeL=="DelVer") {
			...
			if (AN)
			{
				$(document).ready(function()
				{
					$("#ViewDELL").load('rkbmd_standar_kebutuhan_frm_del.php?PgE='+PgE+'&DelVer=Ya&rIdT='+rIdT+'&IdL='+IdL);
				});
			}
		}
		else{
			...
			if (AN)
			{
				$(document).ready(function()
				{
					$("#ViewDELL").load('rkbmd_standar_kebutuhan_frm_del.php?PgE='+PgE+'&rIdT='+rIdT+'&IdL='+IdL);
				});
			}
		}
	}
	...
	function choiseTANGGAL(IdL)
	{
		...
		$(document).ready(function()
		{
			$("#choiseDiv1Cri").load('rkbmd_standar_kebutuhan_frm_choise_top.php');
		});
		
		$(document).ready(function()
		{
			$("#choiseDiv2Cri").load('rkbmd_standar_kebutuhan_frm_choise_mid.php?IdL='+IdL);
		});
	...
	{
		gUNT = $("#fUNT").val();
		window.open('rkbmd_standar_kebutuhan.php?FrmG=STANDAR KEBUTUHAN BMD&Frm='+Frm+'&gUNT='+gUNT+'&IdL='+IdL,'_self');
	}
	...
	function showDATA(CrT,IdT,gIdL)
	{
		...
		if (CrT=='find')
		{
			...
			$(document).ready(function()
			{
				$("#tranDiv2Cri").load('rkbmd_standar_kebutuhan_frm_data_mid.php?gFnD='+gFnD+'&gUPB='+gUPB+'&IdL='+gIdL);
			});
		}
		else
		{
			...
			$(document).ready(function()
			{
				$("#tranDiv1Cri").load('rkbmd_standar_kebutuhan_frm_data_top.php?IdL='+gIdL);
			});
			
			$(document).ready(function()
			{
				$("#tranDiv2Cri").load('rkbmd_standar_kebutuhan_frm_data_mid.php?gUPB='+gUPB+'&IdL='+gIdL);
			});
	...
	function showREF(CrT,gIdT,gIdL)
	{
		...
		$(document).ready(function()
		{
			$("#imgDiv2Cri").load('rkbmd_standar_kebutuhan_frm_data_edit_mid.php?gIdT='+gIdT+'&gRF='+gRF+'&CrT='+CrT+'&IdL='+gIdL);
		});
	}
	...
	function showEDIT(CrT,IdT,IdL)
	{
		...
		if (CrT=='refr')
		{
			$(document).ready(function()
			{
				$("#edtDiv2Cri").load('rkbmd_standar_kebutuhan_frm_data_edit_mid.php?IdT='+IdT+'&gRF='+gRF+'&CrT='+CrT+'&IdL='+IdL);
			});
		}
		else
		{
			...
			$(document).ready(function()
			{
				$("#edtDiv1Cri").load('rkbmd_standar_kebutuhan_frm_data_edit_top.php?CrT='+CrT+'&IdL='+IdL);
			});
	...
	function SaveEDIT(gRF,JeN,IdT,IdL)
	{
		...
		$(document).ready(function()
		{
			filepost = "rkbmd_standar_kebutuhan_frm_data_edit_save_json.php";
			$.post(filepost,
	...
	function choiseFIND_xxxx(Frm,CrT,gid,kde,nma,IdL)
	{
		$(document).ready(function()
		{
			$("#ViewDELL").load('rkbmd_standar_kebutuhan_frm_data_edit_mid_find_add.php?gFrm='+Frm+'&gID='+gid+'&gKD='+kde+'&IdL='+IdL);
		});
		
		dispNO(CrT+'MstCri');
	}
	
	function P_Dokumen(w,h,Frm,IdT,IdL)
	{
		dk="rkbmd_standar_kebutuhan_frm_data_dok";
		var win=null;
		var txtHTML = "";
		var iErrors=0;
	...
	function SaveRecord(field,CrT,IdT,IdL) 
	{ 
		gVL = field.value; 
		$(document).ready(function()
		{
			filepost = "rkbmd_standar_kebutuhan_frm_save_json.php";
			$.post(filepost,
			{"gVL":gVL,"CrT":CrT,"IdT":IdT,"IdL":IdL},
			function( data ) 
			{
				alert(data['mess']);
				RefreshDATA(IdL,'0');
	...
	function LoadASET(CrT,PgE,SkD,KdA,IdL)
	{
		gLST = 400;
		gEXT = "";
		if (CrT=='find')
		{
			...
			FnD = ReplaceText(objfrm.fFindPL.value);
			$(document).ready(function()
			{
				$("#asetDiv2Cri").load('rkbmd_standar_kebutuhan_frm_load_aset_mid.php?PgE='+PgE+'&gEXT='+gEXT+'&gLST='+gLST+'&FnD='+FnD+'&SkD='+SkD+'&KdA='+KdA+'&IdL='+IdL);
				$("#asetDiv3Cri").load('rkbmd_standar_kebutuhan_frm_load_aset_page.php?PgE='+PgE+'&gEXT='+gEXT+'&gLST='+gLST+'FnD='+FnD+'&SkD='+SkD+'&KdA='+KdA+'&IdL='+IdL);
			});
		}
		else
		{
			...
			$(document).ready(function()
			{
				$("#asetDiv1Cri").load('rkbmd_standar_kebutuhan_frm_load_aset_top.php?PgE='+PgE+'&SkD='+SkD+'&KdA='+KdA+'&IdL='+IdL);
				$("#asetDiv2Cri").load('rkbmd_standar_kebutuhan_frm_load_aset_mid.php?PgE='+PgE+'&gEXT='+gEXT+'&gLST='+gLST+'&SkD='+SkD+'&KdA='+KdA+'&IdL='+IdL);
				$("#asetDiv3Cri").load('rkbmd_standar_kebutuhan_frm_load_aset_page.php?PgE='+PgE+'&gEXT='+gEXT+'&gLST='+gLST+'&SkD='+SkD+'&KdA='+KdA+'&IdL='+IdL);
			});
			dispBlockOrNo('asetMstCri');
		}
	}

	function showDetail(IdT,IdL)
	{
		alert(IdT);
	}
</script>
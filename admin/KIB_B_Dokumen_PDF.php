...
class PDF extends FPDF
{
	...
	{	
		...
		$this->Cell(1);
		$this->Cell(0,12,'KARTU INVENTARIS BARANG (KIB-B)',0,0,'C');	
		$this->Ln(5);
		
		$this->Cell(1);
		$this->Cell(0,12,'PERALATAN DAN MESIN',0,0,'C');	
		$this->Ln(8);
		...
		$R2=array(10,22,55,15,17,20,20,12,28,28,28,20,25,22,25,50);
		$this->SetFillColor(255,255,255);
		$this->Cell(1);
		$this->Cell($R2[0],10,'No.','TLRB',0,'C',true);
		$this->Cell($R2[1],10,'Kode','TLRB',0,'C',true);
		$this->Cell($R2[2],10,'Jenis / Nama Barang','TLRB',0,'C',true);
		$this->Cell($R2[3],10,'Register','TLRB',0,'C',true);
		$this->Cell($R2[4],10,'Merk/Type','TLRB',0,'C',true);
		$this->Cell($R2[5],10,'Ukuran/CC','TLRB',0,'C',true);
		$this->Cell($R2[6],10,'Bahan','TLRB',0,'C',true);
		$this->Cell($R2[7],10,'Tahun','TLRB',0,'C',true);
		$this->Cell($R2[8],10,'No.Pabrik','TLRB',0,'C',true);
		$this->Cell($R2[9],10,'No.Rangka','TLRB',0,'C',true);
		$this->Cell($R2[10],10,'No.Mesin','TLRB',0,'C',true);
		$this->Cell($R2[11],10,'No.Polisi','TLRB',0,'C',true);
		$this->Cell($R2[12],10,'No.BPKB','TLRB',0,'C',true);
		$this->Cell($R2[13],10,'Asal-Usul','TLRB',0,'C',true);
		$this->Cell($R2[14],10,'Harga (Rp)','TLRB',0,'C',true);
		$this->Cell($R2[15],10,'Keterangan','TLRB',0,'C',true);
		$this->Ln() ;
	...
	{
		$iG=1; $tJml=0;
		$R3=array(10,22,55,15,17,20,20,12,28,28,28,20,25,22,25,50);
		$this->SetFillColor(255,255,255);
		$this->SetFont('arial','',7);
				
		$SQL = "SELECT * FROM ta_kib_b WHERE Kd_UPB='".$gUpb."' AND Tgl_Perolehan LIKE '".$gThn."-__-__' AND Kd_Pemilik LIKE '".$gMLK."' AND Status='' ORDER BY Tgl_Perolehan, Kd_Aset, No_Register";
		...
		if ($tRo > 0)
		{
			do
			{
				...
				$this->Cell($R3[1],5,$mRo['Kd_Aset'],'TLRB',0,'C',true);
				$this->Cell($R3[2],5,$NmAss,'TLRB',0,'L',true);
				$this->Cell($R3[3],5,$mRo['No_Register'],'TLRB',0,'C',true);
				
				$nMerk = $mRo['Merk'];
				if ($nMerk!="") {$nMerk="Merk : ".$nMerk;} else {$nMerk="";}
				$Col4= $nMerk;
				$nType = $mRo['Type'];
				if ($nType!="") {$nType="Type : ".$nType;} else {$nType="";}
				if ($Col4!="") {$Col4=$Col4." / ".$nType;}
				
				$this->Cell($R3[4],5,'','TLRB',0,'R',true);
				$this->Cell($R3[5],5,$mRo['Ukuran_CC'],'TLRB',0,'C',true);
				$this->Cell($R3[6],5,$mRo['Bahan'],'TLRB',0,'L',true);
				$this->Cell($R3[7],5,substr($mRo['Tgl_Perolehan'],0,-6),'TLRB',0,'C',true);
				$this->Cell($R3[8],5,$mRo['Nomor_Pabrik'],'TLRB',0,'C',true);
				$this->Cell($R3[9],5,$mRo['Nomor_Rangka'],'TLRB',0,'L',true);
				$this->Cell($R3[10],5,$mRo['Nomor_Mesin'],'TLRB',0,'L',true);
				$this->Cell($R3[11],5,$mRo['Nomor_Polisi'],'TLRB',0,'C',true);
				$this->Cell($R3[12],5,$mRo['Nomor_BPKB'],'TLRB',0,'R',true);
				...
				$this->Cell($R3[15],5,substr($mRo['Keterangan'],0,30),'TLRB',0,'L',true);
				$this->Ln() ;
			}

...
<form name="myfrm" method="POST" action="<?="Utility_Import_SQL_BFN.php?FrmG=".$FrmG."&IdL=".$IdL?>">
...
<table border="0" class="table-link" cellspacing="0" cellpadding="0" align="center" style="width:99%; background-color:#D2DAC4">
  ...
  <tr height="25">
	...
	<input type="button" name="B392" value="UPDATE" onclick="RefreshDATA('update','<?=$IdL?>')" style="width:80px; height:20px" />
      <br>
...
<script languange="javascript">
	...
	function RefreshDATA(crt,IdL)
	{
		...
		if (crt=='update')
		{
			fil = "Utility_Import_SQL_BFN_Update";
			vie = "ViewDATA";
			AN=confirm('Update ke KDL?');
			if (!AN){return false;}
		}
		else if (crt=='batal')
		{
			//fil = "Utility_Import_SQL_BFN_Batal";
			//vie = "ViewDELL";
			//AN=confirm('Batalkan?');
			//if (!AN){return false;}
			return false;
		...
		{
			fil = "Utility_Import_SQL_BFN_Data";
			vie = "ViewDATA";
		}

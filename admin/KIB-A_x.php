...
<link rel="stylesheet" href="css/pulldown.css" type="text/css" media="all" />
  ...
  <table border="0" align="center" style="width:99%">
    ...
    <tr> 
      ...
      <td valign="middle"> <table border="0" width="100%" cellpadding="0" style="border-collapse: collapse">
          <tr> 
            ...
            <td width="8%">CARI</td>
            <td width="35%"> <input class="text" type="text" name="fFind" value="<?php echo $gFin?>" style="font-family: Calibri; font-size: 10pt; border: 1px solid #C0C0C0; padding-left: 4px; padding-right: 4px; padding-top: 1px; padding-bottom: 1px; background-color: #E1F986" /></td>
            <td width="2%">&nbsp;</td>
            <td width="10%"> <input type="button" name="B39" value="GO" onclick="P_Find()" style="width: 50px; height: 21px; padding-left: 0px; padding-right: 0px; padding-top: 0px; padding-bottom: 0px" /></td>
            <td width="21%">&nbsp;</td>
          </tr>
        </table></td>
      <td>SUB SUB ROBJ </td>
    ...
    <table align="center" border="0" class="table-list" cellspacing="0" cellpadding="0" style="width:99%">
      <tr> 
        ...
        <th width="90" align="left">Kode</th>
        <th width="60" align="left">Register</th>
        <th width="350" align="left">Nama Barang</th>
        <th align="left">Uraian</th>
        <th width="60" align="left">Tgl.Perolehan</th>
        <th width="100" class="ar">Nilai Perolehan </th>
        <th width="100" class="ar">Nilai Akhir </th>
        <th width="107" class="ac">Actions</th>
      </tr>
      <?php
	  if ($gUpb=="All") {$zUpb = $zSub.".%";}
		...
		if ($gFin!="")
		{$fFindSy = "AND (Harga LIKE '%".$gFin."%' OR Referensi LIKE '%".$gFin."%' OR Kd_Aset_108 LIKE '%".$gFin."%' OR Keterangan LIKE '%".$gFin."%')";}
		else
		{$fFindSy = "";}
		
		$nSQL= "SELECT * FROM ta_kib_a WHERE Kd_UPB LIKE '".$zUpb."' AND Kd_Aset_108 LIKE '".$rBid.".".$rKel.".".$rOBJ.".".$rRin."' AND Tgl_Perolehan LIKE '".$rThn."-__-__' ".$fFindSy." ORDER BY Kd_Aset_108, No_Register LIMIT $Offset, $DataPerPage";
		...
		do
			...
			if ($gNIL==0) {
				$SQG="UPDATE ta_kib_a set Post='N' WHERE IDT='".$mRo['IDT']."'";
				$Rs = mysql_query($SQG);
			}
			...
			//
			?>
		  <tr height="23" style="cursor: pointer" title="clik disini untuk melihat rincian...!" onmouseover="this.style.cursor=&#39;pointer&#39" <?=fBackCLR($iG)?>> 
			...
			<td style="border-bottom: #999999 dotted 1px; border-right:#999999 dotted 1px; text-align:center" onclick="$(&#39;#detail<?php echo $iG?>&#39;).toggle(&#39;past&#39;)"><?php echo $mRo['Kd_Aset_108']?></td>
			<td style="border-bottom: #999999 dotted 1px; border-right:#999999 dotted 1px; text-align:center"  onclick="$(&#39;#detail<?php echo $iG?>&#39;).toggle(&#39;past&#39;)"><?php echo $mRo['No_Register']?></td>
			<td style="border-bottom: #999999 dotted 1px; border-right:#999999 dotted 1px; padding-left:2px"  onclick="$(&#39;#detail<?php echo $iG?>&#39;).toggle(&#39;past&#39;)"><?php echo $NmB ?></td>
			<td style="border-bottom: #999999 dotted 1px; border-right:#999999 dotted 1px; padding-left:2px"  onclick="$(&#39;#detail<?php echo $iG?>&#39;).toggle(&#39;past&#39;)"> 
			  <?php
				if  ((int) strlen($mRo['Keterangan']) > 50)
				{echo substr($mRo['Keterangan'],0,50)." .....";}
				else
				{echo $mRo['Keterangan'];}
				?>        </td>
        <td style="text-align:center; border-bottom: #999999 dotted 1px; border-right:#999999 dotted 1px; padding-left:2px"  onclick="$(&#39;#detail<?php echo $iG?>&#39;).toggle(&#39;past&#39;)"> <?=fConvertDateShort($mRo['Tgl_Perolehan'])?></td>
        <td style="border-bottom:#999999 dotted 1px; border-right:#999999 dotted 1px; padding-right:2px"  align="right" onclick="$(&#39;#detail<?php echo $iG?>&#39;).toggle(&#39;past&#39;)"><?php echo fConvertToRupiah($mRo['Harga'])?></td>
        <td style="border-bottom:#999999 dotted 1px; border-right:#999999 dotted 1px; padding-right:2px"  align="right" onclick="$(&#39;#detail<?php echo $iG?>&#39;).toggle(&#39;past&#39;)" <?php echo $ClR?>> <?=fConvertToRupiah($gNIL)?>        </td>
        <td style="border-bottom: #999999 dotted 1px"  align="center">
		<a href="#" class="ico <?=$del?>" onclick="P_DeleteR('<?=$CeK?>','<?=$ThN?>','<?=$KiB?>','<?=$mRo['IDT']?>','<?=$RegGrp?>','<?=$ReO?>'); return false">&nbsp;Delete</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="#" class="ico edit" onclick="EditData('950','520','center','<?php echo $mRo['IDT']?>'); return false">Edit</a></td>
      </tr>
	  <!--
      <tr> 
        <td colspan="9">
      ...
      <?php
		...
		?>
          ...
          <!-- Pagging -->
          <?php
			$nSQL= "SELECT COUNT(*) AS JmlRc FROM ta_kib_a WHERE Kd_UPB LIKE '".$zUpb."' AND Kd_Aset LIKE '".$rBid.".".$rKel.".".$rOBJ.".".$rRin."' AND Tgl_Perolehan LIKE '".$rThn."-__-__' ".$fFindSy;

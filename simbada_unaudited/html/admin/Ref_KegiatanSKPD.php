...
<table border="0" class="table-link" cellspacing="0" cellpadding="0" align="center" style="width:1200px">
  ...
  <tr height="25">
    ...
    <td align="center">&nbsp;</td>
    <td colspan="3">
	<input name="fUNT" type="text" value="<?=$gUNT?>" <? if ($Lev<=2) {?> onClick="showUNIT('','<?=$_GET['IdL']?>')" onkeypress="if (event.keyCode==13) {showUNIT('find','<?=$_GET['IdL']?>'); return false;} else if (event.keyCode==27) {closeCLICK('unit'); return false;}" <? } else {echo "readonly";}?> style="padding-left:5px; width:110px; border: 1px solid #C0C0C0"/>
	<input name="dUNT" type="text" value="<?=$dUNT?>" <? if ($Lev<=2) {?> onClick="showUNIT('','<?=$_GET['IdL']?>')" onkeypress="if (event.keyCode==13) {showUNIT('find','<?=$_GET['IdL']?>'); return false;} else if (event.keyCode==27) {closeCLICK('unit'); return false;}" <? } else {echo "readonly";}?> style="padding-left:5px; width:520px; border: 1px solid #C0C0C0"/>
...
<script languange="javascript">
	...
	function showUNIT(CrT,gIdL)
	{
		if (CrT=='find')
		{
			...
			{
				$("#unitDiv2Cri").load('Ref_KegiatanSKPD_Find_Unit_Mid.php?gFnD='+gFnD+'&IdL='+gIdL);
			});
		}
		else
		{
			...
			$(document).ready(function()
			{
				$("#unitDiv1Cri").load('Ref_KegiatanSKPD_Find_Unit_Top.php?IdL='+gIdL);
			});
			
			$(document).ready(function()
			{
				$("#unitDiv2Cri").load('Ref_KegiatanSKPD_Find_Unit_Mid.php?IdL='+gIdL);
			});
	...
	function showPROG(CrT,gIdL)
	{
		...
		if (CrT=='find')
		{
			...
			$(document).ready(function()
			{
				$("#progDiv2Cri").load('Ref_KegiatanSKPD_Find_Prog_Mid.php?gFnD='+gFnD+'&gUNT='+fUNT+'&gTHN='+fTHN+'&IdL='+gIdL);
			});
		}
		else
		{
			...
			$(document).ready(function()
			{
				$("#progDiv1Cri").load('Ref_KegiatanSKPD_Find_Prog_Top.php?IdL='+gIdL);
			});
			
			$(document).ready(function()
			{
				$("#progDiv2Cri").load('Ref_KegiatanSKPD_Find_Prog_Mid.php?gUNT='+fUNT+'&gTHN='+fTHN+'&IdL='+gIdL);
			});
	...
	function showCLICK(crt,kde,nma,IdL)
	{
		...
		if (crt=='kegi') 
		{
			$(document).ready(function()
			{
				$("#ViewDELL").load('Ref_KegiatanSKPD_Find_Kegi_Add.php?IdT='+kde+'&gUNT='+fUNT+'&gTHN='+fTHN+'&IdL='+IdL);
			});
		}
	...
	function showKEGI(CrT,IdL)
	{
		...
		if (CrT=='find')
		{
			...
			$(document).ready(function()
			{
				$("#kegiDiv2Cri").load('Ref_KegiatanSKPD_Find_Kegi_Mid.php?gFnD='+gFnD+'&gPRG='+gPRG+'&gTHN='+gTHN+'&IdL='+IdL);
			});
		}
		else
		{
			...
			$(document).ready(function()
			{
				$("#kegiDiv1Cri").load('Ref_KegiatanSKPD_Find_Kegi_Top.php?IdL='+IdL);
			});
			
			$(document).ready(function()
			{
				$("#kegiDiv2Cri").load('Ref_KegiatanSKPD_Find_Kegi_Mid.php?gPRG='+gPRG+'&gTHN='+gTHN+'&IdL='+IdL);
			});
	...
	function RefreshDATA(IdL)
	{
		...
		$(document).ready(function()
		{
			$("#ViewDATA").load('Ref_KegiatanSKPD_Data.php?gUNT='+fUNT+'&gPRG='+fPRG+'&gTHN='+fTHN+'&gAPB='+fAPB+'&IdL='+IdL);
		});
	}
	...
	function P_Remove(IdT,DeL,IdL)
	{
		...
		if (AN)
		{
			$(document).ready(function()
			{
				$("#ViewDELL").load('Ref_KegiatanSKPD_Data_Del.php?IdT='+IdT+'&IdL='+IdL);
			});
		}
	}
	...
	function addITEM(IdL)
	{
		...
		$(document).ready(function()
		{
			$("#ViewDELL").load('Ref_KegiatanSKPD_Data_Add_Item.php?gUNT='+fUNT+'&gPRG='+fPRG+'&gTHN='+fTHN+'&IdL='+IdL);
		});
	}
	...
	function P_Save(field,IdT,CrT,DeL,IdL)
	{
		...
		$(document).ready(function()
		{
			$("#ViewDATA").load('Ref_KegiatanSKPD_Data_Save.php?NiL='+NiL+'&gTHN='+fTHN+'&CrT='+CrT+'&IdT='+IdT+'&IdL='+IdL);
		});
	}

...
function ExecuteRC($NoRG,$frUPB,$RfA,$RfU,$TbK,$tID,$UID)
{
	...
	$NewRefKIB = mNewRefKIB_108('ASL');
	$NewRefGrp = "";
	
	$rRKO = fGlobal("Kd_Aset_108","ta_kib_108","referensi:kd_upb:No_Register",$RfA.":".$frUPB."%:".$NoRG,"=:LIKE:=","","");
	$eRuH = fGlobal("ref_usulan","ta_kib_108","referensi:kd_upb:No_Register",$RfA.":".$frUPB."%:".$NoRG,"=:LIKE:=","","");
	
	#$gRin = "";
	#$gNmB = "";
	#$rDT = fGlobal("Kd_Aset:Nm_Aset","ref_rek_aset108_7","Kd_Aset:Link_Kib_AE","1.5.4.05.%:".$rRKO,"LIKE:=","","");
	#if ($rDT){
	#	$rDT = explode(":",$rDT);
	#	$gRin = $rDT[0];
	#	$gNmB = $rDT[1];
	...
	$gRin = fGlobal("To_Kd_Aset","ta_usulan_verifikasi_rinci_108","Ref_Aset:Ref_Usulan:No_Register",$RfA.":".$RfU.":".$NoRG,"=:=:=","","");
	if (strlen($gRin)==11)
	{
		$gRin.=".01.001"; 
	}
	...
	if ($CekR=='') 
	{
		InsertKib_Mutasi_108($NoRG,$frUPB,$RfA,$RfU,$NewRefKIB,$TglMts,$gRin,$UID);
		InsertPos_Mutasi_108($NoRG,$frUPB,$RfA,$RfU,$NewRefKIB,$TglMts,$gRin,$UID);
		
		UpdateKib_108($NoRG,$frUPB,$RfA,$RfU,$NewRefKIB,$NewReGAset,$TglMts,$gRin,$eRuH,$UID);
		UpdatePos_108($NoRG,$frUPB,$RfA,$RfU,$NewRefKIB,$NewReGAset,$TglMts,$gRin,$eRuH,$UID);
...
{
	$DtE = fGlobal("Ref_Usulan:Ref_Usulan_His:Ref_History:Referensi:Referensi_To:Kd_Aset_108:No_Register:Tgl_Mutasi_Masuk","ta_kib_108_mutasi","Referensi:Ref_Usulan:Kd_UPB_To:No_Register",$RfA.":".$RfU.":".$frUPB."%:".$NoRG,"=:=:LIKE:=","IDT limit 0,1","");
	if ($DtE!="")
	{
		$DeB = explode(":",$DtE);
	...
	UpdateKibPos_108($NoRG,$frUPB,$RfU,$RuH,$ReF,$RfH,$RfT,$KdA,$ReG,$TgM,$UID);
	DeleteKibPos_108_Mutasi($NoRG,$frUPB,$RfU,$ReF,$RfU);
	UpdateUsulanVerRci($tID,'','Belum',$UID);	
}

function DeleteKibPos_108_Mutasi($NoRG,$frUPB,$RfU,$ReF,$RfU)
{
...
function UpdateKibPos_108($NoRG,$frUPB,$RfU,$RuH,$ReF,$RfH,$RfT,$KdA,$ReG,$TgM,$UID)
{
	#No_Register='".$ReG."',
	...
	Recorded=now(),
	Pencatat='".$UID.":UnExe' 
	WHERE Referensi='".$RfT."' AND Ref_Usulan='".$RfU."' AND Kd_UPB LIKE '".$frUPB."%' AND No_Register='".$NoRG."'";
	$RsU = mysql_query($SQU);
...
function InsertPos_Mutasi_108($NoRG,$frUPB,$RfA,$RfU,$NewRefKIB,$TglMts,$gRin,$UID)
{
...
function mNewRefKIB_108_XXX($rF)
{
...
function mNewRefKIB_108($rF)
{
	...
	$NewRefKIB = $rF.".".fMakeReferensi($NewK,11);
...
function InsertKib_Mutasi_108($NoRG,$frUPB,$RfA,$RfU,$NewRefKIB,$TglMts,$gRin,$UID)
{
...
function UpdateKib_108($NoRG,$frUPB,$RfA,$RfU,$NewRefKIB,$NewReGAset,$TglMts,$gRin,$eRuH,$UID)
{
...
function UpdatePos_108($NoRG,$frUPB,$RfA,$RfU,$NewRefKIB,$NewReGAset,$TglMts,$gRin,$eRuH,$UID)
{
	#No_Register='".$NewReGAset."',
	$SQU = "UPDATE ta_kib_post_108 SET 
	Referensi='".$NewRefKIB."', 

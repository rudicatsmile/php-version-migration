...
<form name="myfrm" method="POST" action="<?="Ref_Rekanan.php?IdL=".$_GET['IdL']?>">
...
<table border="0" cellspacing="0" class="table-link" cellpadding="0" align="center" style="width:1200px; height:20px; background: #CEFBE3; font-weight:bold">
  <tr>
    ...
    <td width="207" align="left">PIMPINAN</td>
    <td width="208" align="left">NPWP</td>
...
<script languange="javascript">
	...
	function RefreshDATA(IdL)
	{
		...
		{
			$("#ViewDATA").load('Ref_Rekanan_Data.php?FnD='+FnD+'&IdL='+IdL);
		});
	}
	...
	function formDELE(IdT,IdL)
	{
		...
		if (AN)
		{
			$(document).ready(function()
			{
				$("#ViewDELL").load('Ref_Rekanan_Del.php?IdT='+IdT+'&IdL='+IdL);
			});
		}
	}
	...
	function formEDIT(CrT,IdT,IdL)
	{
		if (CrT=='refr')
		{
			$(document).ready(function()
			{
				$("#alasDiv2Cri").load('Ref_Rekanan_Mid.php?IdT='+IdT+'&IdL='+IdL);
			});
		}
		else
		{
			...
			$(document).ready(function()
			{
				$("#alasDiv1Cri").load('Ref_Rekanan_Top.php?IdL='+IdL);
			});
	...
	function resetEDIT(ReO,IdL)
	{
		$(document).ready(function()
		{
			$("#alasDiv2Cri").load('Ref_Rekanan_Mid.php?IdL='+IdL);
		});
	}
	...
	function saveEDIT(ReO,IdT,IdL)
	{
		...
		$(document).ready(function()
		{
			$.post('Ref_Rekanan_Save.php',
			{"Nmpr":Nmpr,"Nmpi":Nmpi,"Npwp":Npwp,"Npwd":Npwd,"Alam":Alam,"Telp":Telp,"Faxs":Faxs,"Mail":Mail,"IdT":IdT,"IdL":IdL},
			function( data )
			{
				IdT= data['IdT'];

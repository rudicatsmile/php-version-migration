...
$KdU = substr(fGlobal("KdUPB","tb_lembar_kerja","IDT",$SnsIDT,"=","",""),0,11);
...
<table align="center" cellpadding="0" class="table-listpop" cellspacing="0" width="100%" height="240" border="0">
	<tr height="20" onclick="showPetugasChoise('petu','','<?=$fld?>','<?=$AsT?>','<?=$ReO?>','<?=$SnsIDT?>','<?=$IdT?>','<?=$IdL?>'); return false;">
	...
	while ($mRo = mysql_fetch_array($nRs, MYSQL_BOTH))
	{
		...
		$gNm = $mRo[1];
		?>
		<tr height="20" onclick="showPetugasChoise('petu','<?=$gKd?>','<?=$fld?>','<?=$AsT?>','<?=$ReO?>','<?=$SnsIDT?>','<?=$IdT?>','<?=$IdL?>'); return false;">
			<td width="10" style="border-bottom:1px dotted #CCCCCC">&nbsp;</td>

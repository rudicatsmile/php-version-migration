...
<? if ($CrT=='Edit') {?>
	...
	<tr height="25">
	  ...
	  <td class="ar">&nbsp;</td>
	  <td>&nbsp;</td>
	  <td><a href="#" class="ico edit" onclick="updateALL('<?=$gIdT?>','<?=$_GET['IdL']?>')">Update ALL</a></td>
	  <td>&nbsp;</td>
	</tr>
	<? } ?>
	<? if ($gJN=="MK" || $gJN=="TW" || $gJN=="KP") {?>
	<?
	...
	$dKIb = "";
	if ($gKIb) {
		#$dKIb = "KIB-".strtoupper(fNmHuruf((int)substr($gKIb,-1,1)))." : ASET ".strtoupper(fGlobal("Nm_Aset","ref_rek_aset108_3","Kd_Aset",substr($gKIb,0,5),"=","","s"));
		$dKIb = "ASET ".strtoupper(fGlobal("Nm_Aset","ref_rek_aset108_3","Kd_Aset",substr($gKIb,0,5),"=","",""));
	}
	?>
	<tr height="25">
	  <td>&nbsp;</td>
	...
	<tr height="25">
	  ...
	  <input name="dKIBb" id="dKIBb" type="text" value="<?=$dKIb?>" readonly="readonly" onClick="findEDIT('','kib','','<?=$gIdT?>','<?=$_GET['IdL']?>'); return false;" style="padding-left:5px; width:370px; border: 1px solid #C0C0C0"/>
...
<? } else { ?>
	...
	<table align="center" cellpadding="0" class="table-listpop" cellspacing="0" width="100%" height="245" border="0">
		...
		<? } ?>
		<tr height="20">
			<td colspan="5" style="text-align:center"><a href="#" onclick="addIMG('<?=$CrT?>','<?=$gIdT?>'); return false;" class="ico add">Upload file ( <? if ($CrT=='Pdf') {echo "PDF";} else {echo "JPG, JPEG, PNG, BMP, GIF";}?> )</a></td>
		</tr>
		<tr height="100%">

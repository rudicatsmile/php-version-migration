...
while($mRo = sqlsrv_fetch_array($rst))
{
	...
	$SQa = "SELECT sum(harga) FROM Ta_KIBDR WHERE IDPemda='".$mRo[0]."' AND Kd_Riwayat='2'";
	$rsa = sqlsrv_query($conn,$SQa);
	$mRa = sqlsrv_fetch_array($rsa);
	
	$SQb = "SELECT sum(harga) FROM Ta_Fn_KIB_D WHERE IDPemda='".$mRo[0]."' AND Tahun='2030'";
	...
	if ($IdT!='')
	{
		...
		if ($LevUpb=='Ya')
		{
			$CnT = fGlobal("count(*)","ta_kib_108","IdTabelMaster:Kd_UPB",$mRo[0].":".$UpB,"=:=","","");
		}
		else
		{
	...
	$evC = "";
	if (round($DeB)>round($mRo29))
	{
		$evC = "; color:#0000ff";
	}
	else if (round($DeB)<round($mRo29))
	{
		$evC = "; color:#0000ff";
	}
	...
	<tr height="20">
	  ...
	  <td style="border-right:1px solid #ccc; border-bottom:1px solid #ccc; text-align:right; padding-right:2px"><?=fConvertToRupiah($mRo27)?></td>
	  <td style="border-right:1px solid #ccc; border-bottom:1px solid #ccc; text-align:right; padding-right:2px"><?=fConvertToRupiah($mRo28)?></td>
	  <td style="border-right:1px solid #ccc; border-bottom:1px solid #ccc; text-align:right; padding-right:2px; font-weight:bold"><?=fConvertToRupiah($mRo29)?></td>
	  <td style="border-right:1px solid #ccc; border-bottom:1px solid #ccc; text-align:right; padding-right:2px <?=$cvB?>"><?=fConvertToRupiah($mRo30)?></td>
	  <td style="border-right:1px solid #ccc; border-bottom:1px solid #ccc; text-align:right; padding-right:2px"><?=fConvertToRupiah($HrB)?></td>
	  <td style="border-right:0px solid #ccc; border-bottom:1px solid #ccc; text-align:right; padding-right:10px; font-weight:bold  <?=$evC?>"><?=fConvertToRupiah($DeB)?></td>
	</tr>
	<?php
	}
	$dvB = "";

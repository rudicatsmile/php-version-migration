...
P1.Nomor as A3,
P1.Kd_UPB as A4,
P1.Dokumen_Nom as A5,
P1.Dokumen_Tgl as A6,
P2.Kode as A7,
P2.Deskripsi as A8,
P1.OpenRec as A9,  
P1.Uraian as A10 
FROM ta_usulan_108 P1 LEFT JOIN ref_usulan_jenis P2 ON P2.Kode=P1.Jenis 
WHERE P1.Kd_UPB LIKE '$gUnT%' AND (P1.Tanggal BETWEEN '$gTA' AND '$gTB') AND P1.Jenis LIKE '$gJNS' AND P1.Jenis NOT LIKE 'PH' $CrT ORDER BY P1.Tanggal DESC,P1.IDT DESC";
#echo $nSQ;
$nRs = mysql_query($nSQ);
while ($mRo = mysql_fetch_array($nRs, MYSQL_BOTH))
{
	...
	<tr height="26"> 
	  ...
	  <td width="170" <?=$gBG?> style="border-bottom:1px #999999 dotted; border-left:1px #ccc solid; text-align:center; font-weight:<?=$WgT?>"><?=$mRo5?><?php if (substr($mRo6,0,4)!='0000') {echo "<br>".fConvertDateShort($mRo6);}?></td>
	  <td width="230" style="border-bottom:1px #999999 dotted; text-align:left; border-left:1px #ccc solid; padding-left:3px; font-weight:<?=$WgT?>" <?=$gBG?>><?=fGlobal("Nm_Unit","ref_unit","Kd_Unit",substr($mRo4,0,11),"=","","")?></td>
	  <td width="151" <?=$gBG?> style="border-bottom:1px #999999 dotted; padding-left:3px; border-left:1px #ccc solid; font-weight:<?=$WgT?>"><?=$mRo7." -> ".$mRo8?></td>
	  <td width="270" <?=$gBG?> style="border-bottom:1px #999999 dotted; padding-left:3px; border-left:1px #ccc solid; font-weight:<?=$WgT?>"><?=$mRo10?></td>

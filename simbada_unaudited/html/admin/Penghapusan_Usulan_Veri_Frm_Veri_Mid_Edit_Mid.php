...
while ($mRo = mysql_fetch_array($nRs, MYSQL_BOTH))
{
	...
	<tr height="25">
		...
		<td <?=$gBG?> width="60" style="border-bottom: 1px dotted #999999; border-right: 0px solid #ccc"><label style="color:<? if ($Ada1=="checked") {echo "#0000ff";} else {echo "#000";}?>"><input <?=$rDisB?> name="fAda<?=$mID?>" id="fAda<?=$mID?>" type="radio" value="Y" <?=$Ada1?> onclick="saveRECO('Fisik','0','<?=$mID?>','<?=$tID?>','<?=$IdL?>'); return false;" />Ada</label></td>
		<td <?=$gBG?> width="70" style="border-bottom: 1px dotted #999999; border-right: 1px solid #ccc"><label style="color:<? if ($Ada2=="checked") {echo "#ff0000";} else {echo "#000";}?>"><input <?=$rDisB?> name="fAda<?=$mID?>" id="fAda<?=$mID?>" type="radio" value="N" <?=$Ada2?> onclick="saveRECO('Fisik','1','<?=$mID?>','<?=$tID?>','<?=$IdL?>'); return false;" />Tidak</label></td>
		<td <?=$gBG?> width="90" style="border-bottom: 1px dotted #999999; border-right: 0px solid #ccc"><label style="color:<? if ($Pnh1=="checked") {echo "#0000ff";} else {echo "#000";}?>"><input <?=$rDisB?> name="fMnh<?=$mID?>" id="fMnh<?=$mID?>" type="radio" value="Y" <?=$Pnh1?> onclick="saveRECO('Status','0','<?=$mID?>','<?=$tID?>','<?=$IdL?>'); return false;" />Memenuhi</label></td>
		<td <?=$gBG?> width="60" style="border-bottom: 1px dotted #999999; border-right: 1px solid #ccc"><label style="color:<? if ($Pnh2=="checked") {echo "#ff0000";} else {echo "#000";}?>"><input <?=$rDisB?> name="fMnh<?=$mID?>" id="fMnh<?=$mID?>" type="radio" value="N" <?=$Pnh2?> onclick="saveRECO('Status','1','<?=$mID?>','<?=$tID?>','<?=$IdL?>'); return false;" />Tidak</label></td>
...
?>
  ...
  <tr height="68">
	...
	<tr height="25">
	  ...
	  <td width="99"><label style="color:<? if ($Ver1=="checked") {echo "#ff0000";} else {echo "#000";}?>"><input <?=$rDisB?> name="fVer" id="fVer" type="radio" value="N" <?=$Ver1?>  onclick="saveVERI('Verifikasi','<?=$JnS?>','Belum','<?=$tID?>','<?=$IdL?>'); return false;" />BELUM</label></td>
	  <td width="188"><label style="color:<? if ($Ver2=="checked") {echo "#ff0000";} else {echo "#000";}?>"><input <?=$rDisB?> name="fVer" id="fVer" type="radio" value="N" <?=$Ver2?> onclick="saveVERI('Verifikasi','<?=$JnS?>','Verifikasi','<?=$tID?>','<?=$IdL?>'); return false;" />DALAM PROSES</label></td>
	  <td width="131"><label style="color:<? if ($Ver3=="checked") {echo "#ff0000";} else {echo "#000";}?>"><input <?=$rDisB?> name="fVer" id="fVer" type="radio" value="N" <?=$Ver3?>  onclick="saveVERI('Verifikasi','<?=$JnS?>','CekFisik','<?=$tID?>','<?=$IdL?>'); return false;" />CEK FISIK</label></td>
	  <td width="99"><label style="color:<? if ($Ver4=="checked") {echo "#ff0000";} else {echo "#000";}?>"><input <?=$rDisB?> name="fVer" id="fVer" type="radio" value="N" <?=$Ver4?>  onclick="saveVERI('Verifikasi','<?=$JnS?>','Ditolak','<?=$tID?>','<?=$IdL?>'); return false;" />DITOLAK</label></td>
	  <td width="196"><label style="color:<? if ($Ver5=="checked") {echo "#ff0000";} else {echo "#000";}?>"><input <?=$rDisB?> name="fVer" id="fVer" type="radio" value="N" <?=$Ver5?>  onclick="saveVERI('Verifikasi','<?=$JnS?>','Disetujui','<?=$tID?>','<?=$IdL?>'); return false;" />DISETUJUI</label></td>
	</tr>
	<? if ($JnS=='MS'){?>
	<tr height="25">
	  <td>&nbsp;</td>

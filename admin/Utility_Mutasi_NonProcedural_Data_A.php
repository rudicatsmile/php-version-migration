...
if ($gSkP)
{
	...
	while ($mRo = mysql_fetch_array($nRs, MYSQL_BOTH))
	{
		...
		$gBc = "; ".TxBckCLR($iG);
		$imG="";
		#$CeKD = fGlobalNEW("idt","ta_kib_".$gTbL,"referensi:kd_upb",$gRf.":".$gUNI."%","=:like","",DatabaseSB,$ConSB,"");
		$gHA  = fGlobalNEW("IfNull(sum(debet),0)","ta_kib_post","referensi:kd_upb",$gRf.":".$rUP,"=:=","",DatabaseSC,$ConSC,"");
		$CeKD = fGlobalNEW("IDT","tr_tempcopy_aset","referensi:kd_upb_to",$gRf.":".$gUNI."%","=:like","",DatabaseSB,$ConSB,"");
		if ($CeKD){
	...
	<tr height="40px">
	  ...
	  <td valign="top" style="border-bottom:1px dotted #ccc <?=$gBc.$rC?>"><input type="checkbox" name="sDana" value="<?=$gId?>" <?php if ($CeKD!=""){echo "disabled";}?> /></td>
	</tr>
	<?php
		$iG++;
	}
...
<tr height="100%">
  ...
  <td width="80" valign="top" style="border-bottom:1px dotted #ccc; text-align:right; font-weight:bold; color:#FF0000"><?php if ($iG!=1){?>Tandai Semua<?php } ?></td>
  <td width="80" valign="top" style="border-bottom:1px dotted #ccc; text-align:right; font-weight:bold; color:#FF0000">&nbsp;</td>
  <td width="20" valign="top" style="border-bottom:1px dotted #ccc"><?php if ($iG!=1){?><input type="checkbox" name="sDana" value="" onclick="CheckAlla()" /><?php } ?></td>
</tr>
</table>

...
$gBDG = substr($rKdR,0,4);
$dBDG = fGlobal("Deskripsi","ref_keg_90_2","Kode",$gBDG,"=","","");
$gPRG = substr($rKdR,0,7);
$dPRG = fGlobal("Deskripsi","ref_keg_90_3","Kode",$gPRG,"=","","");
$gKEG = $rKdR;
$dKEG = fGlobal("Deskripsi","ref_keg_90_4","Kode",$gKEG,"=","","");
?>
...
<table border="0" class="table-link" cellspacing="0" cellpadding="0" align="center" style="width:1000px">
  ...
  <tr height="25">
    ...
    <td class="ac">&nbsp;</td>
    <td class="al">
	<input name="fURU" id="fURU" type="text" value="<?=$gURU?>" readonly="readonly" style="padding-left:5px; width:70px; border: 1px solid #C0C0C0"/>
	...
	style="padding-left:5px; width:420px; border: 1px solid #C0C0C0"/>
  ...
  <tr height="25">
    ...
    <td class="ac">&nbsp;</td>
    <td class="al">
	<input name="fBDG" id="fBDG" type="text" value="<?=$gBDG?>" readonly="readonly" style="padding-left:5px; width:70px; border: 1px solid #C0C0C0"/>
  ...
  <tr height="25">
    ...
    <td class="ac">&nbsp;</td>
    <td class="al">
	<input name="fPRG" id="fPRG" type="text" value="<?=$gPRG?>" readonly="readonly" style="padding-left:5px; width:70px; border: 1px solid #C0C0C0"/>
    <input name="dPRG" id="dPRG" type="text" value="<?=$dPRG?>" 	
	onclick="showPROG('','<?=$_GET['IdL']?>')" onkeypress="if (event.keyCode==13) {showPROG('find','<?=$_GET['IdL']?>'); return false;} else if (event.keyCode==27) {closeCLICK('prog'); return false;}" 
	style="padding-left:5px; width:420px; border: 1px solid #C0C0C0"/>
	<div id="progMstCri" class="find0Cri">
		<div id="progDiv1Cri" class="find1Cri"></div>
		<div id="progDiv2Cri" class="find2Cri"></div>
	</div>	</td>
    <td class="al"><a href="#" class="ico left" onclick="openLINK('3','BDG','<?=$_GET['IdL']?>')">&nbsp;</a></td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
  ...
  <tr height="25">
    ...
    <td class="ac">&nbsp;</td>
    <td class="al">
	<input name="fKEG" id="fKEG" type="text" value="<?=$gKEG?>" readonly="readonly" style="padding-left:5px; width:70px; border: 1px solid #C0C0C0"/>
    <input name="dKEG" id="dKEG" type="text" value="<?=$dKEG?>" 	
	onclick="showKEGI('','<?=$_GET['IdL']?>')" onkeypress="if (event.keyCode==13) {showKEGI('find','<?=$_GET['IdL']?>'); return false;} else if (event.keyCode==27) {closeCLICK('kegi'); return false;}" 
	style="padding-left:5px; width:420px; border: 1px solid #C0C0C0"/>
	<div id="kegiMstCri" class="find0Cri">
		<div id="kegiDiv1Cri" class="find1Cri"></div>
		<div id="kegiDiv2Cri" class="find2Cri"></div>
	</div>	</td>
    <td class="al"><a href="#" class="ico left" onclick="openLINK('4','PRG','<?=$_GET['IdL']?>')">&nbsp;</a></td>
    <td class="ar">FIND</td>
    <td>&nbsp;</td>
...
<table border="0" cellspacing="0" class="table-link" cellpadding="0" align="center" style="width:1000px; height:20px; background: #C9DCD8; font-weight:bold">
  <tr>
    ...
    <td width="670" align="left" style="padding-left:6px">SUB KEGIATAN
...
<table border="0" class="table-link" cellspacing="0" cellpadding="0" align="center" style="width:1000px; height:300px">
  <tr>
    <td valign="top">
...
<table border="0" class="table-link" cellspacing="0" cellpadding="0" align="center" style="width:1000px; height:30px">
  <tr>
    <td>&nbsp;&nbsp;&nbsp;
	<a href="#" onClick="editDATA('<?=$ReO?>','5','addnew','','','<?=$IdL?>'); return false" class="ico add">&nbsp;Add Item</a>&nbsp;&nbsp;&nbsp;
...
<script languange="javascript">
	...
	function RefreshDATA(IdL)
	{
		...
		gIdM = ReplaceText($("#fKEG").val());
		$(document).ready(function()
		{
			$.ajax({
				url:"Ref_ProKeg_90_5_Data.php",
	...
	function editDATA(ReO,Lev,CrT,IdT,rCek,IdL)
	{
		...
		{
			fKdM = $("#fKEG").val();
			if (fKdM==''){alert('Error kegiatan..!!'); return false;}
	...
	function showPROG(CrT,IdL)
	{
		KdID = $("#fBDG").val()
		if (CrT=='find'){
			gFnD = ReplaceText($("#dPRG").val());
			$(document).ready(function()
			{
				$("#progDiv2Cri").load('Ref_ProKeg_90_All_Prog_Mid.php?KdID='+KdID+'&gFnD='+gFnD+'&IdL='+IdL);
			});
		}
		else {
			dispNO('kegiMstCri');
			dispBLOCK('progDiv2Cri');
			$(document).ready(function()
			{
				$("#progDiv1Cri").load('Ref_ProKeg_90_All_Prog_Top.php?IdL='+IdL);
			});
			
			$(document).ready(function()
			{
				$("#progDiv2Cri").load('Ref_ProKeg_90_All_Prog_Mid.php?KdID='+KdID+'&IdL='+IdL);
			});
			dispBlockOrNo('progMstCri');
		}
		$("#dPRG").select();
	}
	...
	function showKEGI(CrT,IdL)
	{
		...
		if (CrT=='find'){
			...
			$(document).ready(function()
			{
				$("#kegiDiv2Cri").load('Ref_ProKeg_90_All_Kegi_Mid.php?KdID='+KdID+'&gFnD='+gFnD+'&IdL='+IdL);
			});
		}
		else {
			dispBLOCK('kegiDiv2Cri');
			$(document).ready(function()
			{
				$("#progDiv1Cri").load('Ref_ProKeg_90_All_Kegi_Top.php?IdL='+IdL);
			});
			
			$(document).ready(function()
			{
				$("#kegiDiv2Cri").load('Ref_ProKeg_90_All_Kegi_Mid.php?KdID='+KdID+'&IdL='+IdL);
			});
			dispBlockOrNo('kegiMstCri');
		}
		$("#dKEG").select();
	}

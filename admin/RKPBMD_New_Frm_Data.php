...
function RowList($stLOCK,$CrT,$iG,$mRo1,$mRo2,$mRo3,$rIdT,$gTHN,$gUBH,$IdL,$pad,$DatabaseSB,$ConSB,$xB,$Link,$eCeK)
{
	...
	?>
	<tr height="25"> 
	  <td width="29" rowspan="<?=$span?>" style="border-bottom:1px #999999 dotted; text-align:center" <?=$gBG?>><?php if ($iG){echo $iG.".";}?></td>
	  ...
	  <td width="400" rowspan="<?=$span?>" style="border-bottom:1px #999999 dotted; padding-left:<?=$pad?>px; border-left:1px #ccc solid" <?=$gBG?>><?php if ($CrT=='rekn') {echo $xB."<b>".$mRo3."</b>".$gJnSR;} else {echo strtoupper($xB.$mRo3);}?></td>
	  ...
	  <?php
	   ...
	   if ($CrT=='rekn'){
			...
			<tr>
			  ...
			  <td>&nbsp;</td>
			  <td>
			  <label><input name="fRadio<?=$rIdT?>" <?=$eDis?> type="radio" onclick="saveRECO('<?=$stLOCK?>','rb','kondisi_barang','B','<?=$rIdT?>','<?=$_GET['IdL']?>'); return false;" value="B" <?php if ($gKonD=='B'){echo "checked";}?> />B</label>
			  <label><input name="fRadio<?=$rIdT?>" <?=$eDis?> type="radio" onclick="saveRECO('<?=$stLOCK?>','rb','kondisi_barang','RR','<?=$rIdT?>','<?=$_GET['IdL']?>'); return false;" value="RR" <?php if ($gKonD=='RR'){echo "checked";}?> />RR</label>
			  <label><input name="fRadio<?=$rIdT?>" <?=$eDis?> type="radio" onclick="saveRECO('<?=$stLOCK?>','rb','kondisi_barang','RB','<?=$rIdT?>','<?=$_GET['IdL']?>'); return false;" value="RB" <?php if ($gKonD=='RB'){echo "checked";}?> />RB</label>
			  </td>

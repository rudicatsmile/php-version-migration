...
#Write Title
$worksheet->write(0, 0, "KIB-B ( ASET PERALATAN DAN MESIN )", $Merg1);
...
if ($gLmT=="YA")
{
	$nSQL= "SELECT * FROM ta_kib_b WHERE Kd_UPB LIKE '".$gUpb."' AND Kd_Aset LIKE '".$gAss."' AND (Tgl_Perolehan BETWEEN '".$gTgA."' AND '".$gTgB."') ".$fFindSy." ORDER BY Tgl_Perolehan, Kd_Aset, No_Register LIMIT ".$gReC.",10000";
}
else
{
	$nSQL= "SELECT * FROM ta_kib_b WHERE Kd_UPB LIKE '".$gUpb."' AND Kd_Aset LIKE '".$gAss."' AND (Tgl_Perolehan BETWEEN '".$gTgA."' AND '".$gTgB."') ".$fFindSy." ORDER BY Tgl_Perolehan, Kd_Aset, No_Register";
}
...
header("Content-Type: application/x-msexcel; name=\"KiB-B Choise.xls\"");
header("Content-Disposition: inline; filename=\"KiB-B Choise.xls\"");
$fh=fopen($fname, "rb");
fpassthru($fh);
unlink($fname);
?>
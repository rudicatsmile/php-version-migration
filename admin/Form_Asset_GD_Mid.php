...
?>
<body>
<form name="myfrm" method="post" action="<?php echo "Form_Asset_GD_Mid_.php?IdL=".$_GET['IdL']."&rIDT=".$rIDT ?>">
  ...
  <table border="0" align="center" width="1165">
      ...
      <td colspan="2">
		<table border="0" width="100%" cellpadding="0" style="border-collapse: collapse">
          <tr> 
            ...
            <td width="7%"> 
            <input <?php echo $gDisB?> type="hidden" name="fChoise" value="ON" />
			</td>
            <td width="44%">
              <?php echo $gTexB?>            </td>
          </tr>
    ...
    <tr> 
	  ...
	  <tr>
		<?php
		$iG=1;
		$SQL="SELECT IDT FROM ta_kib_d_pdf WHERE Referensi='$gREF' ORDER BY IDT";
		$nRs = mysql_query($SQL);
		while ($mRo = mysql_fetch_array($nRs, MYSQL_BOTH))
		{
			...
			<td width="10" style="border-right:1px solid #CCCCCC"><a href="#" onClick="P_PDF('d','600','300','<?=$rIDT?>','<?=$gIDT?>','<?=$_GET['IdL']?>'); return false" class="ico reff"></a></td>
			<?php
			$iG++;
		}
		?>
	  <td><a href="#" onClick="P_PDF('d','600','300','<?=$rIDT?>','','<?=$_GET['IdL']?>'); return false" class="ico pdf">&nbsp;&nbsp;Upl.PDF</a></td>
	  </tr>
	  </table>	  </td>
    </tr>
    <tr> 
			...
			if (win!=null)
    			{
      				win.window.document.open()       			
					<?php
					$URL_Top = "Form_Invent_Asset_Top.php?FrmG=INVENTARISASI -> PERUBAHAN NILAI -> KIB G (Jalan, Irigasi dan Jaringan)";

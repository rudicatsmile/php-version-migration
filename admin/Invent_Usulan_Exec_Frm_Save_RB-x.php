...
function ExecuteRC($frUPB,$RfA,$RfU,$TbK,$tID,$UID)
{
	...
	while ($mRo = mysql_fetch_array($Rs, MYSQL_BOTH))
	{
		...
		$gRin = "";
		$gNmB = "";
		$rDT = fGlobal("Kd_Aset:Nm_Aset","ref_rek_aset5","Kd_Aset:Link_Kib_AE","07.21.%:".$mRo['Kd_Aset'],"LIKE:=","","");
		if ($rDT){
			$rDT = explode(":",$rDT);
			$gRin = $rDT[0];
			$gNmB = $rDT[1];
		...
		if ($gRin==""){
			$gRin = "07.21.".substr($mRo['Kd_Aset'],-9,9);
	...
	if ($UdTRCI=='YA') {
		$SQ="UPDATE ta_usulan_verifikasi_rinci_108 SET Eksekusi='Sudah', To_Kd_Aset='$gRin', Pencatat='".$UID.":exe', Recorded=now() WHERE IDT='$tID'";
		$Rs = mysql_query($SQ);
	}
}
...
function UnExecuteRC($frUPB,$RfA,$RfU,$TbK,$tID,$UID)
{
	...
	if ($DelMTS == 'YA'){
		$SQ = "DELETE FROM ta_kib_".$TbK."_mutasi WHERE Referensi='$RfA' AND Ref_Usulan='$RfU' AND Kd_UPB LIKE '".$frUPB."%'";
		$Rs = mysql_query($SQ);
		
		$SQ = "DELETE FROM ta_kib_g WHERE Ref_History='$RfA' AND Ref_Usulan='$RfU' AND Kd_UPB LIKE '".$frUPB."%'";
		$Rs = mysql_query($SQ);
	}
	
	$CeG = fGlobal("IDT","ta_kib_post","Referensi:Kd_UPB",$RfA.":".$frUPB."%","=:LIKE","","");
	if ($CeG=="")
	{
		$SQ = "SELECT * FROM ta_kib_post_mutasi WHERE Referensi='".$RfA."' AND Ref_Usulan='".$RfU."' AND Kd_UPB LIKE '".$frUPB."%' ORDER BY IndexData";
		$Rs = mysql_query($SQ);
		while ($mRo = mysql_fetch_array($Rs, MYSQL_BOTH))
		{
	...
	$SQ = "DELETE FROM ta_kib_post_mutasi WHERE Referensi='$RfA' AND Ref_Usulan='$RfU' AND Kd_UPB LIKE '".$frUPB."%'";
	$Rs = mysql_query($SQ);
	
	$SQ = "DELETE FROM ta_kib_post WHERE Ref_History='$RfA' AND Ref_Usulan='$RfU' AND Kd_UPB LIKE '".$frUPB."%'";
	$Rs = mysql_query($SQ);
	
	$SQ="UPDATE ta_usulan_verifikasi_rinci_108 SET Eksekusi='Belum', Pencatat='".$UID.":unExe', Recorded=now() WHERE IDT='$tID'";
	$Rs = mysql_query($SQ);
}

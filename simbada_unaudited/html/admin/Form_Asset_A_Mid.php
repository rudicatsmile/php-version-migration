...
if ($rIDT!="")
{
	...
	if ($tRo > 0)
	{
		...
		$gIMG  = $mRo['file_name'];	
		$gREF  = $mRo['Referensi'];
		$rCnt  = fGlobal("count(*)","ta_kib_post_108","Referensi",$gREF,"=","","");
		...
		$gBtsU = $mRo['Batas_Utara'];
		$gBtsT = $mRo['Batas_Timur'];
		$gBtsS = $mRo['Batas_Selatan'];
		$gBtsB = $mRo['Batas_Barat'];
	}
}
else
{
	$gNoR  = MakeNewNoRegiterAuto('TNH','');
	$gLua  = 0;
	$gSTN  = 0;
	$gTTL  = 0;
	$gHRG  = 0;
...
?>
<body>
<form name="myfrm" method="post" action="<?php echo "Form_Asset_A_Mid_.php?IdL=".$_GET['IdL']."&rIDT=".$rIDT ?>">
  <input type="hidden" name="Simpan">
  <input type="hidden" name="RadioTemp" value="<? echo $gSrT?>">
  <table border="0" align="center" width="1165">
    ...
    <tr> 
      ...
      <td><select name="fAsalUsul" class="boxs" id="select" style="width: 250px" tabindex="0">
		...
		if ($tRo > 0)
		{
			...
			{
				$sel ="";
				if (strtolower($mRo['Perolehan'])==strtolower($gAUS)) 
				{
					$sel ="selected";
					$gKDS=$mRo['Perolehan'];
				}
    ...
    <tr> 
      ...
      <td>JUMLAH SATUAN</td>
      <td>
	  <input name="fSatuan" type="text" id="fSatuan" maxlength="4" value="<?=$gSTN?>" <?=$rRead?> <? if ($KeY=="Y") {echo "readonly";}?> onBlur="NumValidate(this)" onKeyUp="addSeparator(this)" style="width:50px; text-align : right; border: 1px solid #C0C0C0; padding-left: 1px; padding-right: 5px; padding-top: 1px; padding-bottom: 1px" />
      <em><strong>&nbsp;&nbsp;</strong></em><strong><font color="#008000">BIDANG</font></strong><em><strong> </strong></em>	  </td>
    </tr>
    <tr> 
      <td>RINCIAN OBJEK</td>
      <td>
    ...
    <tr>
	  ...
	  <table border="0" width="100%" cellpadding="0" style="border-collapse: collapse">
          <tr> 
            <td width="47%"> <input type="button" name="B39" value="SIMPAN" onclick="P_Save('<?=$ReO?>')" <? if ($ReO=='Y'){echo "disabled";}?> style="width: 90px; height: 23px; padding-left: 0px; padding-right: 0px; padding-top: 0px; padding-bottom: 0px" /> 
            ...
            <td width="6%"> <input <? echo $gDisB?> type="checkbox" name="fChoise" value="ON" /></td>
    ...
    <tr> 
	  ...
	  <tr>
		...
		while ($mRo = mysql_fetch_array($nRs, MYSQL_BOTH))
		{
			...
			<td width="18"><a href="#" onClick="P_PRE('a','800','400','<?=$gIDT?>','<?=$_GET['IdL']?>'); return false" class="ico prev"></a></td>
			<td width="10" style="border-right:1px solid #CCCCCC"><a href="#" onClick="P_PDF('<?=$ReO?>','a','600','300','<?=$rIDT?>','<?=$gIDT?>','<?=$_GET['IdL']?>'); return false" class="ico reff"></a></td>
			<?
			$iG++;
		}
		?>
	  <td><a href="#" onClick="P_PDF('<?=$ReO?>','a','600','300','<?=$rIDT?>','','<?=$_GET['IdL']?>'); return false" class="ico pdf">&nbsp;&nbsp;Upl.PDF</a></td>
	  </tr>
	  </table>	  </td>
    </tr>
    <tr> 
      ...
      <td colspan="2" rowspan="12">  <table border="0" width="430" style="border-collapse: collapse; color: #CC0000; border: 1px solid #8FA908; padding-left: 4px; padding-right: 4px; padding-top: 1px; padding-bottom: 1px">
          ...
          <tr> 
            ...
            <td>KODE LOKASI</td>
            <td>:</td>
            <td colspan="2"><? echo $gLok?></td>
          </tr>
          <tr>
            <td>&nbsp;</td>
            <td>NILAI AKHIR </td>
            <td>:</td>
            <td colspan="2"><? echo fConvertToRupiah($gNIL)?></td>
          </tr>
          <tr> 
            <td>&nbsp;</td>
            <td>&nbsp;</td>
			...
			<table border="0" width="97" bgcolor="#FFFFFF" cellpadding="0" style="font-size: 8pt; font-family: Calibri; border-collapse: collapse">
			  ...
			  <tr>
				...
				<? if ($gIMG!="") {?>
					<a href="#"><img src="<?="SourceIMG.php?rCRT=a&rIDT=".$rIDT?>" height="75" width="80" onclick="PreviewIMG('400','300','center')"></a>
				<? } else {?>
          ...
          <tr> 
            ...
            <td>:</td>
            <td> 
              <? if (isset($gREGa)) {echo $gREGa." s.d ";} else {echo "-";}?>
              <? if (isset($gREGb)) {echo $gREGb;}?>            </td>
          </tr>
          <tr> 
            <td width="15">&nbsp;</td>
            <td width="112">&nbsp;</td>
          ...
          <tr> 
			...
			<table border="0" width="97%" cellpadding="0" style="font-size: 8pt; font-family: Calibri; border-collapse: collapse">
			  ...
			  <tr height="20">
			    <td width="96"><a href="<?="P47_Penggunaan.php?BckFrm=A&rIDT=".$_GET['rIDT']."&IdL=".$_GET['IdL']?>" class="ico use">Penggunaan</a></td>
			    <td width="115"><a href="<?="P47_Pemanfaatan.php?BckFrm=A&rIDT=".$_GET['rIDT']."&IdL=".$_GET['IdL']?>" class="ico mesi">&nbsp;&nbsp;Pemanfaatan</a></td>
			    <td width="92"><!--a href="#<?="P47_Pengamanan.php?BckFrm=A&rIDT=".$_GET['rIDT']."&IdL=".$_GET['IdL']?>" class="ico key">&nbsp;&nbsp;Pengamanan</a--></td>
			    <td>&nbsp;</td>
			  </tr>
			</table>
			</td>
    ...
    <tr>
      ...
      <td>
		<table border="0" width="100%" cellpadding="0" style="font-family: Arial; font-size:8pt; font-weight: bold; border-collapse: collapse">
			<tr>
				<td width="7%"><input name="fSertifikat" type="radio" <? echo $CK1 ?> onclick="P_RadioClick('Ada')" value="Ada" /></td>
				<td width="13%">ADA</td>
				<td width="6%"><input name="fSertifikat" type="radio" <? echo $CK2 ?> onclick="P_RadioClick('Tidak Ada')" value="Tidak Ada" /></td>
				<td width="20%">TIDAK ADA</td>
				<td width="6%"><input name="fSertifikat" type="radio" <? echo $CK3 ?> onclick="P_RadioClick('Dalam Proses')" value="Dalam Proses" /></td>
    ...
    <tr> 
      ...
      <td><input name="fNom" type="text" class="text" id="fNom" style="widht:100%; border: 1px solid #C0C0C0; padding-left: 1px; padding-right: 1px; padding-top: 1px; padding-bottom: 1px" value="<? echo $gNom?>" size="50" maxlength="50" /></td>
      <td>&nbsp;</td>
    </tr>
    <tr> 
      <td>LETAK TANAH </td>
		<td>
			<input name="fAlmt" type="text" class="text" id="fAlmt" style="widht:100%; border: 1px solid #C0C0C0; padding-left: 1px; padding-right: 1px; padding-top: 1px; padding-bottom: 1px" value="<? echo $gAlm?>" size="50" maxlength="50" />
      ...
      <td>KOORDINAT</td>
		<td>
			...
			<input name="fPosisi_koordinat" type="text" class="text" id="fPosisi_koordinat" 
				...
				value="<? echo $posisi_koordinat?>" size="50" maxlength="50" disabled />
		</td>
      <td>&nbsp;</td>
    </tr>
    <tr> 
      <td>PENGGUNAAN</td>
      <td><input name="fGuna" type="text" class="text" id="fGuna" style="widht:100%; border: 1px solid #C0C0C0; padding-left: 1px; padding-right: 1px; padding-top: 1px; padding-bottom: 1px" value="<? echo $gGna?>" size="50" maxlength="50" /></td>
      <td>&nbsp;</td>
    </tr>
    <tr> 
      <td>KETERANGAN</td>
      <td><textarea name="fKetr" cols="48" rows="2" id="fKetr" style="border: 1px solid #C0C0C0"><? echo $gKTR ?></textarea></td>
      <td>&nbsp;</td>
    </tr>
    <tr> 
      <td>BATAS</td>
      <td>
	  U&nbsp; <input name="fBtsU" id="fBtsU" type="text" class="text" style="width:170px; padding-left:2px; border:1px solid #C0C0C0" value="<?=$gBtsU?>"/>
	  T&nbsp;&nbsp; <input name="fBtsT" id="fBtsT" type="text" class="text" style="width:170px; padding-left:2px; border:1px solid #C0C0C0" value="<?=$gBtsT?>"/><br>
	  </td>
      <td>&nbsp;</td>
    </tr>
    <tr> 
      <td>&nbsp;</td>
      <td>
	  S&nbsp;&nbsp; <input name="fBtsS" id="fBtsS" type="text" class="text" style="width:170px; padding-left:2px; border:1px solid #C0C0C0" value="<?=$gBtsS?>"/>
	  B&nbsp;&nbsp; <input name="fBtsB" id="fBtsB" type="text" class="text" style="width:170px; padding-left:2px; border:1px solid #C0C0C0" value="<?=$gBtsB?>"/>
...
<script language="javascript">
			...
			if (win!=null)
    			{
      				win.window.document.open()       			
					<?
					$URL_Top = "Form_Invent_Asset_Top.php?FrmG=INVENTARISASI -> PERUBAHAN NILAI -> KIB A (ASET TANAH)";
					$URL_Mid = "Form_Invent_Asset_Ubh.php?rKib=Kib_A&IdL=".$_GET['IdL']."&rIDT=".$rIDT;
	...
	function P_ViewHistory(w,h,pos)
	{
		...
		<?
			{
			$URL="KIB_A_History.php?rIDT=".$rIDT."&IdL=".$_GET['IdL'];
			}
		?>
	...
	function P_MoveAccount(ReO,KeY,w,h,rIDT,IdL)
	{
		...
		if (win!=null)
		{
			...
			URL_Mid = 'Move_Account_Mid.php?rKib=Kib_A&IdL='+IdL+'&rIDT='+rIDT;
	...
	function P_MoveUPB(ReO,KeY,w,h,rIDT,IdL)
	{
		...
		{
			win.window.document.open()       			
			URL_Top = 'Move_Upb_Kib_Top.php?FrmG=PEMINDAHAN KODE UPB -> KIB A (Tanah)';
			URL_Mid = 'Move_Upb_Kib_Mid.php?rKib=Kib_A&IdL='+IdL+'&rIDT='+rIDT;
   	...
   	function PreviewIMG(w,h,pos)
	{
		...
		<?
			{
			$URL="PreviewIMG.php?rCRT=a&rIDT=".$rIDT."&IdL=".$_GET['IdL'];
			}
		?>
	...
	{	
		...
		{
			win.window.document.open()       			
			URL_Top = "UploadIMG_Top.php?FrmG=UPLOAD FOTO ASET -> KIB A (ASET TANAH)";
			URL_Mid = "UploadIMG_Mid.php?"+"<?="rCRT=a&rIDT=".$rIDT."&IdL=".$_GET['IdL']?>";
	...
	{	
		...
		if (win!=null)
		{
			...
			URL_Mid = "UploadMAP_Mid.php?"+"<?="rCRT=a&rIDT=".$rIDT."&IdL=".$_GET['IdL']?>";
			URL_Bot = "UploadIMG_Bot.php";
			txtHTML="<html><head><title>Simbada Kab. Hulu Sungai Tengah</title></head><frameset framespacing='0' border='0' rows='45,*,30' frameborder='0'><frame name='WinFindAcc_Top' noresize src='"+URL_Top+"' scrolling='no'><frame name='WinFindAcc_Mid' src='"+URL_Mid+"' scrolling='auto'><frame name='WinFindAcc_Bot' src= '"+URL_Bot+"' scrolling='no'><noframes><body><p>=>.............??!</p></body></noframes></frameset></html>"            
			win.focus()
			win.window.document.clear()

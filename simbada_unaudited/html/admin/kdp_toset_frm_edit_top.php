...
<table border="0" class="table-link" cellspacing="0" cellpadding="0" align="center" style="width:1148px; background-color:#D2DAC4">
  ...
  <tr height="23">
	...
	<input name="eUpb" id="eUpb" type="text" value="<?=$eUpb?>" readonly <? if (!$IdT){?> onclick="findUPB('','upb','<?=$IdL?>'); return false;" <? } ?> style="padding-left:5px; height:15px; width:400px; border: 1px solid #C0C0C0"/>
  ...
  <tr height="23">
	...
	<input name="eRin" id="eRin" type="text" value="<?=$eRin?>" readonly <? if (!$IdT){?> onclick="findREK('','rek','<?=$IdL?>'); return false;" <? } ?> style="padding-left:5px; height:15px; width:400px; border: 1px solid #C0C0C0"/>
	</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
  </tr>
  <tr height="23">
    ...
    <td>&nbsp;</td>
    <td>
	<input type="button" name="B3923" value="SAVE" <? if ($eXec=='Y'){echo "disabled";}?> onclick="SaveDATA('<?=$CrDiv?>','<?=$IdT?>','<?=$IdL?>')" style="width:70px; height: 21px" />

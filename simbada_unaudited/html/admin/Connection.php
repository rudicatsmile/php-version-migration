...
define('HostnameSB','db-server');
define('DatabaseSB','simbada_unaudited');
define('UsernameSB','root');
define('PasswordSB','ra11-192011');
...
function fGlobal($nFldD,$nTbl,$nFldF,$nSyR,$fArtM,$nOdr,$fShow)
{
	...
	if ($tRowGlo > 0){
		if ($JmLa > 0){
			...
			for ($iGL=0; $iGL<=$JmLa; $iGL++){
				if ($iGL==0){
					$zFldD=str_replace(":", " ",$mRowGlo[$Sx[$iGL]]);
				}
				else{
					$zFldD=$zFldD.":".str_replace(":", " ",$mRowGlo[$Sx[$iGL]]);
				}
			}
			return $zFldD;
		}
...
#$tesr = fGlobalJOIN("IfNull(sum(P2.debet),0)","ta_kib_b p1","ta_kib_post p2","","Referensi","Referensi","P1.Referensi","ALT.00000019448","=","","s");
#echo $tesr;
/*
function fGlobalJOIN($nFldD,$nTbl,$nTbl2,$rJoin,$rFld1,$rFld2,$nFldF,$nSyR,$fArtM,$nOdr,$fShow)
{
	$JmLa=fHiSplitT2K($nFldD,":");
	$JmLb=fHiSplitT2K($nFldF,":");
	$fArtM = strtoupper($fArtM);
	...
	if ($nOdr!="") { $nOdr=" ORDER BY ".$nOdr;}
	if ($JmLa > 0) {
		$SQGL = "SELECT ".str_replace(":", ", ",$nFldD);
		$SQGL.= " FROM ".strtolower($nTbl);
		$SQGL.= " ".$rJoin." JOIN ".$nTbl2;
		$SQGL.= " ON P2.".$rFld1."=P1.".$rFld2;
		$SQGL.= " WHERE ".$zFldF.$nOdr;
	}
	else {
		$SQGL = "SELECT ".str_replace(":", ", ",$nFldD)." AS FldD";
		$SQGL.= " FROM ".strtolower($nTbl);
		$SQGL.= " ".$rJoin." JOIN ".$nTbl2;
		$SQGL.= " ON P2.".$rFld1."=P1.".$rFld2;
		$SQGL.= " WHERE ".$zFldF.$nOdr;
	...
	if ($tRowGlo > 0){
		if ($JmLa > 0){
			$Sx=explode(':',$nFldD);
			$Sx=str_replace('p1.','',$Sx);
			$Sx=str_replace('p2.','',$Sx);
			for ($iGL=0; $iGL<=$JmLa; $iGL++){
				if ($iGL==0){
					$zFldD=str_replace(":", " ",$mRowGlo[$Sx[$iGL]]);
				}

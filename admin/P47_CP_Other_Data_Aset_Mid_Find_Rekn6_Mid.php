...
<table align="center" cellpadding="0" class="table-listpop" cellspacing="0" width="100%" height="240" border="0">
	<?php
	$iG=1;
	$nSQ = "SELECT Kd_Aset, Nm_Aset FROM ref_rek_aset108_6 WHERE Kd_Aset LIKE '".$Rek5."%' $CrT ORDER BY Kd_Aset";
	$nRs = mysql_query($nSQ);
	while ($mRo = mysql_fetch_array($nRs, MYSQL_BOTH))
	{
		...
		$gNm = $mRo[1];
		?>
		<tr height="20" onclick="showCLICK('rekn6','<?=$gKd?>','<?=$gNm?>','<?=$IdL?>'); return false;">

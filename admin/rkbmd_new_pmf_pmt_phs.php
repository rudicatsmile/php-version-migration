...
<table border="0" class="table-link" cellspacing="0" cellpadding="0" align="center" style="width:99%">
  ...
  <tr height="25">
    ...
    <td style="padding-right:5px"><input type="button" name="B39" id="B39" value="GO" onclick="RefreshDATA('<?=$Frm?>','<?=$Crit?>','<?=$IdL?>')" style="width: 30px; height: 21px" />
...
<table border="0" cellspacing="0" class="table-link" cellpadding="0" align="center" style="width:99%; height:20px; background: #C9DCD8; font-weight:bold">
  <tr style="text-align:center">
    ...
    <td width="300" style="border-right:1px solid #ccc">SKPD</td>
    <td width="300" style="border-right:1px solid #ccc">Uraian</td>
    <td width="120" style="border-right:1px solid #ccc">Nilai</td>
...
<script languange="javascript">
	...
	function RefreshDATA(Frm,Crit,IdL)
	{
		gJNS = "";
		...
		{
			$.ajax({
				url:"rkbmd_new_pmf_pmt_phs_data.php",
				data: {Frm:Frm,Crit:Crit,gUnT:gUnT,gHR:gHR,gBL:gBL,gTH:gTH,gHRd:gHRd,gBLd:gBLd,gTHd:gTHd,gFnD:gFnD,IdL:IdL},
				type:"get",
				beforeSend:function()
				{
					$("#loadingImg").show();
	...
	function showEDIT(IdT,Frm,Crit,IdL)
	{
		//window.open('rkbmd_new_pmf_pmt_phs_frm.php?IdT='+IdT+'&Frm='+Frm+'&Crit='+Crit+'&IdL='+IdL+'&FrmG='+FrmG,'_self');
		...
		fE="";
		window.open('rkbmd_new_pmf_pmt_phs_frm.php?FrmG=RENCANA '+fR+' BARANG MILIK DAERAH '+fE+'&IdT='+IdT+'&Frm='+Frm+'&Crit='+Crit+'&IdL='+IdL,'_self');
	}
	...
	function P_Dokumen(w,h,IdT,Frm,Crit,IdL)
	{
		...
		URL='rkbmd_new_pmf_pmt_phs_frm_data_dok_'+Frm.toLowerCase()+'.php?Crit='+Crit+'&IdT='+IdT+'&IdL='+IdL;
	...
	function showDOC(pil,w,h,IdL)
	{
		...
		LeftPosition=(screen.width)?(screen.width-w)/2:100; 
		TopPosition=(screen.height)?(screen.height-h)/2:100;
		URL='rkbmd_new_pmf_pmt_phs_data_dok_'+pil+'.php?gUNT='+gUNT+'&HR1='+HR1+'&HR2='+HR2+'&HR3='+HR3+'&BL1='+BL1+'&BL2='+BL2+'&BL3='+BL3+'&TH1='+TH1+'&TH2='+TH2+'&TH3='+TH3+'&IdL='+IdL;
		settings='width='+w+',height='+h+',top='+TopPosition+',left='+LeftPosition+',location=no,directories=no,status=no,menubar=no,toolbar=no,resizable=no,maximize=no,scrollbars=yes,navigation=no';
		window.open(URL,'',settings);
		//closeCLICK('choise');
	}
	
	function showDELE(IdT,Cek,Frm,Crit,IdL)
	{
		...
		if (AN)
		{
			...
			{
				$.ajax({
					url:"rkbmd_new_pmf_pmt_phs_data_dell.php",
					data: {IdT:IdT,Frm:Frm,Crit:Crit,IdL:IdL},
					type:"get",
					beforeSend:function()
					{
						$("#loadingImg").show();

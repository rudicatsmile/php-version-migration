...
<table border="0" class="table-link" cellspacing="0" cellpadding="0" align="center" style="width:99%; background:#339999; color:#fff">
  <tr>
    <td width="10">&nbsp;</td>
    <td width="73">&nbsp;</td>
    <td width="15">&nbsp;</td>
  ...
  <tr height="25">
	...
	<input name="dUNT" type="text" value="<?=$dUNT?>" <? if (!$IdT) {?> onClick="showUNIT('','<?=$_GET['IdL']?>')" onkeypress="if (event.keyCode==13) {showUNIT('find','<?=$_GET['IdL']?>'); return false;} else if (event.keyCode==27) {closeCLICK('unit'); return false;}" <? } else {echo "readonly";}?> style="padding-left:5px; width:372px; border: 1px solid #C0C0C0; text-transform:uppercase"/>
...
<table border="0" class="table-link" cellspacing="0" cellpadding="0" align="center" style="width:99%; height:30px; background: #EAE6E6">
  <tr>
    ...
    <td><input type="hidden" name="B39" <?=$DisA?> value="SAVE" onclick="Save('<?=$stLOCK?>')" style="width: 80px; height: 21px" />
	  ...
	  <input type="hidden" name="B12" value="ADD PROGRAM" onclick="showPROG('<?=$stLOCK?>','','<?=$IdT?>','<?=$_GET['IdL']?>')" style="width: 130px; height: 21px; color:#0000FF" /><?=str_repeat("&nbsp;",2)?>
	  <input type="button" name="B13" value="DAFTAR USULAN" onclick="showDATANew('<?=$_GET['IdL']?>')" style="width: 130px; height: 21px" />
	  &nbsp;&nbsp;
	  <input type="button" name="B14" <? if ($IdT=="") {echo "disabled";}?> value="CETAK RINCI" onclick="P_Dokumen('800','400','<?=$gUBH?>','<?=$IdT?>','<?=$_GET['IdL']?>')" style="width: 120px; height: 21px; color:#0000FF" />
...
<script languange="javascript">
	...
	function RefreshDATA(stLOCK,IdL)
	{
		...
		{
			$("#ViewDATA").load('RKPBMD_New_Telaah_Frm_Data.php?gREF='+gREF+'&gTHN='+gTHN+'&gUBH='+gUBH+'&IdL='+IdL);
		});
		
		$(document).ready(function()
		{
			$.ajax({
				url:"RKPBMD_New_Telaah_Frm_Data.php",
	...
	function showDATANew(IdL)
	{
		...
		window.open('RKPBMD_New_Telaah.php?FrmG=DAFTAR USULAN PEMELIHARAAN BMD (TELAAH)&gUNT='+gUNT+'&IdL='+IdL,'_self');
	}
	...
	function P_Dokumen(w,h,UbH,IdT,IdL)
	{
		...
		URL='RKPBMD_New_Telaah_Dokumen'+file+'.php?IdT='+IdT+'&IdL='+IdL;
	...
	function saveRECO(stLOCK,rb,crt,fld,rIdT,IdL)
	{
		...
		$(document).ready(function()
		{
			$("#ViewDELL").load('RKPBMD_New_Telaah_Frm_Find_PKRK_Save.php?stLOCK='+stLOCK+'&crt='+crt+'&fld='+fld+'&rIdT='+rIdT+'&IdL='+IdL);
		});
	}

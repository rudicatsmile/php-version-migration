...
class PDF extends FPDF
{
	...
	{	
		...
		$this->Cell(1);
		$this->Cell(0,12,'KARTU INVENTARIS BARANG (KIB-C)',0,0,'C');	
		$this->Ln(5);
		
		$this->Cell(1);
		$this->Cell(0,12,'BANGUNAN DAN GEDUNG',0,0,'C');	
		$this->Ln(8);
		...
		$R2=array(8,53,22,15,15,20,15,20,40,15,20,15,20,22,22,25,50);
		...
		$this->Cell($R2[4],10,'Kondisi','TLRB',0,'C',true);
		$this->Cell($R2[5],10,'Bertingkat','TLRB',0,'C',true);
		$this->Cell($R2[6],10,'Beton','TLRB',0,'C',true);
		$this->Cell($R2[7],10,'Luas Lantai','TLRB',0,'C',true);
		$this->Cell($R2[8],10,'Letak/Lokasi','TLRB',0,'C',true);
		$this->Cell($R2[9],10,'Tgl.Dok','TLRB',0,'C',true);
		$this->Cell($R2[10],10,'No.Dok','TLRB',0,'C',true);
		$this->Cell($R2[11],10,'Luas (M2)','TLRB',0,'C',true);
		$this->Cell($R2[12],10,'Status Tanah','TLRB',0,'C',true);
		$this->Cell($R2[13],10,'Kode Tanah','TLRB',0,'C',true);
		$this->Cell($R2[14],10,'Asal-Usul','TLRB',0,'C',true);
		$this->Cell($R2[15],10,'Harga (Rp)','TLRB',0,'C',true);
	...
	{
		$iG=1; $tJml=0;
		$R3=array(8,53,22,15,15,20,15,20,40,15,20,15,20,22,22,25,50);
		$this->SetFillColor(255,255,255);
		$this->SetFont('arial','',7);
				
		$SQL = "SELECT * FROM ta_kib_c WHERE Kd_UPB='".$gUpb."' AND Tgl_Perolehan LIKE '".$gThn."-__-__' AND Kd_Pemilik LIKE '".$gMLK."' AND Status='' ORDER BY Tgl_Perolehan, Kd_Aset, No_Register";
		...
		if ($tRo > 0)
		{
			do
			{
				...
				$this->Cell($R3[4],5,$mRo['Kondisi'],'TLRB',0,'C',true);
				$this->Cell($R3[5],5,$mRo['Bertingkat'],'TLRB',0,'C',true);
				$this->Cell($R3[6],5,$mRo['Beton'],'TLRB',0,'C',true);
				if ($mRo['Luas_Lantai'] > 0) {$LuasL = fConvertToRupiahBulat($mRo['Luas_Lantai']);} else {$LuasL="";}
				$this->Cell($R3[7],5,$LuasL,'TLRB',0,'C',true);
				$this->Cell($R3[8],5,substr($mRo['Lokasi'],0,30),'TLRB',0,'L',true);
				$this->Cell($R3[9],5,$mRo['Dokumen_Tanggal'],'TLRB',0,'L',true);
				...
				$this->Cell($R3[14],5,$mRo['Asal_Usul'],'TLRB',0,'L',true);
				...
				$this->Cell($R3[15],5,fConvertToRupiah($gAKH),'TLRB',0,'R',true);

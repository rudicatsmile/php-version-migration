...
<table border="0" class="table-link" cellspacing="0" cellpadding="0" align="center" style="width:99%">
  ...
  <tr height="25">
	...
	{
		$sel ="";
		if ($gTHN==$i){$sel ="selected";}
		echo '<option '.$sel.' value="'.$i.'">'.$i.'</option>';
	}
	?>
    </select>	</td>
    <td>
	<select class="boxs" name="fUBH" id="fUBH" tabindex="0" style="width:90px; background:#99FF00" onchange="RefreshDATA('<?=$IdL?>')">
  ...
  <tr>
    ...
    <td>&nbsp;</td>
    <td width="66">&nbsp;</td>
    <td><div id="loadingImg" style="width:40px; height:10px; display:none; text-align:center"><img src="Images/loading3.gif" alt="" width="30" height="30"></div></td>
  </tr>
</table>
<table border="0" cellspacing="0" class="table-link" cellpadding="0" align="center" style="width:99%; height:20px; background: #C9DCD8; font-weight:bold">
  <tr>
    ...
    <td>DESKRIPSI</td>
    <td width="90">RKPBMD</td>
    <td width="90">Jumlah/Satuan</td>
...
<script languange="javascript">
	...
	function RefreshDATA(IdL)
	{
		...
		if (ShoW==1){
			...
			{
				$("#ViewDATA").load('RKPBMD_New_Data.php?gUnT='+gUnT+'&gFnD='+gFnD+'&gTHN='+gTHN+'&gUBH='+gUBH+'&IdL='+IdL);
			});
		}
		else{
			$(document).ready(function()
	...
	{
		window.open('RKPBMD_New_Frm.php?FrmG=RENCANA KEBUTUHAN PEMELIHARAAN BARANG MILIK DAERAH&IdT='+IdT+'&IdL='+IdL,'_self');
	}
	
	function P_Dokumen(IdT,w,h,UbH,IdL)
	{
		var win=null;
		var txtHTML = "";
		var iErrors=0;
		...
		URL='RKPBMD_New_Dokumen'+file+'.php?IdT='+IdT+'&IdL='+IdL;
	...
	function showDOC(IdT,w,h,IdL)
	{
		...
		LeftPosition=(screen.width)?(screen.width-w)/2:100; 
		TopPosition=(screen.height)?(screen.height-h)/2:100;
		URL='RKPBMD_New_DokumenX.php?IdT='+IdT+'&IdL='+IdL;
		settings='width='+w+',height='+h+',top='+TopPosition+',left='+LeftPosition+',location=no,directories=no,status=no,menubar=no,toolbar=no,resizable=no,maximize=no,scrollbars=yes,navigation=no';
		window.open(URL,'',settings);
	}
	...
	function exportFORM(IdL)
	{
		...
		if (AN)
		{
			...
			if (Expo==1){
				$(document).ready(function()
				{
					$("#ViewDATA").load('RKPBMD_New_Export.php?gUNT='+gUNT+'&gTHN='+gTHN+'&gUBH='+gUBH+'&IdL='+IdL);
				});
			}
			else{
				$(document).ready(function()

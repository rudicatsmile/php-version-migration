...
<table align="center" cellpadding="0" class="table-listpop" cellspacing="0" width="100%" height="100%" border="0" style="font-family:arial; font-size:8pt">
	...
	while ($mRo = mysql_fetch_array($nRs, MYSQL_BOTH))
	{
		...
		$CeK = fGlobal("IDT","ta_rkpbmd_new_reknaset","Referensi:Ref_Aset",$gREF.":".$gRF,"=:=","","");
		if ($CeK!=""){
			$ico = "none";
			$aset= "edit";
			$mesg= "mesga";
			...
			$CeK = fGlobal("Referensi","ta_rkpbmd_new_reknaset","Ref_Aset:Kd_Unit:Referensi",$gRF.":".$gUNT."%:".$gREF,"=:LIKE:<>","","");
			if ($CeK!=""){
				$ico = "none";
				$aset= "none";
				$mesg= "mesgb";
		...
		<tr height="20">
			...
			<a href="#" class="ico <?=$ico?>" onclick="showCLICK('aset','<?=$mesg?>','<?=$refs?>','<?=$kIdT?>','<?=$rIdT.":".$gTBL?>','<?=$IdL?>'); return false;"><?=$aset?></a>			</td>
	...
	<tr height="20" valign="top">
		...
		<td>&nbsp;</td>
		<td colspan="7">&nbsp;</td>
		<td colspan="2" align="center" style=" border:1px solid #FF0000; vertical-align:middle; cursor: pointer" onclick="proslNIL('aset','<?=$kIdT?>','<?=$gTBL?>','<?=$IdL?>'); return false;">

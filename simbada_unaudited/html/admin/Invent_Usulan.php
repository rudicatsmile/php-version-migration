...
<table border="0" class="table-link" cellspacing="0" cellpadding="0" align="center" style="width:99%">
  ...
  <tr height="25">
    ...
    <td align="center">&nbsp;</td>
    <td><select class="boxs" name="fJNS" tabindex="0" style="width:180px" onchange="RefreshDATA('<?=$FrmG?>','<?=$IdL?>')">
...
<script languange="javascript">
	...
	function RefreshDATA(FrmG,IdL)
	{
		...
		//{
		//	$("#ViewDATA").load('Invent_Usulan_Data.php?FrmG='+FrmG+'&gUnT='+gUnT+'&gHR='+gHR+'&gBL='+gBL+'&gTH='+gTH+'&gHRd='+gHRd+'&gBLd='+gBLd+'&gTHd='+gTHd+'&gFnD='+gFnD+'&gJNS='+gJNS+'&IdL='+IdL);
		//});
	...
	{
		window.open('Invent_Usulan_Frm.php?IdT='+IdT+'&FrmG='+FrmG+'&IdL='+IdL,'_self');
	}
	...
	function showDOC(pil,w,h,IdL)
	{
		...
		LeftPosition=(screen.width)?(screen.width-w)/2:100; 
		TopPosition=(screen.height)?(screen.height-h)/2:100;
		URL='Invent_Usulan_Frm_Data_Dok_'+pil+'.php?gUNT='+gUNT+'&JeNS='+JeNS+'&HR1='+HR1+'&HR2='+HR2+'&HR3='+HR3+'&BL1='+BL1+'&BL2='+BL2+'&BL3='+BL3+'&TH1='+TH1+'&TH2='+TH2+'&TH3='+TH3+'&IdL='+IdL;
		settings='width='+w+',height='+h+',top='+TopPosition+',left='+LeftPosition+',location=no,directories=no,status=no,menubar=no,toolbar=no,resizable=no,maximize=no,scrollbars=yes,navigation=no';
		window.open(URL,'',settings);
		//closeCLICK('choise');
	}
	...
	function showDELE(FrmG,ReO,IdT,Cek,IdL)
	{
		...
		if (AN)
		{
			...
			{
				$.ajax({
					url:"Invent_Usulan_Data_Dell.php",
	...
	function showDELEAll(FrmG,IdT,Cek,IdL)
	{
		...
		if (AN)
		{
			...
			{
				$.ajax({
					url:"Invent_Usulan_Data_Dell_All.php",
					data: {FrmG:FrmG,IdT:IdT,IdL:IdL},
					type:"get",
					beforeSend:function()
					{

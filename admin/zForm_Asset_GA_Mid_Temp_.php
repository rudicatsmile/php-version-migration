...
require "Form_Asset_G_Mid_Temp_Posting.php";
...
if ($Smp=="Save")
{
	...
	$nSQ = "SELECT No_Berkas, Nomor, Kd_Aset, Nilai, Pros FROM ta_pengadaan WHERE IDT='$gIdT'";
	...
	if ($rIDT) {$gNiL = fHitSisaD("ta_kib_g_temp",$gNoM,$gNiL,$fTotal,$rIDT,DatabaseSB,$ConSB);}	//''**''//
	else {$gNiL = fHitSisaD("ta_kib_g_temp",$gNoM,$gNiL,$fTotal,"",DatabaseSB,$ConSB);}				//''**''//
	$gHRG = 0;																						//''**''//
	...
	if ($rIDT)
	{
		$rSTN = fGlobalNEW("Jumlah_Bidang","ta_kib_g_temp","IDT",$rIDT,"=","",DatabaseSB,$ConSB,"");
		$rUpb = fGlobalNEW("Kd_UPB","ta_kib_g_temp","IDT",$rIDT,"=","",DatabaseSB,$ConSB,"");
		$RfTMP= fGlobalNEW("Referensi","ta_kib_g_temp","IDT",$rIDT,"=","",DatabaseSB,$ConSB,"");	//'''''''''''''''''''
		
		$SQL = "UPDATE ta_kib_g_temp SET 
		Kd_UPB='$gUpb',
		...
		if (($gSTN != $rSTN) || ($gUpb != $rUpb)) {DeleteData("ta_kib_g",$rIDT,$gNoM,$RfTMP,DatabaseSB,$ConSB);}	//'''''''''''''''''''
		else
		{
			//''''''''''''''''''''''''' {Harga => Ref_Temp}
	...
	else
	{
		...
		//''''''''''''''''''''''''' {Referensi}
		$SQL = "INSERT INTO ta_kib_g_temp SET 
		No_Pengadaan='$gNoM',
		Referensi='$NewRfTMP',
		Tanggal='$gTgL',
		Kd_UPB='$gUpb',
		...
		No_SP2D='',
		Jumlah_Bidang='$gSTN',
		Nilai_Pengadaan='$gNiL', Harga_Satuan='$gHRG'";
		$rst = mysql_query($SQL) or die(mysql_error());
		
		$rIDT = fGlobal("Max(IDT)","ta_kib_g_temp","No_Pengadaan",$gNoM,"=","","");		//'''''''''''''''''
	}
...
$URL="Form_Asset_A_Mid_Temp.php?gIdT=".$gIdT."&rIDT=".$rIDT."&IdL=".$IdL;
header("Location: ".$URL);
?>

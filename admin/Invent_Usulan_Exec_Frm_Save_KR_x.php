...
function ExecuteRC($frUPB,$RfA,$RfU,$TbK,$tID,$UID)
{
	...
	while ($mRo = mysql_fetch_array($Rs, MYSQL_BOTH))
	{
		...
		$CekR = fGlobal("IDT","ta_kib_g","Ref_Usulan:Ref_Mutasi",$RfU.":".$RfA,"=:=","","");
		if ($CekR=="")
		{
			if ($TbK=="a")
			{
				...
				if ($RsG){
					PostingKib_Mutasi($frUPB,$RfA,$RfU,$NewRefKIB,$TglMts,$gRin,$TbK,$tID,$UID);
				}
			}
...
function PostingKib_Mutasi($frUPB,$RfA,$RfU,$NewRefKIB,$TglMts,$gRin,$TbK,$tID,$UID)
{

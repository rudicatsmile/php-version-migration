...
<table align="center" cellpadding="0" class="table-listpop" cellspacing="0" width="100%" height="240" border="0">
	<tr height="18" onclick="showCLICK('upb','00.00.00.00.00.000','ALL','<?=$IdL?>'); return false;">
		<td>&nbsp;</td>
	...
	<?
	$iG=1;
	$nSQ = "SELECT Kd_UPB, Nm_UPB FROM ref_upb WHERE Kd_UPB LIKE '%' $CrT ORDER BY Kd_UPB";
	#echo $nSQ;
	$nRs = mysql_query($nSQ);
	while ($mRo = mysql_fetch_array($nRs, MYSQL_BOTH))
	{
		...
		$gNm = $mRo[1];
		?>
		<tr height="18" onclick="showCLICK('upb_cr','<?=$gKd?>','<?=$gNm?>','<?=$IdL?>'); return false;">
			<td valign="top" width="10" style="border-bottom:1px dotted #CCCCCC">&nbsp;</td>
			<td valign="top" width="115" style="border-bottom:1px dotted #CCCCCC"><?=$gKd?></td>

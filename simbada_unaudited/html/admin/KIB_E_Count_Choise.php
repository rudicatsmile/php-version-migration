...
<table class='table' border='0' width='526' cellspacing='1' style='font-family: Calibri; font-size: 10pt; font-weight: bold; border-collapse: collapse'>
  ...
  <?
		...
		for ($i=0; $i<=$gMax; $i=$i+5000)
		{
			...
			<td>&nbsp;</td>
			<td><?=$gNO?>.</td>
			<td><a href='#' onClick="P_OpenDoc('800','400','<?=$ReC?>','<?=$TtD?>','<?=$HeA?>','<?=$LoadEND?>'); return false"><? echo "Dokumen KIB-E &nbsp;&nbsp;&nbsp;( Record &nbsp;&nbsp; => &nbsp;&nbsp; ".fConvertToRupiahBulat($gA)." &nbsp; s/d &nbsp; ".fConvertToRupiahBulat($gB)." )"?></a></td>
			</tr>
			<?
		}
		?>

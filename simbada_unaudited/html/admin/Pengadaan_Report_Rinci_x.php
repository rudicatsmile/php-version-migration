		...
		<tr>
          ...
          <?
				...
				while ($mRo = mysql_fetch_array($nRs, MYSQL_BOTH))
				{
						...
						$Col[3] = round($mRo[2]);
						$Col[4] = round(sumPengadaanPROG($gUnt,$KdP,$gThA,$DatabaseSB,$ConSB));
				...
				function vwKEGI($gUnt,$KdP,$gThA,$DatabaseSB,$ConSB)
				{
					...
					WHERE Kd_Unit='".$gUnt."' AND Kd_Kegiatan LIKE '".$KdP."%' AND Periode='".$gThA."' GROUP BY Kd_Kegiatan";
					...
					while ($mRoA = mysql_fetch_array($nRsA, MYSQL_BOTH))
					{
						...
						$Col[3] = round($mRoA[2]);
						$Col[4] = round(sumPengadaanKEGI($gUnt,$KdK,$gThA,$DatabaseSB,$ConSB,""));
				...
				function vwSUBK($gUnt,$KdK,$gThA,$DatabaseSB,$ConSB)
				{
					$iGe=1;
					$nSQE = "SELECT Kd_SubKegiatan, Nm_SubKegiatan, IfNull(sum(Nilai),0) as Nilai 
					FROM ta_pengadaan 
					WHERE Kd_Unit='".$gUnt."' AND Kd_SubKegiatan LIKE '".$KdK."%' AND Periode='".$gThA."' GROUP BY Kd_SubKegiatan";
					...
					while ($mRoE = mysql_fetch_array($nRsE, MYSQL_BOTH))
					{
						...
						$Col[3] = round($mRoE[2]);
						$Col[4] = round(sumPengadaanSUBK($gUnt,$KdS,$gThA,$DatabaseSB,$ConSB,""));
						$Col[5] = round($Col[3],2)-round($Col[4],2);
						if ($Col[3]>0 || $Col[4]>0){
				...
				function vwBELA($gUnt,$KdS,$gThA,$DatabaseSB,$ConSB)
				{
					...
					WHERE Kd_Unit='".$gUnt."' AND Kd_SubKegiatan = '".$KdS."' AND Periode='".$gThA."' GROUP BY Kd_Rek13";
				...
				function sumPengadaanSUBK($gUnt,$KdS,$gThA,$DatabaseSB,$ConSB,$Lod)
				{
					$LoadNIL = 0;
					$nSQP = "SELECT Nomor FROM ta_pengadaan WHERE Kd_Unit='".$gUnt."' AND Kd_SubKegiatan = '".$KdS."' AND Periode='".$gThA."' ORDER BY Nomor";
					if ($Lod){echo $nSQP."<br>";}
					$nRsP = mysql_query($nSQP);
					while ($mRoP = mysql_fetch_array($nRsP, MYSQL_BOTH))
					{

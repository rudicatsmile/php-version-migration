  ...
  <table border="0" align="center" style="width:99%">
    ...
    <tr> 
	  ...
	  <select class="boxs" name="fBid" tabindex="0" style="width: 480px" onchange="P_Change()">
		...
		$nSQ = "SELECT Kd_Aset, Nm_Aset FROM ref_rek_aset108_4 WHERE Kd_Aset LIKE '1.5.4.__' ORDER BY Kd_Aset";
		$nRs = mysql_query($nSQ) or die(mysql_error());
		$mRo = mysql_fetch_assoc($nRs);
		$tRo = mysql_num_rows($nRs);
		if ($tRo > 0)
  ...
  <table border="0" align="center" id="tbViewData" style="width:99%">
    <tr> 
	  ...
	  <table align="center" border="0" class="table-list" cellspacing="0" cellpadding="0" style="width:99%">
		...
		if ($gFin!="")
		{$fFindSy = "AND Kd_Aset_108 LIKE '1.5.4.%' AND (Ref_Usulan LIKE '%".$gFin."%' OR No_Register LIKE '%".$gFin."%' OR Pencatat LIKE '%".$gFin."%' OR No_Pengadaan LIKE '%".$gFin."%' OR Kd_Aset_108 LIKE '%".$gFin."%' OR Ref_Temp LIKE '%".$gFin."%' OR Nm_Aset LIKE '%".$gFin."%' OR Harga LIKE '%".$gFin."%' OR Referensi LIKE '%".$gFin."%' OR Ref_History LIKE '%".$gFin."%' OR Nomor_Polisi LIKE '%".$gFin."%' OR Nomor_Rangka LIKE '%".$gFin."%' OR Nomor_BPKB LIKE '%".$gFin."%' OR Nomor_Mesin LIKE '%".$gFin."%' OR Keterangan LIKE '%".$gFin."%' OR Merk LIKE '%".$gFin."%')";}
		else
		{$fFindSy = "AND Kd_Aset_108 LIKE '1.5.4.%'";}
		
		/*
		$nSQA= "SELECT kd_aset FROM ta_kib_108 WHERE referensi LIKE 'KDL%' AND kd_aset_108 ='' GROUP BY kd_aset";
		#echo $nSQA."<br>";
		...
		while ($mRoA = mysql_fetch_array($nRsA, MYSQL_BOTH))
		{
			...
			#echo $mRoA[0]." : ".$x08A."<br>";
			if ($x08A){
				$eU = "UPDATE ta_kib_108 SET kd_aset_108='".$x08A."' WHERE Kd_Aset='".$mRoA[0]."' AND Referensi LIKE 'KDL%'";
				echo $eU."<br>";
				$nR = mysql_query($eU);
				
				$eU = "UPDATE ta_kib_post_108 SET kd_aset_108='".$x08A."' WHERE Kd_Aset='".$mRoA[0]."' AND Referensi LIKE 'KDL%'";
				echo $eU."<br>";
				$nR = mysql_query($eU);
			}
		}
		...
		*/
		...
		do
			...
			if ($gNIa==0){
				#AmbilPostingYgTertinggal($mRo['Referensi'],substr($mRo['Kd_UPB'],0,11),DatabaseSB,$ConSB,"");
			}
...
<script language="javascript">
			...
			if (win!=null)
    			{
      				win.window.document.open()       			
					<?
						$URL_Top = "Find_Acc_Top.php?CrAcc=KIB-G&FrmG=".$_GET['FrmG']."&gUnt=".$zUnt."&gSub=".$zSub."&gUpb=".$zUpb."&IdL=".$_GET['IdL'];
						$URL_Mid = "Find_Acc_Mid.php?CrAcc=KIB-G&FrmG=".$_GET['FrmG']."&gUnt=".$zUnt."&gSub=".$zSub."&gUpb=".$zUpb."&IdL=".$_GET['IdL'];
	...
	function P_OpenDoc(w,h,IdL)
	{
		...
		LeftPosition=(screen.width)?(screen.width-w)/2:100; 
		TopPosition=(screen.height)?(screen.height-h)/2:100;
		URL= 'KIB_G_Dokumen.php?CriT=BPK&gUnt='+gUnt+'&gSub='+gSub+'&gUpb='+gUpb+'&gBid='+gBid+'&gKel='+gKel+'&gOBJ='+gOBJ+'&gRin='+gRin+'&gExt='+gExt+'&gFin='+gFin+'&gThn='+gThn+'&gThnA='+gThnA+'&IdL='+IdL;
		settings='width='+w+',height='+h+',top='+TopPosition+',left='+LeftPosition+',location=no,directories=no,status=no,menubar=yes,toolbar=yes,resizable=ya,maximize=yes,scrollbars=yes,navigation=no';
		window.open(URL,'',settings);
	}
</script>
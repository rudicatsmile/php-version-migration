...
<table border="0" width="800" align="center" cellspacing="0" cellpadding="0" style="border:0px solid #000; border-collapse: collapse; font-family:Calibri; font-size:10pt">
  ...
  <tr style="text-align:center">
	...
	{ 
		$fs = "";
		$x0 = "";
		$eRa = "1.3.5.02.02.02.012";
		$eRb = "1.3.5.02.02.02.014";
		$x1 = substr($nR,0,3);
		$x2 = $mRb[2];
		
		$x3 = fGlobal("count(*)","ta_kib_108","Kd_UPB:Kd_Aset_108:Tgl_Perolehan:extracom:KdpToAset",$KdS."%:".$eRa.":".$TGz.":".$RdB.":N","LIKE:=:<:LIKE:=","",$fs);
		$x3 = $x3 + fGlobal("count(*)","ta_kib_108","Kd_UPB:Kd_Aset_108:Tgl_Perolehan:extracom:KdpToAset",$KdS."%:".$eRb.":".$TGz.":".$RdB.":N","LIKE:=:<:LIKE:=","",$fs);
		
		$x4 = fGlobal("ifnull(sum(Debet),0)","ta_kib_post_108","Kd_UPB:Kd_Aset_108:Tanggal:extracom:KdpToAset",$KdS."%:".$eRa.":".$TGz.":".$RdB.":N","LIKE:=:<:LIKE:=","",$fs);
		$x4 = $x4 + fGlobal("ifnull(sum(Debet),0)","ta_kib_post_108","Kd_UPB:Kd_Aset_108:Tanggal:extracom:KdpToAset",$KdS."%:".$eRb.":".$TGz.":".$RdB.":N","LIKE:=:<:LIKE:=","",$fs);
	...
	function Level3($nR,$Mst,$KdS,$TG,$TGx,$TGz,$rSem,$rThn,$yThn,$RdB,$DatabaseSB,$ConSB)
	{
		...
		Kd_Aset as A1, 
		Nm_Aset as A2 
		FROM ref_rek_aset108_3 WHERE Kd_Aset LIKE '".substr($nR,0,5)."' ORDER BY Kd_Aset";
		$nRc = mysql_query($SQc);
		#echo $SQc."<br>";
	...
	function Level4($Mst,$KdS,$TG,$TGx,$TGz,$rSem,$rThn,$yThn,$RdB,$DatabaseSB,$ConSB)
	{
		...
		Kd_Aset as A1, 
		Nm_Aset as A2 
		FROM ref_rek_aset108_4 WHERE Kd_Aset LIKE '".$Mst.".02' ORDER BY Kd_Aset";
		#echo $SQd."<br>";
		$nRd = mysql_query($SQd);
	...
	function Level5($Mst,$KdS,$TG,$TGx,$TGz,$rSem,$rThn,$yThn,$RdB,$DatabaseSB,$ConSB)
	{
		...
		Kd_Aset as A1, 
		Nm_Aset as A2 
		FROM ref_rek_aset108_5 WHERE Kd_Aset LIKE '".$Mst.".02' ORDER BY Kd_Aset";
		#echo $SQe."<br>";
		$nRe = mysql_query($SQe);
	...
	function Level6($Mst,$KdS,$TG,$TGx,$TGz,$rSem,$rThn,$yThn,$RdB,$DatabaseSB,$ConSB)
	{
		...
		Kd_Aset as A1, 
		Nm_Aset as A2 
		FROM ref_rek_aset108_6 WHERE Kd_Aset LIKE '".$Mst.".02' ORDER BY Kd_Aset";
		#echo $SQf."<br>";
		$nRf = mysql_query($SQf);
		...
		{ 
			...
			if ($iGe>1) {EmptRow();}
			TempRow($x0,$x1,$x2,$x3,$x4,$xB);
			Level7($x1,$KdS,$TG,$TGx,$TGz,$rSem,$rThn,$yThn,$RdB,$DatabaseSB,$ConSB);
			#TempRow($x0,$x1,$x2,$x3,$x4,$xB);
			$iGf++;
		}
	}
	
	function Level7($Mst,$KdS,$TG,$TGx,$TGz,$rSem,$rThn,$yThn,$RdB,$DatabaseSB,$ConSB)
	{
		$iGg = 1;
		$SQg = "SELECT 
		'' as A0,
		Kd_Aset as A1, 
		Nm_Aset as A2 
		FROM ref_rek_aset108_7 WHERE (Kd_Aset = '".$Mst.".012' OR Kd_Aset = '".$Mst.".014') ORDER BY Kd_Aset";
		#echo $SQf."<br>";
		$nRg = mysql_query($SQg);
		while ($mRg = mysql_fetch_array($nRg)) 
		{ 
			$fs = "";
			$x0 = "";
			$eRa = "1.3.5.02.02.02.012";

...
<tr>
  <td colspan="3" style="font-size:12pt; font-weight:bold; border-bottom:3px double #000000">BERDASARKAN PERMENDAGRI NOMOR 90 TAHUN 2019</td>
</tr>
<tr height="10">
  <td colspan="3"></td>
</tr>
...
<tr>
  ...
  <td width="20">:</td>
  <td><?=substr($eKD,0,1)." : ".strtoupper(fGlobal("nm_rek","ref_rek_90_1","kd_rek",substr($eKD,0,1),"=","",""))?></td>
</tr>
<? } ?>
<? if ($Lev>1){?>
<tr>
  <td>KELOMPOK</td>
  <td>:</td>
  <td><?=substr($eKD,0,3)." : ".strtoupper(fGlobal("nm_rek","ref_rek_90_2","kd_rek",substr($eKD,0,3),"=","",""))?></td>
</tr>
<? } ?>
<? if ($Lev>2){?>
<tr>
  <td>JENIS</td>
  <td>:</td>
  <td><?=substr($eKD,0,6)." : ".strtoupper(fGlobal("nm_rek","ref_rek_90_3","kd_rek",substr($eKD,0,6),"=","",""))?></td>
</tr>
<? } ?>
<? if ($Lev>3){?>
<tr>
  <td>OBJEK</td>
  <td>:</td>
  <td><?=substr($eKD,0,9)." : ".strtoupper(fGlobal("nm_rek","ref_rek_90_4","kd_rek",substr($eKD,0,9),"=","",""))?></td>
</tr>
<? } ?>
<? if ($Lev>4){?>
<tr>
  <td>RINCIAN OBJEK</td>
  <td>:</td>
  <td><?=substr($eKD,0,12)." : ".strtoupper(fGlobal("nm_rek","ref_rek_90_5","kd_rek",substr($eKD,0,12),"=","",""))?></td>
</tr>
<? } ?>
<tr height="10">
  <td colspan="3"></td>
...
if ($Lev=='0')
{
	$iA=1;
	if ($eKD!=""){
		$nSQ = "SELECT kd_rek, nm_rek FROM ref_rek_90_1 WHERE kd_rek LIKE '".$eKD."%' ORDER BY kd_rek";
	}
	else{
		$nSQ = "SELECT kd_rek, nm_rek FROM ref_rek_90_1 ORDER BY kd_rek";
	}
	#echo $nSQ;
	$nRs = mysql_query($nSQ);
...
{
	$iB=1;
	$nSQa = "SELECT kd_rek, nm_rek FROM ref_rek_90_2 WHERE kd_rek LIKE '".$Kd."%' ORDER BY kd_rek";
	if ($fS) echo $nSQa."<br>";
...
{
	$iC=1;
	$nSQb = "SELECT kd_rek, nm_rek FROM ref_rek_90_3 WHERE kd_rek LIKE '".$Kd."%' ORDER BY kd_rek";
	if ($fS) echo $nSQb."<br>";
...
{
	$iD=1;
	$nSQc = "SELECT kd_rek, nm_rek FROM ref_rek_90_4 WHERE kd_rek LIKE '".$Kd."%' ORDER BY kd_rek";
	if ($fS) echo $nSQc."<br>";
...
{
	$iE=1;
	$nSQd = "SELECT kd_rek, nm_rek FROM ref_rek_90_5 WHERE kd_rek LIKE '".$Kd."%' ORDER BY kd_rek";
	if ($fS) echo $nSQd."<br>";
...
{
	$nSQe = "SELECT kd_rek, nm_rek FROM ref_rek_90_6 WHERE kd_rek LIKE '".$Kd."%' ORDER BY kd_rek";
	if ($fS) echo $nSQe."<br>";

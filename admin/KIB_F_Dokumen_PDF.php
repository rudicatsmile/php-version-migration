...
class PDF extends FPDF
{
	...
	{	
		...
		$this->Cell(1);
		$this->Cell(0,12,'KARTU INVENTARIS BARANG (KIB-F)',0,0,'C');	
		$this->Ln(5);
		
		$this->Cell(1);
		$this->Cell(0,12,'KONSTRUKSI DALAM PENGERJAAN',0,0,'C');	
		$this->Ln(8);
		...
		$R2=array(8,52,16,16,13,15,55,15,25,20,30,20,20,28,63);
		$Hr=9;
		...
		$this->Cell($R2[3],$Hr,'','TL',0,'C',true);
		$this->Cell($R2[4],$Hr,'','TR',0,'L',true);
		$this->Cell($R2[5],$Hr,'','TLR',0,'C',true);
		$this->Cell($R2[6],$Hr,'','TLR',0,'C',true);
		$this->Cell($R2[7],$Hr,'','TL',0,'C',true);
		$this->Cell($R2[8],$Hr,'','TR',0,'C',true);
		$this->Cell($R2[9],$Hr,'','TLR',0,'C',true);
		$this->Cell($R2[10],$Hr,'','TLR',0,'C',true);
		...
		$this->Cell($R2[2],$Hr,'','LRB',0,'C',true);
		$this->Cell($R2[3],$Hr,'','TLRB',0,'C',true);
		$this->Cell($R2[4],$Hr,'','TLRB',0,'L',true);
		$this->Cell($R2[5],$Hr,'','LRB',0,'C',true);
		$this->Cell($R2[6],$Hr,'','LRB',0,'C',true);
		$this->Cell($R2[7],$Hr,'','TLRB',0,'C',true);
		$this->Cell($R2[8],$Hr,'','TLRB',0,'C',true);
		$this->Cell($R2[9],$Hr,'','LRB',0,'C',true);
		$this->Cell($R2[10],$Hr,'','LRB',0,'C',true);
		...
		$this->Text(72, 56, 'Bangunan'); $this->Text(73, 60, '(P,SPD)');
		$this->Text(88, 53, 'Konstruksi Bangunan');
		$this->Text(88, 60.5, 'Bertingkat /'); $this->Text(91, 64.5, 'Tidak');
		$this->Text(105, 60.5, 'Beton /'); $this->Text(105, 64.5, 'Tidak');

		$this->Text(117, 58, 'Luas (M2)');
		$this->Text(144, 58, 'Letak / Lokasi / Alamat');
		
		$this->Text(200, 53, 'Dokumen');
		$this->Text(188, 62, 'Tanggal');
		$this->Text(210, 62, 'Nomor');
		
		$this->Text(231, 54, 'Tanggal,'); $this->Text(228, 58, 'Bulan,Tahun'); $this->Text(232, 62, 'Mulai');
		$this->Text(252, 58, 'Status Tanah');
		
		$this->Text(278, 56, 'Nomor Kode'); $this->Text(281, 60, 'Tanah');
		
		$this->Text(300, 56, 'Asal-Usul'); $this->Text(298, 60, 'Pembiayaan');
		
		$this->Text(322, 56, 'Nilai Kontrak'); $this->Text(327, 60, '(Rp)');
		$this->Text(370, 58, 'Keterangan');
	...
	{
		$iG=1; $tJml=0;
		$R3=array(8,52,16,16,13,15,55,15,25,20,30,20,20,28,63);
		$this->SetFillColor(255,255,255);
		$this->SetFont('arial','',7);
		$SQL = "SELECT * FROM ta_kib_f WHERE Kd_UPB='".$gUpb."' AND Tgl_Perolehan LIKE '".$gThn."-__-__' AND Kd_Pemilik LIKE '".$gMLK."' AND Status='' ORDER BY Tgl_Perolehan, Kd_Aset, No_Register";
		...
		if ($tRo > 0)
		{
			do
			{
				...
				$this->Cell($R3[2],5,$mRo['Tipe_Bangunan'],'TLRB',0,'C',true);
				$this->Cell($R3[3],5,$mRo['Bertingkat'],'TLRB',0,'C',true);
				$this->Cell($R3[4],5,$mRo['Beton'],'TLRB',0,'L',true);
				if ((int)$mRo['Luas']>0) {$Luas=fConvertToRupiahBulat($mRo['Luas']);} else {$Luas="";}
				$this->Cell($R3[5],5,$Luas,'TLRB',0,'C',true);
				$this->Cell($R3[6],5,ucfirst(strtolower($mRo['Lokasi'])),'TLRB',0,'L',true);
				if ($mRo['Dokumen_Tanggal']!="" && substr($mRo['Dokumen_Tanggal'],0,4)!="0000") {$TgL=fConvertDateShort($mRo['Dokumen_Tanggal']);} else {$TgL="";}
				$this->Cell($R3[7],5,$TgL,'TLRB',0,'L',true);
				$this->Cell($R3[8],5,$mRo['Dokumen_Nomor'],'TLRB',0,'L',true);
				if ($mRo['Tgl_Mulai']!="" && substr($mRo['Tgl_Mulai'],0,4)!="0000") {$TgM=fConvertDateShort($mRo['Tgl_Mulai']);} else {$TgM="";}
				$this->Cell($R3[9],5,$TgM,'TLRB',0,'C',true);

				$this->Cell($R3[10],5,$mRo['Status_Tanah'],'TLRB',0,'L',true);
				$this->Cell($R3[11],5,$mRo['Kode_Tanah'],'TLRB',0,'L',true);
				$this->Cell($R3[12],5,$mRo['Asal_Usul'],'TLRB',0,'C',true);
				...
				$this->Cell($R3[13],5,fConvertToRupiah($gAKH),'TLRB',0,'R',true);
				$this->Cell($R3[14],5,ucfirst(strtolower(substr($mRo['Keterangan'],0,52))),'TLRB',0,'L',true);
				$this->Ln() ;
			}

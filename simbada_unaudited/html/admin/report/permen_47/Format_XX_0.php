...
{
	$mR = "ASET TETAP ".fGlobal("Nm_Aset","ref_rek_aset108_3","Kd_Aset",$nR,"=","","");
}
...
<table border="0" width="800" align="center" cellspacing="0" cellpadding="0" style="border:0px solid #000; border-collapse: collapse; font-family:Calibri; font-size:10pt">
  <tr height="25" style="font-weight:bold; text-align:center">
    ...
    <td rowspan="2" style="border:1px solid #000">Jumlah <br>Barang</td>
  ...
  <tr style="text-align:center">
	...
	{ 
		...
		if ($nR=="_._._")
		{
			...
			#$x4 = $x4 - fGlobal("ifnull(sum(Koreksi_Akumulasi),0)","ta_kib_post_penyusutan_bulanan_108","Kd_UPB:Kd_Aset_108:Tahun:Nilai_Akhir:Triwulan:extracom",$KdS."%:".$x1.".%:".$yThn.":Y:IV:".$RdB,"LIKE:LIKE:=:=:=:LIKE","","");
		}
		else
		{
			$x1 = substr($nR,0,3);
			...
			#$x4 = $x4 - fGlobal("ifnull(sum(Koreksi_Akumulasi),0)","ta_kib_post_penyusutan_bulanan_108","Kd_UPB:Kd_Aset_108:Tahun:Nilai_Akhir:Triwulan:extracom",$KdS."%:".$nR.".%:".$yThn.":Y:IV:".$RdB,"LIKE:LIKE:=:=:=:LIKE","","");
		}
	...
	function Level3($nR,$Mst,$KdS,$TG,$TGx,$TGz,$rSem,$rThn,$yThn,$RdB,$DatabaseSB,$ConSB)
	{
		...
		if ($nR=="_._._")
		{
			...
			$nRc = mysql_query($SQc);
			#FROM ref_rek_aset108_3 WHERE Kd_Aset LIKE '".$Mst."%' AND Kd_Aset <> '1.3.7' AND Kd_Aset <> '1.5.5' AND Kd_Aset <> '1.5.6' ORDER BY Kd_Aset";
		}
		else
		{
			$SQc = "SELECT 
			...
			$nRc = mysql_query($SQc);
			#FROM ref_rek_aset108_3 WHERE Kd_Aset LIKE '".$nR."' AND Kd_Aset <> '1.3.7' AND Kd_Aset <> '1.5.5' AND Kd_Aset <> '1.5.6' ORDER BY Kd_Aset";
		}
		#echo $SQc."<br>";
		...
		{ 
			...
			{
				$x4 = 0;#fGlobal("ifnull(sum(Koreksi_Akumulasi),0)","ta_kib_post_penyusutan_bulanan_108","Kd_UPB:Kd_Aset_108:Tahun:Nilai_Akhir:Triwulan:extracom",$KdS."%:".$x1.".%:".$yThn.":Y:IV:".$RdB,"LIKE:LIKE:=:=:=:LIKE","","fs");
			}
			else
			{
				$x4 = fGlobal("ifnull(sum(Debet),0)","ta_kib_post_108","Kd_UPB:Kd_Aset_108:Tanggal:extracom:KdpToAset",$KdS."%:".$x1.".%:".$TGz.":".$RdB.":N","LIKE:LIKE:<:LIKE:=","",$fs);
				#$x4 = $x4 - fGlobal("ifnull(sum(Koreksi_Akumulasi),0)","ta_kib_post_penyusutan_bulanan_108","Kd_UPB:Kd_Aset_108:Tahun:Nilai_Akhir:Triwulan:extracom",$KdS."%:".$x1.".%:".$yThn.":Y:IV:".$RdB,"LIKE:LIKE:=:=:=:LIKE","",$fs);
			}
			$xB = "bold";
			if ($iGc>1) {EmptRow();}
			TempRow($x0,$x1,$x2,$x3,$x4,$xB);

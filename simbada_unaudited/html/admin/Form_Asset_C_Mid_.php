...
$gKdT   = $fKodeTnh;
$gLti   = fConvertToNumeric($fLuasLantai);
$gLTn   = fConvertToNumeric($fLuasTanah);
$gSTT   = $fStatusTnh;
$gLKS   = mysql_real_escape_string($fAlamat);
...
if ($Smp=="Save")
	{
			...
			if ($gGRP!="")
			{
				if ($gChoiceGRP=="ON")		//PILIHAN EDIT GROUP ATAU TIDAK
				{
					...
					else if ($gSTN > $gSTN_Old)		//JIKA JUMLAH ITEM YANG BARU LEBIH BESAR MAKA TAMBAH RECORD BARU
					{
						...
						//CARI REGISTER TERAKHIR BARANG DI TABEL KIB
						$nSQL = "SELECT IFNULL(MAX(No_Register),0) AS LasReG FROM ta_kib_108 WHERE referensi LIKE 'BNG%' AND no_register NOT LIKE '%HEX%'";
		...
		else
		{
			...
			if ($gSTN > 1)		//MODUL ENTRY GROUP
			{
				...
				$NewG = fGlobal("max(no_register)","ta_kib_108","referensi:no_register NOT","BNG%:%HEX%","LIKE:LIKE","","");
				$LastReGrp = ((int)$NewG) + 1;
			...
			{
				//CARI REGISTER RERAKHIR BARANG DI TABEL KIB
				$NewReGAset = MakeNewNoRegiterAuto('BNG','');
...
$URL="Form_Asset_C_Mid.php?rIDT=".$rIDT."&IdL=".$IdL."&gUnt=".$gUnt."&gSub=".$gSub."&gUpb=".$gUpb."&gThn=".$gThn."&gBid=".$gBid."&gKel=".$gKel."&gOBJ=".$gOBJ."&gRin=".$gRin."&gFin=".$gFin;
header("Location: ".$URL);
?>

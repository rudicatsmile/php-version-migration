...
if ($gThnD!="" && $gBlnD!="" && $gHriD!="") {
	$gTglD= $gThnD."-".$gBlnD."-".$gHriD;}
else {
	$gTglD= "0000-00-00";}

if ($gThnM!="" && $gBlnM!="" && $gHriM!="") {
...
if ($Smp=="Save")
	{
			...
			if ($gGRP!="")
			{
				if ($gChoiceGRP=="ON")		//PILIHAN EDIT GROUP ATAU TIDAK
				{
					...
					else if ($gSTN > $gSTN_Old)		//JIKA JUMLAH ITEM YANG BARU LEBIH BESAT MAKA TAMBAH RECORD BARU
					{
						...
						$NewG = fGlobal("max(no_register)","ta_kib_108","referensi:no_register NOT","KDP%:%HEX%","LIKE:LIKE","","");
		...
		else
		{
			...
			{
				//CARI REGISTER RERAKHIR BARANG DI TABEL KIB
				$NewReGAset = MakeNewNoRegiterAuto('KDP','');
...
$URL="Form_Asset_F_Mid.php?rIDT=".$rIDT."&IdL=".$IdL."&gUnt=".$gUnt."&gSub=".$gSub."&gUpb=".$gUpb."&gThn=".$gThn."&gBid=".$gBid."&gKel=".$gKel."&gOBJ=".$gOBJ."&gRin=".$gRin."&gFin=".$gFin;
header("Location: ".$URL);
?>

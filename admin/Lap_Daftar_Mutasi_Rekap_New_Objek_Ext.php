<?php
	function ViewObjek($KdOBJ,$mUpb,$mThn,$nThn,$mG,$LoadMutasi,$LoadExtrac,$rPil)  #****#
	{
		$iGD  = 1;
		...
		while ($mRoD = mysql_fetch_array($nRsD, MYSQL_BOTH))
		{
			...
			#}
				...
				if ($nThn<=2026) {
					...
					$Col[5] = $Col[5] + fGlobal("IfNull(count(*),0)", "ta_kib_108"."_mutasi","Kd_Aset_108:Tgl_Perolehan:Tgl_Mutasi:Tgl_Mutasi:Kd_UPB:extracom",$KdAsT."%:".$mThn."-12-31:".$nThn."-01-01:".($nThn+1)."-12-31:".$mUpb."%:".$LoadExtrac,"LIKE:<=:>=:<=:LIKE:LIKE","","");
			...
			if ($tView!=0) {
				...
				if ($rPil==4) {ViewRciObjek($KdAsT,$mUpb,$mThn,$nThn,$mG,$LoadMutasi,$LoadExtrac);}
			}
			$iGD++;
		}
	}

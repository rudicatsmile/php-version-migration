...
require "Form_Asset_A_Mid_Temp_Posting.php";
...
if ($Smp=="Save")
{
	...
	while ($mRo = mysql_fetch_array($nRs, MYSQL_BOTH))
	{
		...
		$gBRK = $mRo[1];
		$gTgL = fGlobalNEW("Tg_Berita_Acara","ta_pengadaan","Nomor",$gBRK,"=","",DatabaseSB,$ConSB,"");
		$gNoM = $mRo[1];
		$gRin = $mRo[2];
		$gNiL = $mRo[3];
		$gPRO = $mRo[4];
	...
	if ($rIDT)
	{
		$rSTN = fGlobalNEW("Jumlah_Bidang","ta_kib_108_temp","IDT",$rIDT,"=","",DatabaseSB,$ConSB,"");
		...
		Kd_UPB='$gUpb',
		Nm_Aset_108='".mysql_real_escape_string($gNma)."',
		Luas_M2='$gLua',
		Alamat='$gAlm',
		Hak_Tanah='$gHak',
		Penggunaan='$gGna',
		...
		$ReF = fGlobalNEW("referensi","ta_kib_108_temp","IDT",$rIDT,"=","",DatabaseSB,$ConSB,"");
		$SQ = "UPDATE ta_kib_108 SET Nm_Aset='".mysql_real_escape_string($gNma)."' WHERE ref_temp='".$ReF."'";
		mysql_query($SQ);
		
		if (($gSTN != $rSTN) || ($gUpb != $rUpb)) 
		{
			DeleteData("ta_kib_108",$rIDT,$gNoM,$RfTMP,DatabaseSB,$ConSB);
		}	//'''''''''''''''''''
		else
		{
			//''''''''''''''''''''''''' {Harga => Ref_Temp}
	...
	else
	{
		...
		$rIDT = fGlobal("Max(IDT)","ta_kib_108_temp","No_Pengadaan",$gNoM,"=","","");		//'''''''''''''''''
	}
...
$URL="Form_Asset_A_Mid_Temp.php?".$urI."gIdT=".$gIdT."&rIDT=".$rIDT."&IdL=".$IdL;
header("Location: ".$URL);
?>
...
#Write Title
$worksheet->write(0, 0, "KIB-A ( ASET TANAH )", $Merg1);
...
$tNIL= 0;
$iG  = 11;
$nSQL= "SELECT * FROM ta_kib_a WHERE Kd_UPB LIKE '".$gUpb."' AND Kd_Aset LIKE '".$gAss."' AND (Tgl_Perolehan BETWEEN '".$gTgA."' AND '".$gTgB."') ".$fFindSy." ORDER BY Tgl_Perolehan, Kd_Aset, No_Register";
...
do
	{
		...
		#Write Body
		$worksheet->write($iG, 0, $iG-$iA, $r_cent);
...
header("Content-Type: application/x-msexcel; name=\"KiB-A Choise.xls\"");
header("Content-Disposition: inline; filename=\"KiB-A Choise.xls\"");
$fh=fopen($fname, "rb");
fpassthru($fh);
unlink($fname);
?>
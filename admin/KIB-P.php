...
<body>
<form name="myfrm" method="post" action="<?php echo "KIB-P_.php?FrmG=".$_GET['FrmG']."&IdL=".$_GET['IdL'] ?>">
  ...
  <table border="0" align="center" style="width:99%">
    ...
    <tr> 
      ...
      <td> <select class="boxs" name="fBid" tabindex="0" style="width: 480px" onchange="P_Change()">
          ...
          <?php
		$nSQ = "SELECT Kd_Aset, Nm_Aset FROM ref_rek_aset108_4 WHERE Kd_Aset LIKE '1.1.12.__' ORDER BY Kd_Aset";
		$nRs = mysql_query($nSQ) or die(mysql_error());
		$mRo = mysql_fetch_assoc($nRs);
		$tRo = mysql_num_rows($nRs);
		if ($tRo > 0)
      ...
      <td>&nbsp;</td>
      <td align="right"><input type="hidden" name="B35" value="INPUT DATA" onclick="InputData('950','520','center')" style="width: 90px; height: 24px; padding-left: 0px; padding-right: 0px; padding-top: 0px; padding-bottom: 0px" /></td>
    </tr>
    <tr> 
      ...
      <td> <select class="boxs" name="fKel" tabindex="0" style="width: 480px" onchange="this.form.submit()">
		...
		if ($tRo > 0)
		{
			if ($gKel!="All")
            {
                ...
                if (substr($gKel,0,9)!=$gBid) {$gKel=$mRo['Kd_Aset'];}
            }
			do
			{
				$sel ="";
    ...
    <tr> 
      ...
      <td> <select class="boxs" name="fOBJ" tabindex="0" style="width: 480px" onchange="this.form.submit()">
		...
		if ($tRo > 0)
		{
			...
			if (substr($gOBJ,0,12)!=$gKel) {$gOBJ=$mRo['Kd_Aset'];}
            }
			do
			{
				$sel ="";
      ...
      <td>&nbsp;</td>
      <td align="right"><input type="hidden" name="B37" disabled value="EXPORT DATA" onclick="P_ToExcel('800','400','center','ToExcel/KIB_P_ToExcel')" style="width: 90px; height: 24px; padding-left: 0px; padding-right: 0px; padding-top: 0px; padding-bottom: 0px" /></td>
    </tr>
    <tr> 
      <td>TAHUN</td>
      <td valign="middle"> <table border="0" width="100%" cellpadding="0" style="border-collapse: collapse">
          <tr> 
            ...
            <td width="9%">CARI</td>
            <td width="36%"> <input class="text" type="text" name="fFind" id="fFind" value="<?php echo $gFin?>" style="font-family: Calibri; font-size: 10pt; border: 1px solid #C0C0C0; padding-left: 4px; padding-right: 4px; padding-top: 1px; padding-bottom: 1px; background-color: #E1F986" /></td>
            <td width="2%">&nbsp;</td>
			...
			<input type="button" name="B392" value="Import" disabled onclick="P_Import(); return false;" style="width: 55px; height: 21px; padding-left: 0px; padding-right: 0px; padding-top: 0px; padding-bottom: 0px" />
			<?php } ?>			</td>
          </tr>
        </table></td>
      <td>SUB-SUB ROBJ</td>
      ...
      <td width="100" align="right"> 
        <input type="hidden" name="B38" value="CARI REKENING" onclick="OpenAccNumb('600','500','center')" style="width: 90px; height: 24px; padding-left: 0px; padding-right: 0px; padding-top: 0px; padding-bottom: 0px" /></td>
    </tr>
    <tr> 
      <td>&nbsp;</td>
      <td>&nbsp;</td>
  ...
  <table border="0" align="center" id="tbViewData" style="width:99%">
    ...
    <table align="center" border="0" class="table-list" cellspacing="0" cellpadding="0" style="width:99%">
      <tr> 
        <th width="29" class="ac">No</th>
        <th width="100" class="ac">Referensi</th>
        <th width="59" class="ac">Register</th>
        <th class="ac">Nama Barang</th>
        <th width="278" class="ac">Uraian</th>
        <th width="87" class="ac">Perolehan</th>
        <th width="90" class="ac">Tgl.Mutasi</th>
        <th width="97" class="ac">Nilai Perolehan </th>
        <th width="101" class="ac">Nilai Akhir </th>
        <th width="220" class="ac">Actions</th>
      </tr>
      <?php
		if ($eMuT!=""){
			$tMuTZ="_mutasi";
		...
		if ($gFin!="")
		{$fFindSy = "AND (Nm_Aset LIKE '%".$gFin."%' OR Harga = '".$gFin."' OR Referensi LIKE '%".$gFin."%' OR Ref_History LIKE '%".$gFin."%' OR Ref_Usulan LIKE '%".$gFin."%' OR Ref_Temp LIKE '%".$gFin."%' OR Kd_Aset LIKE '%".$gFin."%' OR Keterangan LIKE '%".$gFin."%')";}
		#{$fFindSy = "AND (Harga = '".$gFin."')";}
		else
		{$fFindSy = "";}
		
		###################
		/*
		$nSQA= "SELECT Referensi, Ref_Group, No_Register, Kd_UPB, Kd_Aset, Tgl_Perolehan, Harga, No_Pengadaan FROM ta_kib_108 WHERE Kd_UPB LIKE '".substr($zUpb,0,11)."%' AND Tgl_Perolehan LIKE '2017-__-__' ORDER BY Kd_Aset, No_Register";
		...
		*/
		...
		$nSQL= "SELECT * FROM ta_kib_108".$tMuTZ." WHERE referensi LIKE '$gRf%' AND extracom LIKE '".$zExt."' AND Kd_UPB LIKE '".$zUpb."' AND Kd_Aset_108 LIKE '".$rBid.".".$rKel.".".$rOBJ.".".$rRin."' AND Tgl_Perolehan LIKE '".$rThn."-__-__' ".$fFindSy." ORDER BY Kd_Aset, No_Register LIMIT $Offset, $DataPerPage";
		#echo $nSQL;
		...
		do
			...
			//UPDATE UPB POST
			$eP="";
			$rUPB = fGlobal("kd_upb","ta_kib_post_108","Referensi",$mRo['Referensi'],"=","","");
			if ($rUPB!=$mRo['Kd_UPB'])
			{
				$eP="<br><font style='color:#ff0000'>error upb</font>";
				$eSQ = "UPDATE ta_kib_post_108 SET Kd_UPB='".$mRo['Kd_UPB']."' WHERE Referensi='".$mRo['Referensi']."'";
				#echo $eSQ."<br>";
				#$enR = mysql_query($eSQ);
			}
			
			if ((substr((int)$mRo['Tgl_Perolehan'],0,4)>=2017) && $mRo['No_Pengadaan']==""){
			...
			$gCEU = fGlobal("Referensi","ta_usulan_verifikasi_rinci_108","Ref_Aset:Kd_UPB:Eksekusi",$mRo['Referensi'].":".$mRo['Kd_UPB'].":Belum","=:=:=","","");
			if ($gCEU!=""){
			...
			$Ci = "";
			if ($mRo['Ref_Temp']!='' && substr($mRo['Ref_Temp'],0,3)=='AST')
			{
				$Ci = "<br><font style='color:#0000FF'>".$mRo['Ref_Temp']."</font>";
				
				$gCEC = fGlobal("Referensi_SP3B","ta_sp3d_spj_rinci","Referensi",$mRo['Ref_Temp'],"=","","");
				if ($gCEC==''){
					$Ci = "<br><font style='color:#FF0000'>".$mRo['Ref_Temp']."</font>";
				}
				else
				{
					$gCEO = fGlobal("IDT","ta_sp3d","Referensi:Tahun",$gCEC.":".substr($mRo['Tgl_Perolehan'],0,4),"=:=","","");
					if ($gCEO==''){
						$Ci = "<br><font style='color:#FF9900'>".$mRo['Ref_Temp']."##</font>";
					}
				}
			}
			?>
		  <tr height="60" style="cursor: pointer <?=$Blnk?>" onmouseover="this.style.cursor=&#39;pointer&#39" <?=fBackCLR($iG)?>> 
			...
			<td style="border-bottom: #999999 dotted 1px; border-right:#999999 dotted 1px; text-align:center"><?="<b>".$mRo['Referensi']."</b><br><font style='font-size:8pt'>".$mRo['Ref_History']."<br>".$mRo['Ref_Usulan']."</font>".$kdUPB.$eP.$Ci?></td>
			...
			<td style="border-bottom: #999999 dotted 1px; border-right:#999999 dotted 1px; padding-left:2px"> <?=$BV.$mRo['Keterangan']?></td>
        <td style="text-align:center; border-bottom: #999999 dotted 1px; border-right:#999999 dotted 1px; padding-left:2px"><?=fConvertDateShort($mRo['Tgl_Perolehan'])?></td>
        ...
        <td style="border-bottom:#999999 dotted 1px; border-right:#999999 dotted 1px; padding-right:2px <?=$ClR?>"  align="right" <?php echo $ClR?>> <?=fConvertToRupiah($gNIL)?>        </td>
		...
		<?php if ($eMuT==""){?>
		<!--a href="#" class="ico edit" onclick="EditData('1020','550','center','<?php echo $mRo['IDT']?>'); return false">Edit</a-->
		<!--&nbsp;&nbsp;|&nbsp;
		<a href="#" class="ico merg" onclick="MergerData('<?=$NeN?>','<?=$CeK?>','<?=$ThN?>','<?=$KiB?>','950','525','center','<?=$mRo['IDT']?>','<?=$_GET['IdL']?>'); return false">Merger</a>&nbsp;&nbsp;|&nbsp;-->
      ...
      <tr> 
		...
		<table id="detail<?php echo $iG?>" cellpadding="3" border="0" width="900" style="border:1px solid #C0C0C0; font-family: Calibri; font-size: 9pt; font-style: Italic; color: #800000; display: none; border-collapse:collapse">
            <tr> 
              ...
              <td width="9">:</td>
              <td width="100"><?php echo fConvertToRupiah($mRo['Luas_M2'])?>&nbsp;m<sup>2</sup></td>
              <td width="4">&nbsp;</td>
              <td width="60">Sertifikat Tanggal</td>
              <td width="9">:</td>
              <td width="200">
                <?php echo fConvertDateShort($mRo['Sertifikat_Tanggal'])?>              </td>
            </tr>
            <tr> 
              <td>&nbsp;</td>
              <td>Jenis</td>
              ...
              <td>:</td>
              <td>
                <?php echo $mRo['Hak_Tanah']?>              </td>
              <td>&nbsp;</td>
              <td>Sertifikat Nomor</td>
              <td>:</td>
              <td><?php echo $mRo['Sertifikat_Nomor']?></td>
            </tr>
            <tr> 
              <td>&nbsp;</td>
              <td>Objek</td>
              ...
              <td>Lokasi</td>
              <td>:</td>
              <td valign="top"><?php echo $mRo['Alamat']?></td>
            </tr>
            <tr> 
              <td>&nbsp;</td>
              <td>Rincian</td>
			...
			if (win!=null)
    			{
					...
					<?php
						...
						$URL_Mid = "Open_KIB_Mid.php?CrKIB=KIB_P&gUnt=".$zUnt."&gSub=".$zSub."&gUpb=".$zUpb."&IdL=".$_GET['IdL'];
			...
			if (win!=null)
    			{
					...
					<?php
						...
						$URL_Mid = "Find_Acc_Mid.php?CrAcc=KIB-P&FrmG=".$_GET['FrmG']."&gUnt=".$zUnt."&gSub=".$zSub."&gUpb=".$zUpb."&IdL=".$_GET['IdL'];
...
function P_Import()
{
	...
	function MergerData(NeN,KeY,ThN,KiB,w,h,pos,IDT,IdL)
	{
		if (NeN > 0) {alert('Access denied, data sudah memiliki nilai hasil merger, silahkan RESTORE atau KAPITALISASI lebih dulu..!!'); return false;}
		if (KeY=='Y') {alert('Access denied, data kib '+KiB+' tahun '+ThN+' sudah terkunci..!!'); return false;}
		var win=null;
	...
	function P_OpenDoc(w,h,IdL)
	{
		...
		LeftPosition=(screen.width)?(screen.width-w)/2:100; 
		TopPosition=(screen.height)?(screen.height-h)/2:100;
		URL= 'KIB_P_Dokumen.php?CriT=BPK&gUnt='+gUnt+'&gSub='+gSub+'&gUpb='+gUpb+'&gBid='+gBid+'&gKel='+gKel+'&gOBJ='+gOBJ+'&gRin='+gRin+'&gExt='+gExt+'&gFin='+gFin+'&gThn='+gThn+'&IdL='+IdL;
		settings='width='+w+',height='+h+',top='+TopPosition+',left='+LeftPosition+',location=no,directories=no,status=no,menubar=yes,toolbar=yes,resizable=ya,maximize=yes,scrollbars=yes,navigation=no';
		window.open(URL,'',settings);
	}
	...
	function P_OpenKibar(w,h,IdT,IdL)
	{
		...
		LeftPosition=(screen.width)?(screen.width-w)/2:100; 
		TopPosition=(screen.height)?(screen.height-h)/2:100;
		URL= 'report/P47_Kibar_P.php?IdT='+IdT+'&IdL='+IdL;
		settings='width='+w+',height='+h+',top='+TopPosition+',left='+LeftPosition+',location=no,directories=no,status=no,menubar=yes,toolbar=yes,resizable=ya,maximize=yes,scrollbars=yes,navigation=no';
		window.open(URL,'',settings);
	}

...
<form name="myfrm" method="post" action="<?="Pengadaan_Mid_.php?gNOM=".$gNOM."&gIdT=".$gIdT."&gUnt=".$gUnt."&IdL=".$_GET['IdL']?>">
  ...
  <table border="0" align="center" style="width:900px">
    ...
    <tr height="20">
      ...
      <td>PERUNTUKAN</td>
      <td colspan="3">
	  <input name="fUK" id="fUK" type="text" readonly value="<? if ($gUT) {echo $gUT." : ".fGlobalNEW("nm_upb","ref_upb","kd_upb",$gUT,"=","",DatabaseSB,$ConSB,"");} else {echo "NONE";}?>" style="width:550px; border: 1px solid #C0C0C0; padding-left: 1px; padding-right: 1px; padding-top: 1px; padding-bottom: 1px" />
	    ...
	    <tr>
	     <td valign="top">
	          <? if ($gIdT) {?>
				...
				<? if ($gIdT!='' && $gRin!=''){?>
				<a href="<?="Form_Asset_".$gTmp."_Mid_Temp.php?gIdT=".$gIdT."&IdL=".$_GET['IdL']?>" class="ico item">&nbsp;&nbsp;Input Detail Data Aset <? if ($iG>1) {echo "BARU";}?>
				</a>
				<? } ?>
	          <? } ?>
		 &nbsp;</td>
			...
			if (win!=null)
    			{
					...
					?>       			
       			txtHTML="<html><head><title>Simbada</title></head><frameset framespacing='0' border='0' rows='102,*,30' frameborder='0'><frame name='WinFindBRK_Top' noresize src='<? echo $URL_Top?>' scrolling='no'><frame name='WinFindBRK_Mid' src='<? echo $URL_Mid?>' scrolling='auto'><frame name='WinFindBRK_Bot' src= '<? echo $URL_Bot?>' scrolling='no'><noframes><body><p>=>.............??!</p></body></noframes></frameset></html>"            
       			win.focus()

...
if ($FnD)
{
	$CrT ="WHERE (Kode LIKE '%$gFnD%' OR Deskripsi LIKE '%$FnD%')"; 
}
?>
...
$iG=1;
$tJmL= 0;
$nSQ = "SELECT IDT, Kode, Deskripsi FROM ref_keg_90_1 $CrT ORDER BY Kode";
#echo $nSQ;
$nRs = mysql_query($nSQ);
while ($mRo = mysql_fetch_array($nRs, MYSQL_BOTH))
{
	...
	$eIdT = $mRo[0];
	$eKdR = $mRo[1];
	$rCek = fGlobal("IDT","ref_keg_90_2","Kode",$eKdR.".%","LIKE","IDT LIMIT 0,1","");
	$rDel="dele";
	if ($rCek){$rDel="delt";}
	?>
	<tr height="25"> 
	  ...
	  <a href="#" onClick="deleteDATA('<?=$ReO?>','1','<?=$eIdT?>','<?=$rCek?>','<?=$IdL?>'); return false" class="ico <?=$rDel?>">&nbsp;delete</a>&nbsp;&nbsp;

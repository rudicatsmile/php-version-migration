...
<?php
extract($_GET);
$gREF= "PHT.".fGetDate('year').".XXXXXXXX";
$gHR = fGetDate('mday');
$gBL = fGetDate('mon');
$gTH = fGetDate('year');
$gHRd  = "00";
...
$gPER = "Permohonan Penghentian Status <br>Penggunaan BMD";
$gLAM = "3";
...
if ($IdT)
{
	...
	JbKepala as A10,
	NiKepala as A11 
	FROM ta_surat_usulan_penghentian WHERE IDT='$IdT'";
...
<form name="myfrm" method="POST" action="<?="Surat_permohonan_penghentian_frm_.php?IdT=".$IdT."&FrmG=".$_GET['FrmG']."&IdL=".$_GET['IdL']?>" enctype="multipart/form-data">
...
<table border="0" class="table-link" cellspacing="0" cellpadding="0" align="center" style="width:750px; color:#fff; background:#669999">
  ...
  <tr height="25">
	...
	<input type="button" name="B13" value="...." onclick="showDATA('','<?=$IdT?>','<?=$_GET['FrmG']?>','<?=$_GET['IdL']?>')" style="width: 30px; height:21px; color:#FF0000" />	</td>
    </tr>
  <tr height="25">
    <td>&nbsp;</td>
  ...
  <tr height="25">
	...
	<input name="dUNT" id="dUNT" type="text" value="<?=$dUNT?>" <?php if ($IdT=='' && $Lev <= 1) {?> onClick="showUNIT('','unit','','<?=$_GET['IdL']?>')" onkeypress="if (event.keyCode==13) {showUNIT('find','unit','','<?=$_GET['IdL']?>'); return false;} else if (event.keyCode==27) {closeCLICK('unit'); return false;}" <?php } else {echo "readonly";}?> style="padding-left:5px; width:450px; border: 1px solid #C0C0C0"/>
...
<table border="0" class="table-link" cellspacing="0" cellpadding="0" align="center" style="width:750px; height:30px; background: #EAE6E6">
  <tr>
	...
	&nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;&nbsp;
	<input type="button" name="B14" <?php if ($IdT=="") {echo "disabled";}?> value="CETAK" onclick="P_Dokumen('800','400','<?=$IdT?>','<?=$_GET['IdL']?>')" style="width: 120px; height: 21px; color:#0000FF" />	</td>
    </tr>
</table>
</form>
</body>
...
<script languange="javascript">
	...
	function showUNIT(CrT,div,fr,IdL)
	{
		if (CrT=='find')
		{
			...
			{
				$("#"+div+"Div2Cri").load('Surat_permohonan_penghentian_frm_find_unit_mid.php?gFnD='+FnD+'&div='+div+'&IdL='+IdL);
			});
		}
		else
		{
			...
			$(document).ready(function()
			{
				$("#"+div+"Div1Cri").load('Surat_permohonan_penghentian_frm_find_unit_top.php?div='+div+'&IdL='+IdL);
			});
			
			$(document).ready(function()
			{
				$("#"+div+"Div2Cri").load('Surat_permohonan_penghentian_frm_find_unit_mid.php?div='+div+'&IdL='+IdL);
			});
	...
	function showDATA(CrT,IdT,FrmG,IdL)
	{
		...
		if (CrT=='find')
		{
			...
			$(document).ready(function()
			{
				$("#tranDiv2Cri").load('Surat_permohonan_penghentian_frm_data_mid.php?gFnD='+gFnD+'&gUnt='+gUnt+'&FrmG='+FrmG+'&IdL='+IdL);
			});
		}
		else
		{
			...
			$(document).ready(function()
			{
				$("#tranDiv1Cri").load('Surat_permohonan_penghentian_frm_data_top.php?FrmG='+FrmG+'&IdL='+IdL);
			});
			
			$(document).ready(function()
			{
				$("#tranDiv2Cri").load('Surat_permohonan_penghentian_frm_data_mid.php?gUnt='+gUnt+'&FrmG='+FrmG+'&IdL='+IdL);
			});
	...
	function P_Dokumen(w,h,IdT,IdL)
	{
		...
		URL='Surat_permohonan_penghentian_frm_data_dok.php?IdT='+IdT+'&IdL='+IdL;

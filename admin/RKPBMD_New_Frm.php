...
<?php
extract($_GET);
$gREF= "RKP.".(fGetDate('year')+1).".XXXXXXXX";
...
if ($IdT)
{
	$nSQL= "SELECT Referensi, Kd_Unit, Tahun, Uraian, Apbd FROM ta_rkpbmd_new WHERE IDT='$IdT'";
	...
	$stLOCK = fGlobalNEW("pemeliharaanBMD","ta_unit_lock","kdUnit:tahunBMD",$gUNT.":".$gTHN,"=:=","",DatabaseSB,$ConSB,"");
}
?>
<body onload="RefreshDATA('<?=$stLOCK?>','<?=$IdL?>')">
<?php require "FileMenu.php";?>
<form name="myfrm" method="POST" action="<?="RKPBMD_New_Frm_.php?FrmG=".$_GET['FrmG']."&IdT=".$IdT."&IdL=".$_GET['IdL']?>" enctype="multipart/form-data">
...
<table border="0" class="table-link" cellspacing="0" cellpadding="0" align="center" style="width:99%; background:#909EB8">
  <tr>
    <td width="10">&nbsp;</td>
    <td width="73">&nbsp;</td>
    <td width="15">&nbsp;</td>
  ...
  <tr height="25">
	...
	<div id="detaMstCri" class="findpkr0Cri">
		<div id="detaDiv1Cri" class="findpkr1Cri"></div>
		<div id="detaDiv2Cri" class="findpkr2Cri"></div>
	</div>	</td>
    <td align="center">&nbsp;</td>
  ...
  <tr>
    ...
    <td><input type="button" name="B39" <?=$DisA?> value="SAVE" onclick="Save('<?=$stLOCK?>')" style="width: 80px; height: 21px" />
	  ...
	  <input type="button" name="B13" value="DAFTAR USULAN" onclick="showDATANew('<?=$_GET['IdL']?>')" style="width: 130px; height: 21px" />
	  	  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	  <input type="button" name="B14" <?php if ($IdT=="") {echo "disabled";}?> value="CETAK RINCI ( I )"  onclick="P_Dokumen('800','400','<?=$gUBH?>','RKPBMD_New_Dokumen','<?=$IdT?>','<?=$_GET['IdL']?>')" style="width: 120px; height: 21px; color:#0000FF" />
	  <input type="button" name="B14" <?php if ($IdT=="") {echo "disabled";}?> value="CETAK RINCI ( II )" onclick="P_Dokumen('800','400','<?=$gUBH?>','RKPBMD_New_Dokumen_ii','<?=$IdT?>','<?=$_GET['IdL']?>')" style="width: 120px; height: 21px; color:#0000FF" />
...
<script languange="javascript">
	...
	function showUNIT(CrT,IdL)
	{
		if (CrT=='find')
		{
			...
			{
				$("#unitDiv2Cri").load('RKPBMD_New_Frm_Find_Unit_Mid.php?gFnD='+gFnD+'&IdL='+IdL);
			});
		}
		else
		{
			...
			$(document).ready(function()
			{
				$("#unitDiv1Cri").load('RKPBMD_New_Frm_Find_Unit_Top.php?IdL='+IdL);
			});
			
			$(document).ready(function()
			{
				$("#unitDiv2Cri").load('RKPBMD_New_Frm_Find_Unit_Mid.php?IdL='+IdL);
			});
	...
	*/
	...
	function showCLICK(crt,mesg,refs,kde,nma,IdL)
	{
		...
		if (crt=='aset') 
		{
			$(document).ready(function()
			{
				$("#ViewDATA").load('RKPBMD_New_Frm_Find_PKRK_Aset_Add.php?kIdT='+kde+'&rDTA='+nma+'&IdL='+IdL);
			});
		}
		document.getElementById(crt+'MstCri').style.display = "none";
	}
	...
	function RefreshDATA(stLOCK,IdL)
	{
		...
		$(document).ready(function()
		{
			$("#ViewDATA").load('RKPBMD_New_Frm_Data.php?gREF='+gREF+'&gTHN='+gTHN+'&gUBH='+gUBH+'&IdL='+IdL);
		});
		
		$(document).ready(function()
		{
			$.ajax({
				url:"RKPBMD_New_Frm_Data.php",
	...
	*/
	function showDATANew(IdL)
	{
		...
		window.open('RKPBMD_New.php?FrmG=DAFTAR USULAN PEMELIHARAAN BMD&gUNT='+gUNT+'&IdL='+IdL,'_self');
	}
	/*
	...
	*/
	...
	function showPROG(stLOCK,CrT,IdT,IdL)
	{
		...
		if (CrT=='find')
		{
			...
			$(document).ready(function()
			{
				$("#progDiv2Cri").load('RKPBMD_New_Frm_Find_PKRK_Mid.php?gFrm=prog&gFnD='+gFnD+'&stLOCK='+stLOCK+'&IdT='+IdT+'&gUNT='+gUNT+'&IdL='+IdL);
			});
		}
		else
		{
			...
			$(document).ready(function()
			{
				$("#progDiv1Cri").load('RKPBMD_New_Frm_Find_PKRK_Top.php?gFrm=prog&stLOCK='+stLOCK+'&IdT='+IdT+'&IdL='+IdL);
			});
			
			$(document).ready(function()
			{
				$("#progDiv2Cri").load('RKPBMD_New_Frm_Find_PKRK_Mid.php?gFrm=prog&stLOCK='+stLOCK+'&IdT='+IdT+'&gUNT='+gUNT+'&IdL='+IdL);
			});
	...
	function showPKRK_add(stLOCK,crt,iD,IdT,IdP,IdL)
	{
		...
		$(document).ready(function()
		{
			$("#ViewDATA").load('RKPBMD_New_Frm_Find_PKRK_Add.php?stLOCK='+stLOCK+'&crt='+crt+'&iD='+iD+'&IdT='+IdT+'&IdP='+IdP+'&IdL='+IdL);
			//$("#ViewDELL").load('RKPBMD_New_Frm_Find_PKRK_Add.php?stLOCK='+stLOCK+'&crt='+crt+'&iD='+iD+'&IdT='+IdT+'&IdP='+IdP+'&IdL='+IdL);
		});
	}
	
	function remoPKRK(stLOCK,crt,rIdT,IdL)
	{
		if (stLOCK=='1'){alert('Access denied, data sudah terkunci..!!'); return false;}
		...
		if (AN)
		{
			$(document).ready(function()
			{
				$("#ViewDELL").load('RKPBMD_New_Frm_Find_PKRK_Rem.php?stLOCK='+stLOCK+'&crt='+crt+'&rIdT='+rIdT+'&IdL='+IdL);
			});
		}
	}
	...
	function showKEGI(stLOCK,CrT,rIdT,IdL)
	{
		...
		if (CrT=='find')
		{
			...
			$(document).ready(function()
			{
				$("#kegiDiv2Cri").load('RKPBMD_New_Frm_Find_PKRK_Mid.php?gFrm=kegi&gFnD='+gFnD+'&stLOCK='+stLOCK+'&rIdT='+rIdT+'&gUNT='+gUNT+'&IdL='+IdL);
			});
		}
		else
		{
			...
			$(document).ready(function()
			{
				$("#kegiDiv1Cri").load('RKPBMD_New_Frm_Find_PKRK_Top.php?gFrm=kegi&stLOCK='+stLOCK+'&rIdT='+rIdT+'&IdL='+IdL);
			});
			
			$(document).ready(function()
			{
				$("#kegiDiv2Cri").load('RKPBMD_New_Frm_Find_PKRK_Mid.php?gFrm=kegi&stLOCK='+stLOCK+'&rIdT='+rIdT+'&gUNT='+gUNT+'&IdL='+IdL);
			});
	...
	function showSUBK(stLOCK,CrT,rIdT,IdL)
	{
		...
		if (CrT=='find')
		{
			...
			$(document).ready(function()
			{
				$("#subkDiv2Cri").load('RKPBMD_New_Frm_Find_PKRK_Mid.php?gFrm=subk&gFnD='+gFnD+'&stLOCK='+stLOCK+'&rIdT='+rIdT+'&gUNT='+gUNT+'&IdL='+IdL);
			});
		}
		else
		{
			...
			$(document).ready(function()
			{
				$("#subkDiv1Cri").load('RKPBMD_New_Frm_Find_PKRK_Top.php?gFrm=subk&stLOCK='+stLOCK+'&rIdT='+rIdT+'&IdL='+IdL);
			});
			
			$(document).ready(function()
			{
				$("#subkDiv2Cri").load('RKPBMD_New_Frm_Find_PKRK_Mid.php?gFrm=subk&stLOCK='+stLOCK+'&rIdT='+rIdT+'&gUNT='+gUNT+'&IdL='+IdL);
			});
	...
	function showREKN(stLOCK,CrT,rIdT,IdL)
	{
		...
		if (CrT=='find')
		{
			...
			$(document).ready(function()
			{
				$("#reknDiv2Cri").load('RKPBMD_New_Frm_Find_PKRK_Mid.php?gFrm=rekn&gFnD='+gFnD+'&stLOCK='+stLOCK+'&rIdT='+rIdT+'&gUNT='+gUNT+'&IdL='+IdL);
			});
		}
		else
		{
			...
			$(document).ready(function()
			{
				$("#reknDiv1Cri").load('RKPBMD_New_Frm_Find_PKRK_Top.php?gFrm=rekn&stLOCK='+stLOCK+'&rIdT='+rIdT+'&IdL='+IdL);
			});
			
			$(document).ready(function()
			{
				$("#reknDiv2Cri").load('RKPBMD_New_Frm_Find_PKRK_Mid.php?gFrm=rekn&stLOCK='+stLOCK+'&rIdT='+rIdT+'&gUNT='+gUNT+'&IdL='+IdL);
			});
	...
	function saveRECO(stLOCK,rb,crt,fld,rIdT,IdL)
	{
		if (stLOCK=='1'){alert('Access denied, data sudah terkunci..!!'); return false;}
		if (rb=='rb'){
			fld = fld;
		...
		$(document).ready(function()
		{
			$("#ViewDELL").load('RKPBMD_New_Frm_Find_PKRK_Save.php?stLOCK='+stLOCK+'&crt='+crt+'&fld='+fld+'&rIdT='+rIdT+'&IdL='+IdL);
		});
	}
	
	function showASET(CrT,kIdT,IdL)
	{
		if (!kIdT) {alert('Data belum tersimpan...!'); return false;}
		var fUNT = objfrm.fUNT.value;
		if (CrT=='find')
		{
			var gTBL = "";
			...
			$(document).ready(function()
			{
				$("#asetDiv2Cri").load('RKPBMD_New_Frm_Find_PKRK_Aset_Mid.php?gTBL='+gTBL+'&gFnD='+gFnD+'&kIdT='+kIdT+'&gUNT='+fUNT+'&IdL='+IdL);
			});
		}
		else
		{
			...
			$(document).ready(function()
			{
				$("#asetDiv1Cri").load('RKPBMD_New_Frm_Find_PKRK_Aset_Top.php?kIdT='+kIdT+'&IdL='+IdL);
			});
			
			$(document).ready(function()
			{
				$("#asetDiv2Cri").load('RKPBMD_New_Frm_Find_PKRK_Aset_Mid.php?gTBL='+gTBL+'&kIdT='+kIdT+'&gUNT='+fUNT+'&IdL='+IdL);
			});
	...
	function formDetail(CrT,stLOCK,IdT,IdL)
	{
		if (CrT=='refr')
		{
			$(document).ready(function()
			{
				$("#detaDiv2Cri").load('RKPBMD_New_Frm_Data_Detail_Mid.php?stLOCK='+stLOCK+'&IdT='+IdT+'&IdL='+IdL);
			});
		}
		else
		{
			dispBLOCK('detaDiv2Cri');
			$(document).ready(function()
			{
				$("#detaDiv1Cri").load('RKPBMD_New_Frm_Data_Detail_Top.php?stLOCK='+stLOCK+'&IdT='+IdT+'&IdL='+IdL);
			});
			...
			dispBlockOrNo('detaMstCri');
		}
	}
	
	function formDetailSave(crt,fld,IdT,IdR,stLOCK,IdL)
	{
		//alert(crt);
		if (crt=='gunakan' || crt=='kondisi_barang')
		{
			fld = fld;
		}
		else
		...
		$("#ViewDELL").load('RKPBMD_New_Frm_Data_Detail_Mid_Save.php?crt='+crt+'&fld='+fld+'&IdT='+IdT+'&IdR='+IdR+'&stLOCK='+stLOCK+'&IdL='+IdL);

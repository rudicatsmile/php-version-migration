...
<table border="0" width="800" cellspacing="0" cellpadding="0" align="center" style="border:0px solid #000; border-collapse: collapse; font-family:Calibri; font-size:10pt">
  <tr>
    <td style="text-align:right">Format IV.L.2.3.b</td>
  </tr>
</table>
...
<table border="0" width="800" align="center" cellspacing="0" cellpadding="0" style="border:0px solid #000; border-collapse: collapse; font-family:Calibri; font-size:10pt">
  <tr height="25" style="font-weight:bold; text-align:center">
    ...
    <td rowspan="2" style="border:1px solid #000">Akumulasi<br>Penyusutan<br>(Rp)</td>
  ...
  <tr style="text-align:center">
	...
	{ 
		...
		$x4 = fGlobal("ifnull(sum(Penyusutan_Akumulasi),0)","ta_kib_post_penyusutan_108","Kd_UPB:Kd_Aset_108:Tahun:Extracom",$KdS."%:1.3.%:".($rThn-1).":N","LIKE:LIKE:=:LIKE","","");
		$x5 = $x3 - $x4;
		
		$xB = "bold";
		TempRow($x0,$x1,$x2,$x3,$x4,$x5,$xB);
		Level3($x1,$KdS,$TG,$TGx,$TGz,$rSem,$rThn,DatabaseSB,$ConSB);
		$t3 = $t3+$x3;
		$t4 = $t4+$x4;
		$t5 = $t5+$x5;
	}
	...
	function Level3($Mst,$KdS,$TG,$TGx,$TGz,$rSem,$rThn,$DatabaseSA,$ConSA,$DatabaseSB,$ConSB)
	{
		...
		{ 
			...
			{
				$x4 = fGlobal("ifnull(sum(Penyusutan_Akumulasi),0)","ta_kib_post_penyusutan_108","Kd_UPB:Kd_Aset_108:Tahun:Extracom",$KdS."%:".$x1."%:".($rThn-1).":N","LIKE:LIKE:=:LIKE","","");
				$x5 = $x3 - $x4;
			}
	...
	function Level4($Mst,$KdS,$TG,$TGx,$TGz,$rSem,$rThn,$DatabaseSA,$ConSA,$DatabaseSB,$ConSB)
	{
		...
		{ 
			...
			{
				$x5 = fGlobal("ifnull(sum(Penyusutan_Akumulasi),0)","ta_kib_post_penyusutan_108","Kd_UPB:Kd_Aset_108:Tahun:Extracom",$KdS."%:1.3.2.%:".($rThn-1).":N","LIKE:LIKE:=:LIKE","","");;
			}
			else if ($x1=='1.3.7.02')
			{
				$x5 = fGlobal("ifnull(sum(Penyusutan_Akumulasi),0)","ta_kib_post_penyusutan_108","Kd_UPB:Kd_Aset_108:Tahun:Extracom",$KdS."%:1.3.3.%:".($rThn-1).":N","LIKE:LIKE:=:LIKE","","");;
			}
			else if ($x1=='1.3.7.03')
			{
				$x5 = fGlobal("ifnull(sum(Penyusutan_Akumulasi),0)","ta_kib_post_penyusutan_108","Kd_UPB:Kd_Aset_108:Tahun:Extracom",$KdS."%:1.3.4.%:".($rThn-1).":N","LIKE:LIKE:=:LIKE","","");;
			}
			else if ($x1=='1.3.7.04')
			{
				$x5 = fGlobal("ifnull(sum(Penyusutan_Akumulasi),0)","ta_kib_post_penyusutan_108","Kd_UPB:Kd_Aset_108:Tahun:Extracom",$KdS."%:1.3.5.%:".($rThn-1).":N","LIKE:LIKE:=:LIKE","","");;
			}
			else
			{
				$x4 = fGlobal("ifnull(sum(Penyusutan_Akumulasi),0)","ta_kib_post_penyusutan_108","Kd_UPB:Kd_Aset_108:Tahun:Extracom",$KdS."%:".$x1."%:".($rThn-1).":N","LIKE:LIKE:=:LIKE","","");
	...
	<?php function TempRow($x0,$x1,$x2,$x3,$x4,$x5,$xB)
	{
	?>

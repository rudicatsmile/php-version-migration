...
<form name="myfrm" method="POST" action="<?="Ref_Usulan_Syarat.php?IdL=".$_GET['IdL']?>">
...
<table border="0" cellspacing="0" class="table-link" cellpadding="0" align="center" style="width:1200px; height:20px; background: #CEFBE3; font-weight:bold">
  <tr>
    ...
    <td width="150" align="center">SYARAT</td>
...
<script languange="javascript">
	...
	function RefreshDATA(IdL)
	{
		...
		{
			$("#ViewDATA").load('Ref_Usulan_Syarat_Data.php?gJN='+fJN+'&IdL='+IdL);
		});
	}
	...
	function formDELE(ReO,gID,DeL,gIdL)
	{
		...
		if (AN)
		{
			...
			$(document).ready(function()
			{
				$("#alasDiv2Cri").load('Ref_Usulan_Syarat_Del.php?gID='+gID+'&gJN='+gJN+'&IdL='+gIdL);
			});
		}
	}
	...
	function formEDIT(ReO,gID,IdL)
	{
		...
		$(document).ready(function()
		{
			$("#alasDiv1Cri").load('Ref_Usulan_Syarat_Top.php?gID='+gID+'&IdL='+IdL);
		});
		
		$(document).ready(function()
		{
			$("#alasDiv2Cri").load('Ref_Usulan_Syarat_Mid.php?gID='+gID+'&gJN='+gJN+'&IdL='+IdL);
		});
	...
	function resetEDIT(ReO,IdL)
	{
		...
		$(document).ready(function()
		{
			$("#alasDiv2Cri").load('Ref_Usulan_Syarat_Mid.php?gJN='+gJN+'&IdL='+IdL);
		});
	}
	...
	{
		if (ReO=='Y') {alert('Access denied, akses readony..!!'); return false;}
		var gUS = objfrm.fUse.checked;
		var gJN = objfrm.fJNS.value;
		var gDS = ReplaceText(document.getElementById('fDS').value);
		$(document).ready(function()
		{
			$("#alasDiv2Cri").load('Ref_Usulan_Syarat_Save.php?gID='+gID+'&gUS='+gUS+'&gDS='+gDS+'&gJN='+gJN+'&IdL='+IdL);
		});
	}

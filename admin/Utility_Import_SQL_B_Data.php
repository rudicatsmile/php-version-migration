...
if ($gFnD!=''){$SyT = "AND (P1.IDPemda LIKE '%".$FnD."%' OR P1.Keterangan LIKE '%".$FnD."%')";}
...
while($mRo = sqlsrv_fetch_array($rst))
{
	...
	$SQa = "SELECT sum(harga) FROM Ta_KIBBR WHERE IDPemda='".$mRo[0]."' AND Kd_Riwayat='2'";
	$rsa = sqlsrv_query($conn,$SQa);
	$mRa = sqlsrv_fetch_array($rsa);
	
	$SQb = "SELECT sum(harga) FROM Ta_Fn_KIB_B WHERE IDPemda='".$mRo[0]."' AND Tahun='2030'";
...
<tr height="100%">
  ...
  <td width="70" style="border-right:1px solid #ccc; border-bottom:1px solid #ccc"></td>
  <td width="300" style="border-right:1px solid #ccc; border-bottom:1px solid #ccc"></td>

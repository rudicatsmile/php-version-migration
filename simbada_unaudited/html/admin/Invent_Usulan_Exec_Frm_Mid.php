...
function AddTEMP($PHM,$UpB,$NeW,$NrG,$DatabaseSB,$ConSB,$fS)
{
	$rCK = fGlobal("IDT","ta_usulan_rinci_108","referensi:ref_aset:no_register",$PHM.":".$NeW.":".$NrG,"=:=:=","",$fS);
	if ($rCK=='')
	{
		$eSQ = "SELECT Referensi,Kd_UPB,Kd_Aset_108,No_Register,Nm_Aset,Tgl_Perolehan,Keterangan,Harga,Ref_Mutasi,Ref_Usulan,extracom FROM ta_kib_108 
		WHERE Referensi= '".$NeW."' AND Kd_UPB LIKE '".$UpB."%' AND No_Register='".$NrG."'";
		if ($fS) {echo $eSQ."<br>";}
		$eRs = mysql_query($eSQ);
		while ($eRo = mysql_fetch_array($eRs, MYSQL_BOTH))
		{
			$eRo8 = fGlobal("ifNull(sum(debet),0)","ta_kib_post_108","Referensi:Kd_UPB:No_Register",$NeW.":".$UpB."%:".$NrG,"=:LIKE:=","","");
			...
			$gFLD.= ", Nm_Aset";
			$gVAL.= ", '".mysql_real_escape_string($eRo[4])."'";
			
			$gFLD.= ", Tgl_Perolehan";
			$gVAL.= ", '".$eRo[5]."'";
			
			$gFLD.= ", Uraian";
			$gVAL.= ", '".mysql_real_escape_string($eRo[6])."'";
...
<table border="0" class="table-link" cellspacing="0" cellpadding="0" align="center" style="width:1080px; height:380px">
  <tr>
	...
	<tr height="25" style="font-size:8pt; font-weight:bold; text-align:center; background-color: #ABD6CD">
	  ...
	  <td style="border-right: 1px solid #ccc; border-bottom: 1px solid #ccc">NOMOR</td>
	  <td style="border-right: 1px solid #ccc; border-bottom: 1px solid #ccc">REF.ASET ASAL</td>
	  <td style="border-right: 1px solid #ccc; border-bottom: 1px solid #ccc">REF.ASET BARU</td>
	...
	while ($nRo = mysql_fetch_array($Rs, MYSQL_BOTH))
	{
		...
		if ($nRo[10]=='Sudah')
		{
			$NeW = fGlobal("Referensi_To","ta_kib_108_mutasi","Referensi:Ref_Usulan:No_Register",$tRA.":".$nRo[1].":".$nRo[4],"=:=:=","","");
			#INJECT
			if ($NeW!="")
			{
				if ($nRo[1]=='PHM.2024.00000248xx' && $Lev==0)
				{
					#Insert otomatis ke usulan penghapusan dari PHM.2024.00000248 ke PHM.2024.00000293
					AddTEMP('PHM.2024.00000293xx',$tUP,$NeW,$nRo[4],DatabaseSB,$ConSB,"");
				}
				
				if ($nRo[1]=='PHM.2024.00000140xx' && $Lev==0)
				{
					#Insert otomatis ke usulan penghapusan dari PHM.2024.00000140 ke PHM.2024.00000239
					AddTEMP('PHM.2024.00000239xx',$tUP,$NeW,$nRo[4],DatabaseSB,$ConSB,"");
				}
			}
			
			$RnE = fGlobal("IDT","ta_kib_108_mutasi","Ref_History:Ref_Usulan_His",$tRA.":".$nRo[1],"=:=","","");
			if ($RnE!="")
			{
				$CnE="; color:#ff0000";
				#$ReM="YA";
		...
		if ($tJN=="MK" && $nRo[12]==''){
			$LeNon="Proses data MUTASI ANTAR KIB ditolak, rekening tujuan belum ditentukan..!";
			$ExecAll.="No";
		}
		...
		<tr height="25">
		  ...
		  <td width="100" style="text-align:center; border-bottom: 1px dotted #999; border-right: 1px solid #ccc <?=$CnE?>" <?=$gBG?>><?=$nRo[2]?></td>
		  <td width="100" style="text-align:center; border-bottom: 1px dotted #999; border-right: 1px solid #ccc; color:#0000FF" <?=$gBG?>><?=$NeW?></td>
		  <td <?=$gBG?> width="100" style="text-align:center; border-bottom: 1px dotted #999; border-right: 1px solid #ccc <?=$Err?>"><?=$nRo[3]?><? if ($tJN=="MK") {if ($nRo[12]) {echo "<br>".$nRo[12];} else {echo "<br><font style='color:#ff0000'>Rek tujuan...??</font>";}}?></td>
		  ...
		  <td width="60" style="text-align:center; border-bottom: 1px dotted #999; border-right: 1px solid #ccc; font-size:8pt" <?=$gBG?>><? if ($nRo[15]!="0000-00-00"){echo fConvertDateShort($nRo[15]);}?></td>
		  <td <?=$gBG?> width="160" style="border-bottom: 1px dotted #999; text-align:center">
		  <?
		  if ($JenS=='RB' || $JenS=='HB' || $JenS=='LE' || $JenS=='PL' || $JenS=='KR' || $JenS=='TG' || $JenS=='MS' || $JenS=='MK' || $JenS=='PH' || $JenS=='KP'){
		  if ($nRo[10]=='Sudah'){
		  	if ($CnE==''){
			?>
	...
	<tr height="100%">
	  ...
	  <td style="border-right: 1px solid #ccc">&nbsp;</td>
	  <td align="center">
	  <? if ($UID=='creator' && $JenS=='MS'){?>
	  <a href="#" onClick="execFORM_All('<?=$ExecAll?>','<?=$RefU?>','<?=$PgE?>','<?=$IdT?>','<?=$IdL?>'); return false" class="ico mess">

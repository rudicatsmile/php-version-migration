	...
	while ($mRo = mysql_fetch_array($nRs, MYSQL_BOTH))
	{
		...
		<tr style="vertical-align:top">
          ...
          <td style="border-bottom:1px dotted #ccc">
			  <label><input name="Radio<?=$IdR?>" type="radio" onchange="formDetailSave('kondisi_barang','B','<?=$IdT?>','<?=$IdR?>','<?=$stLOCK?>','<?=$IdL?>'); return false;" value="B" <? if ($KonD=='B'){echo "checked";}?> />B</label>
			  <label><input name="Radio<?=$IdR?>" type="radio" onchange="formDetailSave('kondisi_barang','RR','<?=$IdT?>','<?=$IdR?>','<?=$stLOCK?>','<?=$IdL?>'); return false;" value="RR" <? if ($KonD=='RR'){echo "checked";}?> />RR</label>
			  <label><input name="Radio<?=$IdR?>" type="radio" onchange="formDetailSave('kondisi_barang','RB','<?=$IdT?>','<?=$IdR?>','<?=$stLOCK?>','<?=$IdL?>'); return false;" value="RB" <? if ($KonD=='RB'){echo "checked";}?> />RB</label>

...
<?php
#$nSQ = "SELECT IDT, Kd_Rek_Old, Nm_Rek FROM ref_rek_90_5 WHERE Kd_Rek_Old LIKE '_._._.__.__' AND Kd_Rek='' ORDER BY IDT";
#echo $nSQ."<br>";
#$nRs = mysql_query($nSQ);
...
#	$NewR.= substr($mRo[1],-6,6);
#	echo $NewR."<br>";
	
#	$nS = "UPDATE ref_rek_90_5 SET Kd_Rek='".$NewR."' WHERE IDT='".$IdT."'";
#	echo $nS."<br>";
#	$nR = mysql_query($nS);
#}

$iG=1;
$nSQ = "SELECT IDT, Kd_Rek, Nm_Rek FROM ref_rek_90_5 WHERE Kd_Rek LIKE '".$KdR4."%' $CrT ORDER BY Kd_Rek";
#echo $nSQ;
$nRs = mysql_query($nSQ);
while ($mRo = mysql_fetch_array($nRs, MYSQL_BOTH))
{
	...
	$eIdT = $mRo[0];
	$eKdR = $mRo[1];
	$rCek = fGlobal("IDT","ref_rek_90_6","Kd_Rek",$eKdR."%","LIKE","","");
	?>
	<tr height="25"> 
	  ...
	  <a href="<?="Ref_Kode_Rekn_90_6.php?FrmG=REFERENSI -> KODE REKENING PERMENDAGRI 90&rKdR=".$eKdR."&IdL=".$_GET['IdL']?>" class="ico prev">&nbsp;View</a>&nbsp;&nbsp;&nbsp;
	  <a href="#" onClick="editDATA('5','','<?=$eIdT?>','<?=$rCek?>','<?=$IdL?>'); return false" class="ico edit">&nbsp;Edit</a>&nbsp;&nbsp;&nbsp;
	  <a href="#" onClick="deleteDATA('5','<?=$eIdT?>','<?=$rCek?>','<?=$IdL?>'); return false" class="ico dell">&nbsp;Delete</a>&nbsp;&nbsp;

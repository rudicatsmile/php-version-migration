  ...
  <tr height="28">
	...
	<?php if ($JmREC>0)
	{
		...
		if ($JmREC < $PerPage)
		{
			...
			?>
			<a href="#" <?=$d?> onclick="pageDATA('0','<?=$PerPage?>','<?=$IdL?>');return false;">01</a>
			<?php
		}
		else
		{
			for ($iA = 0; $iA <= $PaG-1; $iA++)
			{
				...
				{
					?>
					<a href="#" <?=$d?> onclick="pageDATA('<?=$iB?>','<?=$iA*$PerPage?>','<?=$IdL?>');return false;"><?=substr("00".($iA+1),-2,2)?></a>
					<?php
				}
				else if (($PaG-1) < 1000)
				{
					?>
					<a href="#" <?=$d?> onclick="pageDATA('<?=$iB?>','<?=$iA*$PerPage?>','<?=$IdL?>');return false;"><?=substr("000".($iA+1),-3,3)?></a>
					<?php
				} else {
					?>
					<a href="#" <?=$d?> onclick="pageDATA('<?=$iB?>','<?=$iA*$PerPage?>','<?=$IdL?>');return false;"><?=substr("0000".($iA+1),-4,4)?></a>
					<?php 
				}
			}
		}

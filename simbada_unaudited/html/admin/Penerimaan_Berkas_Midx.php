...
if ($gIdT)
{
	...
	if ($tRo > 0)
	{
		...
		$gCrB  = $mRo['CritBayar']; 
		//$gCrP  = $mRo['CritBayar_LinkUangMuka']; 
...
else
{
	...
	//$mSKD = 
	#fGlobalNEW("Satuan_Kerja","satuan_kerja","ID_Satker",$mSKD,"=","",DatabaseSA,$ConSA,"");
	#$nTHN = fGlobalNEW("Periode","peraturan_daerah","Activated","Y","=","",DatabaseSA,$ConSA,"");
	$nTHN = 2022;
	#$nAGG = fGlobalNEW("Perubahan","peraturan_daerah","Activated","Y","=","",DatabaseSA,$ConSA,"");
	$nAGG = 0;
	$gCrB = "";
	$gCrT = 0;
	$zRP  = "";
...
<form name="myfrm" method="post" action="<?="Penerimaan_Berkas_Mid_.php?gIdT=".$gIdT."&gUnt=".$gUnt."&IdL=".$_GET['IdL']?>">
  ...
  <table border="0" align="center" style="width:900px">
    ...
    <tr height="25">
      <td>&nbsp;</td>
      <td>UNIT KERJA</td>
      <td colspan="2"><input name="f01" type="text" readonly value="<?=$mSKD." : ".strtoupper($nSKD)?>" style="width:600px; border: 1px solid #C0C0C0; padding-left: 1px; padding-right: 1px; padding-top: 1px; padding-bottom: 1px" /></td>
    </tr>
    <!--tr height="25">
      <td>&nbsp;</td>
      <td>DATABASE</td>
      <td colspan="2">
	  <input name="f2" type="text" readonly="readonly" value="<?=strtoupper(DatabaseSA)?>" style="width:150px; border: 1px solid #C0C0C0; padding-left: 1px; padding-right: 1px; padding-top: 1px; padding-bottom: 1px" />
	  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;PERIODE &nbsp;&nbsp;
	  <input name="f3" type="text" readonly="readonly" value="<?=$nTHN?>" style="text-align:center; width:40px; border: 1px solid #C0C0C0; padding-left: 1px; padding-right: 1px; padding-top: 1px; padding-bottom: 1px" />
	  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;PERUBAHAN &nbsp;&nbsp;
	  <input name="f4" type="text" readonly="readonly" value="<?=$nAGG?>" style="text-align:center; width:30px; border: 1px solid #C0C0C0; padding-left: 1px; padding-right: 1px; padding-top: 1px; padding-bottom: 1px" /></td>
    </tr-->
    <tr height="25">
      <td>&nbsp;</td>
      <td>PROGRAM</td>
	  ...
	  <select name="fPR" style="width: 605px; border: 1px solid #C0C0C0; padding-left: 1px; padding-right: 1px; padding-top: 1px; padding-bottom: 1px" onchange="this.form.submit()">
		...
		$nSQ = "SELECT idProgram, nmProgram FROM ta_apbd_program_skpd WHERE kdUnit = '".$gUnt."' AND Periode='".$tTbl."' AND apbd='".$uTbl."' ORDER BY idProgram";
		$nRs = mysql_query($nSQ) or die(mysql_error());
    ...
    <tr height="25">
	  ...
	  <select name="fKG" style="width: 605px; border: 1px solid #C0C0C0; padding-left: 1px; padding-right: 1px; padding-top: 1px; padding-bottom: 1px" onchange="this.form.submit()">
        <?
		#CallConnection(DatabaseSA,$ConSA);
		#$nSQ = "SELECT Id_Kegiatan, Nama_Kegiatan FROM kegiatan WHERE Id_Kegiatan LIKE '".$zPR."%' GROUP BY Id_Kegiatan";
		$nSQ = "SELECT idKegiatan, nmKegiatan FROM ta_apbd_kegiatan_skpd WHERE kdUnit = '".$gUnt."' AND idKegiatan LIKE '".$zPR."%' AND Periode='".$tTbl."' AND apbd='".$uTbl."' ORDER BY idKegiatan";
		$nRs = mysql_query($nSQ) or die(mysql_error());
		while ($mRo = mysql_fetch_array($nRs, MYSQL_BOTH))
		{
			$sel = "";
    ...
    <tr height="25">
	  ...
	  <select name="fSB" style="width: 605px; border: 1px solid #C0C0C0; padding-left: 1px; padding-right: 1px; padding-top: 1px; padding-bottom: 1px" onchange="this.form.submit()">
		...
		#CallConnection(DatabaseSA,$ConSA);
		#$nSQ = "SELECT Id_Kegiatan, Nama_Kegiatan FROM kegiatan WHERE Id_Kegiatan LIKE '".$zPR."%' GROUP BY Id_Kegiatan";
		$nSQ = "SELECT idSubKegiatan, nmSubKegiatan FROM ta_apbd_kegiatan_sub_skpd WHERE kdUnit = '".$gUnt."' AND idSubKegiatan LIKE '".$zKG."%' AND Periode='".$tTbl."' AND apbd='".$uTbl."' ORDER BY idSubKegiatan";
		$nRs = mysql_query($nSQ) or die(mysql_error());
		while ($mRo = mysql_fetch_array($nRs, MYSQL_BOTH))
		{
			$sel = "";
    ...
    <tr height="25">
      ...
      <td colspan="2">
	  <?
	  #echo "SELECT kdRekening, nmRekening FROM ta_apbd_rekening_skpd WHERE kdUnit = '".$gUnt."' AND idSubKegiatan = '".$zSB."' AND Periode='".$tTbl."' GROUP BY kdRekening ORDER BY kdRekening";
	  ?>
	  <? if ($gIdT) {?>
	  ...
	  <option value=""></option>
		...
		$nSQ = "SELECT kdRekening, nmRekening FROM ta_apbd_rekening_skpd WHERE kdUnit = '".$gUnt."' AND idSubKegiatan = '".$zSB."' AND Periode='".$tTbl."' AND apbd='".$uTbl."' GROUP BY kdRekening ORDER BY kdRekening";
		$nRs = mysql_query($nSQ) or die(mysql_error());
		while ($mRo = mysql_fetch_array($nRs, MYSQL_BOTH))
		{
			$sel = "";
	...
	<? if ($gUnt=='24.04.07.01' || $gUnt=='24.04.23.06' || $gUnt=='24.04.23.08') {?>
    <tr height="25">
      <td>&nbsp;</td>
	  ...
	  <? } else {?>
			...
			<option value=""></option>
			<?
			$nSQ = "SELECT idSubUnit, nmSubUnit FROM ta_apbd_rekening_skpd WHERE kdUnit = '".$gUnt."' AND idSubKegiatan = '".$zSB."' AND kdRekening='".$zRK."' AND Periode='".$tTbl."' ORDER BY kdRekening";
			$nRs = mysql_query($nSQ) or die(mysql_error());
			while ($mRo = mysql_fetch_array($nRs, MYSQL_BOTH))
			{
				$sel = "";
    ...
    <tr height="25">
	  ...
	  <?
		...
		if ($zRP!='')
		{
			$gAGG = fGlobalNEW("IfNull(sum(fnJumlah),0)","ta_apbd_rekening_skpd","kdUnit:kdRekening:idSubKegiatan:Periode:idSubUnit:apbd",$gUnt.":".$zRK.":".$zSB.":".$tTbl.":".$zRP.":".$uTbl,"=:=:=:=:=:=","",DatabaseSB,$ConSB,"");
			...
			$gBrK = fGlobalNEW("IfNull(sum(Nilai),0)","ta_penerimaan_berkas","Kd_Unit:Kd_Rek13:Kd_SubKegiatan:Periode:Kd_Peruntukan",$gUnt.":".$zRK.":".$zSB.":".$tTbl.":".$zRP,"=:=:=:=:=","",DatabaseSB,$ConSB,"");
		}
		else
		{
			$gAGG = fGlobalNEW("IfNull(sum(fnJumlah),0)","ta_apbd_rekening_skpd","kdUnit:kdRekening:idSubKegiatan:Periode:apbd",$gUnt.":".$zRK.":".$zSB.":".$tTbl.":".$uTbl,"=:=:=:=:=","",DatabaseSB,$ConSB,"");
			...
			$gBrK = fGlobalNEW("IfNull(sum(Nilai),0)","ta_penerimaan_berkas","Kd_Unit:Kd_Rek13:Kd_SubKegiatan:Periode",$gUnt.":".$zRK.":".$zSB.":".$tTbl,"=:=:=:=","",DatabaseSB,$ConSB,"");
		}
		$gSiA = $gToA-$gBrK;
	  ?>
	  <input name="fAgg" type="text" readonly="readonly" value="<?=fConvertToRupiah($gAGG)?>" style="text-align:right; width:100px; border: 1px solid #C0C0C0; padding-left: 1px; padding-right: 2px; padding-top: 1px; padding-bottom: 1px" />
	  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;PEMBERKASAN (Rp)&nbsp;&nbsp;&nbsp;
	  <input name="fBer" type="text" readonly="readonly" value="<?=fConvertToRupiah($gBrK)?>" style="text-align:right; width:100px; border: 1px solid #C0C0C0; padding-left: 1px; padding-right: 2px; padding-top: 1px; padding-bottom: 1px" />
	  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;SISA AGG. (Rp)&nbsp;&nbsp;&nbsp;
	  <input name="fSis" type="text" readonly="readonly" value="<?=fConvertToRupiah($gSiA)?>" style="text-align:right; width:100px; border: 1px solid #C0C0C0; padding-left: 1px; padding-right: 2px; padding-top: 1px; padding-bottom: 1px; color:#0000ff" />	  </td>
    </tr>
    <tr>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
    ...
    <tr height="25">
      ...
      <td>NOMOR</td>
      <td colspan="2">
	  <input name="fNOM1" type="hidden" value="<?=$gNOM1?>" readonly style="width:204px; border: 1px solid #C0C0C0; padding-left: 1px; padding-right: 1px; padding-top: 1px; padding-bottom: 1px" />
    ...
    <tr height="25">
      <td>&nbsp;</td>
      <td>JUMLAH BARANG</td>
      <td colspan="2"><input name="fITM" type="text" value="<?=$gITM?>" onKeyUp="addSeparator(this)" style="width:70px; border: 1px solid #C0C0C0; padding-left: 1px; padding-right:4px; padding-top: 1px; padding-bottom: 1px; background: #FFFFFF" />&nbsp;&nbsp;&nbsp;&nbsp;(<i>Unit/Buah/Bidang</i>)</td>
    </tr>
    <tr height="25">
      <td>&nbsp;</td>
      <td>NILAI BARANG</td>
    ...
    <tr height="25">
	  ...
	  <table>
        <tr>
          <td width="110"><label><input name="fTRMt" type="radio" value="Perencanaan" <?=$gC1?> />&nbsp;PERENCANAAN</label></td>
          <td width="100"><label><input name="fTRMt" type="radio" value="UangMuka" <?=$gC2?> />&nbsp;UANG MUKA</label></td>
          ...
          <td width="140"><label><input name="fTRMt" type="radio" value="Pelunasan" <?=$gC4?> />&nbsp;100 % / <i>PELUNASAN</i> </label></td>

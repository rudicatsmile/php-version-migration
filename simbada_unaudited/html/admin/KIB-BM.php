...
if (isset($_GET['gThn'])) {$gThn   = $_GET['gThn'];} else {$gThn  = (date('Y')-1);}
...
<form name="myfrm" method="post" action="<?php echo "KIB-B_.php?FrmG=".$_GET['FrmG']."&IdL=".$_GET['IdL'] ?>">
  ...
  <table border="0" align="center" style="width:99%">
    ...
    <tr> 
      ...
      <td width="99">KELOMPOK</td>
      <td colspan="6"> <select class="boxs" name="fBid" tabindex="0" style="width: 480px" onchange="P_Change()">
          <option value=""></option>
    ...
    <tr> 
      ...
      <td align="right"><input type="button" name="B37" value="EXPORT DATA" onclick="P_ToExcel('800','400','center','ToExcel/KIB_B_ToExcel')" style="width: 90px; height: 24px; padding-left: 0px; padding-right: 0px; padding-top: 0px; padding-bottom: 0px" /></td>
    </tr>
    <tr>
      <td>RUANG</td>
      <td valign="middle">
    ...
    <tr> 
      ...
      <td valign="middle"> <table border="0" width="100%" cellpadding="0" style="border-collapse: collapse">
          <tr> 
            <td width="46%"> <input class="text" type="text" name="fFind" value="<? echo $gFin?>" style="width: 200px; font-family: Calibri; font-size: 10pt; padding-left: 4px; padding-right: 4px; padding-top: 1px; padding-bottom: 1px; background-color: #E1F986" /></td>
      ...
      <td width="134" valign="bottom"><label><input type="checkbox" <? if ($gMuT!=""){echo "checked";}?> name="fMuT" value="ON" onclick="this.form.submit()"/>MUTASI MASUK</label></td>
    ...
    <tr> 
      ...
      <td colspan="6">&nbsp;</td>
      <td><label style="float:right; color:#FF0000"><input type="checkbox" name="fSdhMutasi" <? if ($eMuT!="") {echo "checked";}?> onclick="P_Find()" value="ON" />
  ...
  <table border="0" align="center" id="tbViewData" style="width:99%">
    <tr> 
	  ...
	  <table align="center" border="0" class="table-list" cellspacing="0" cellpadding="0" style="width:99%">
		...
		if ($gFin!="")
		{$fFindSy = "AND (Kd_Aset LIKE '%".$gFin."%' OR Ref_Temp LIKE '%".$gFin."%' OR Nm_Aset LIKE '%".$gFin."%' OR Harga LIKE '%".$gFin."%' OR Referensi LIKE '%".$gFin."%' OR Nomor_Polisi LIKE '%".$gFin."%' OR Nomor_Rangka LIKE '%".$gFin."%' OR Nomor_BPKB LIKE '%".$gFin."%' OR Nomor_Mesin LIKE '%".$gFin."%' OR Kd_Aset LIKE '%".$gFin."%' OR Keterangan LIKE '%".$gFin."%' OR Merk LIKE '%".$gFin."%')";}
		else
		{$fFindSy = "";}
		
		###################
		/*
		$nSQA= "SELECT Referensi, Ref_Group, No_Register, Kd_UPB, Kd_Aset, Tgl_Perolehan, Harga, No_Pengadaan 
		FROM ta_kib_b WHERE Kd_UPB LIKE '".substr($zUpb,0,11)."%' AND Tgl_Perolehan LIKE '2017-%-%' ORDER BY Kd_Aset, No_Register";
		$nRsA= mysql_query($nSQA) or die(mysql_error());
		while ($mRoA = mysql_fetch_array($nRsA, MYSQL_BOTH))
		{
			$mRoRf = $mRoA[0];
			$gCeK = fGlobalNEW("IDT","ta_kib_post","Referensi",$mRoRf,"=","",DatabaseSB,$ConSB,"");
			if ($gCeK==""){
				$gREF = $mRoA[0];
				$gGRP = $mRoA[1];
				$gREG = $mRoA[2];
		...
		*/
		...
		#$nSQL= "SELECT * FROM ta_kib_b WHERE extracom LIKE '".$zExt."' AND Kd_UPB LIKE '".$zUpb."' AND Kd_Ruang LIKE '$zRua' AND Kd_Aset LIKE '".$rBid.".".$rKel.".".$rOBJ.".".$rRin."' AND Tgl_Perolehan LIKE '".$rThn."-__-__' ".$fFindSy." ORDER BY Kd_Aset, Tgl_Perolehan, No_Register LIMIT $Offset, $DataPerPage";
		
		$nSQL= "SELECT * FROM ta_kib_b".$tMuTZ." WHERE extracom LIKE '".$zExt."' AND Kd_UPB LIKE '".$zUpb."' 
		AND Kd_Ruang LIKE '$zRua' AND Kd_Aset LIKE '".$rBid.".".$rKel.".".$rOBJ.".".$rRin."' 
		AND Tgl_Perolehan >='".$gThnA."-01-01' AND Tgl_Perolehan <='".$gThn."-12-31' ".$fFindSy.$tMuT."ORDER BY Kd_Aset, Tgl_Perolehan, No_Register LIMIT $Offset, $DataPerPage";
		#echo $nSQL;
		...
		do
			...
			$gCEU = fGlobal("Referensi","ta_usulan_rinci","Ref_Aset:Kd_UPB",$mRo['Referensi'].":".$mRo['Kd_UPB'],"=:=","","");
			if ($gCEU!=""){
				$gSkP = substr(fGlobal("To_UPB","ta_usulan_rinci","Ref_Aset:Kd_UPB",$mRo['Referensi'].":".$mRo['Kd_UPB'],"=:=","",""),0,11);
				$gSkM = "";
				if ($gSkP!="" && $gSkP!=$SkT){
			...
			if ($tMuTZ==""){
				$gCeU = $mRo['Ref_Mutasi'];
				if ($gCeU!=""){
					$SG = fGlobal("Kd_UPB","ta_kib_b_mutasi".$tMuTZ,"Referensi_To:Kd_UPB_To",$mRo['Referensi'].":".substr($mRo['Kd_UPB'],0,11)."%","=:LIKE","","");
			...
			if ($mRo['Tgl_Mutasi']=="0000-00-00"){
				$gAda = fGlobal("IDT","Ta_KIB_Post","Referensi:Kd_UPB:Ref_Mutasi",$mRo['Referensi'].":".substr($mRo['Kd_UPB'],0,11)."%:","=:LIKE:<>","","");
				if ($gAda){
					$gAda="**";
				}
			}
			...
			$gHRG = $mRo['Harga'];
			$gNIa = fGlobal("IfNull(sum(debet),0)","ta_kib_post".$tMuTZ,"referensi:kd_upb",$mRo['Referensi'].":".substr($mRo['Kd_UPB'],0,11)."%","=:LIKE","","");
			$gNIb = 0;
			...
			//
			?>
			<!--tr height="23" style="cursor: pointer; vertical-align:top <?=$Blnk?>" onmouseover="this.style.cursor=&#39;pointer&#39" <?=fBackCLR($iG)?>>-->
			<tr height="40" style="cursor: pointer <?=$Blnk?>" <?=fBackCLR($iG)?>> 
              ...
              <td style="border-bottom: #999999 dotted 1px; border-right:#999999 dotted 1px; text-align:center"><? echo $mRo['Referensi'].$kdUPB?></td>
              <td style="border-bottom: #999999 dotted 1px; border-right:#999999 dotted 1px; text-align:center"><? echo $mRo['No_Register']?></td>
              <td style="border-bottom: #999999 dotted 1px; border-right:#999999 dotted 1px; padding-left:2px"><? echo $NmB.$TtK?></td>
              <td style="border-bottom: #999999 dotted 1px; border-right:#999999 dotted 1px; padding-left:2px"><? echo $mRo['Merk']?></td>
              ...
              <td width="70" style="border-bottom: #999999 dotted 1px; border-right:#999999 dotted 1px; text-align:center"><? if ($mRo['Tgl_Mutasi']!="0000-00-00") {echo fConvertDateShort($mRo['Tgl_Mutasi']);}?><?=$gAda?></td>
              <td style="border-bottom: #999999 dotted 1px; border-right:#999999 dotted 1px; padding-right:2px" align="right"><? echo fConvertToRupiah($mRo['Harga'])?></td>
              <td style="border-bottom: #999999 dotted 1px; border-right:#999999 dotted 1px; padding-right:2px <?=$ClR?>" align="right"><? echo fConvertToRupiah($gNIL)?></td>
      ...
      <tr> 
		...
		<table id="detail<? echo $iG?>" cellpadding="3" border="0" width="900" style="border:1px solid #C0C0C0; font-family: Calibri; font-size: 9pt; font-style: Italic; color: #800000; display: none; border-collapse:collapse">
            <tr> 
              ...
              <td width="10">:</td>
              <td width="200"> 
                <? echo fGlobal("Nm_Aset","Ref_Rek_Aset2","Kd_Aset",substr($mRo['Kd_Aset'],0,5),"=","","")?>              </td>
              <td width="10">&nbsp;</td>
              <td width="40">Merk</td>
              <td width="9">:</td>
              <td width="100">
		...
		?>
          ...
          <!-- Pagging -->
          <?
			$nSQL= "SELECT COUNT(*) AS JmlRc FROM ta_kib_b".$tMuTZ." WHERE extracom LIKE '$zExt' AND Kd_UPB LIKE '".$zUpb."' AND Kd_Ruang LIKE '$zRua' AND Kd_Aset LIKE '".$rBid.".".$rKel.".".$rOBJ.".".$rRin."' AND Tgl_Perolehan >= '".$gThnA."-01-01' AND Tgl_Perolehan <= '".$gThn."-12-31' ".$fFindSy.$tMuT;
...
<script language="javascript">
	...
	function P_OpenDoc(w,h,IdL)
	{
		...
		LeftPosition=(screen.width)?(screen.width-w)/2:100; 
		TopPosition=(screen.height)?(screen.height-h)/2:100;
		URL= 'KIB_B_Dokumen.php?CriT=BPK&gUnt='+gUnt+'&gSub='+gSub+'&gUpb='+gUpb+'&gBid='+gBid+'&gKel='+gKel+'&gOBJ='+gOBJ+'&gRin='+gRin+'&gExt='+gExt+'&gFin='+gFin+'&gThn='+gThn+'&gThnA='+gThnA+'&IdL='+IdL;
		settings='width='+w+',height='+h+',top='+TopPosition+',left='+LeftPosition+',location=no,directories=no,status=no,menubar=yes,toolbar=yes,resizable=ya,maximize=yes,scrollbars=yes,navigation=no';
		window.open(URL,'',settings);
	}
</script>

...
LEFT JOIN ta_rkbmd_standar_kebutuhan_rinci p2 ON p2.referensi=p1.referensi 
LEFT JOIN ref_unit p3 ON p3.kd_unit=p1.kd_unit 
WHERE p1.kd_unit LIKE '$gUnT%' AND (p1.Tanggal BETWEEN '$gTA' AND '$gTB') $CrT GROUP BY p1.referensi";
#echo $nSQ;
#return false;
$nRs = mysql_query($nSQ);
while ($mRo = mysql_fetch_array($nRs, MYSQL_BOTH))
{
	...
	<tr height="40"> 
	  ...
	  <td width="350" style="border-bottom:1px #999999 dotted; text-align:left; border-left:1px #ccc solid; padding-left:3px" <?=$gBG?>><?=$mRo[5]?></td>
	  <td width="350" style="border-bottom:1px #999999 dotted; border-left:1px #ccc solid; padding-left:3px" <?=$gBG?>><?=$mRo[6]?></td>
	  ...
	  <a href="#" onClick="showDELE('<?=$IdT?>','<?=$rCek?>','<?=$IdL?>'); return false" class="ico dell">delete</a>&nbsp;&nbsp;&nbsp;
	  <!--a href="#" onclick="P_Dokumen('800','400','<?=$IdT?>','<?=$IdL?>'); return false" class="ico prev">&nbsp;print</a-->	  </td>
    </tr>
	<?php
	$iG++;
	$tJmL=$tJmL+$mRo[7];
...
<tr height="20">
  <td style="text-align:right; font-weight:bold; padding-right:10px" colspan="7">&nbsp;</td>

...
<table align="center" cellpadding="0" class="table-listpop" cellspacing="0" width="100%" height="100%" border="0" style="font-family:arial; font-size:8pt">
	...
	while ($mRo = mysql_fetch_array($nRs, MYSQL_BOTH))
	{
		...
		if ($gTBL=='g'){
			$NiA = fGlobal("ifNull(sum(debet),0)","ta_kib_post_108","Referensi:Kd_UPB:Ref_Mutasi:Ref_Usulan",$gRF.":".$gUP."%:".$gRH.":".$gRU,"=:LIKE:=:=","","");
			if ($NiA==0){
				$NiA = fGlobal("ifNull(sum(debet),0)","ta_kib_post_108","Referensi:Kd_UPB",$gRF.":".$gUP."%","=:LIKE","","");
			}
		}
		else{
			$NiA = fGlobal("ifNull(sum(debet),0)","ta_kib_post_108","Referensi:Kd_UPB",$gRF.":".$gUP."%","=:LIKE","","");
		...
		{
			$CeK = fGlobal("IDT","ta_usulan_verifikasi_rinci_108","Kd_UPB:Referensi:Ref_Aset:Eksekusi:No_Register",$gUPB."%:".$gREF.":".$gRF.":Belum:".$gRG,"LIKE:=:=:=:=","","");
			if ($CeK!=""){
				$ico = "none";
				$aset= "none";
				$mesg= "mesgb";

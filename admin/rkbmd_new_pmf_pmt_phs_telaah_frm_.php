...
if ($fSave=='Upload')
{
	...
	$URL="rkbmd_new_pmf_pmt_phs_frm.php?Upl=YA&Frm=".$Frm."&Crit=".$Crit."&IdT=".$IdT."&IdL=".$IdL."&FrmG=".$FrmG;
	header("Location: ".$URL);
	*/
}
else if ($fSave=='Save')
{
	...
	{
		$gNeW = 1;
		$rMax = fGlobal("max(referensi)","ta_rkbmd_new_pmf_pmt_phs","referensi",$Frm.".".fGetDate('year')."%","LIKE","","");
		if ($rMax)
		{
			$gNeW = (int)substr($rMax,-8,8)+1;
		}
		$gREF = $Frm.".".fGetDate('year').".".substr(str_repeat('0',8).$gNeW,-8,8);
		...
		$IdT = fGlobal("max(IDT)","ta_rkbmd_new_pmf_pmt_phs","IDT","%","LIKE","","");
	}
	$URL="rkbmd_new_pmf_pmt_phs_frm.php?Frm=".$Frm."&Crit=".$Crit."&IdT=".$IdT."&IdL=".$IdL."&FrmG=".$FrmG;;
	header("Location: ".$URL);
}
else
{
	$URL="rkbmd_new_pmf_pmt_phs_frm.php?Frm=".$Frm."&Crit=".$Crit."&IdL=".$IdL."&FrmG=".$FrmG;;
	header("Location: ".$URL);
}
?>
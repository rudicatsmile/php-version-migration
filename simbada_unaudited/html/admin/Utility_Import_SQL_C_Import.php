...
while($mRo = sqlsrv_fetch_array($rst))
{
	...
	$gSTT = $mRo[24];
	$gLKS = $mRo[21];
	$gKOT = substr('0'.$mRo[49],-1,1).".".substr('0'.$mRo[50],-1,1).".".$mRo[51].".".substr('00'.$mRo[52],-2,2).".".substr('00'.$mRo[53],-2,2).".".substr('00'.$mRo[54],-2,2).".".substr('000'.$mRo[55],-3,3);
	$gKND = "B";
	$gTgD = "";
	if (!is_null($mRo[22]))
	{
		$gTgD = date_format($mRo[22],"Y-m-d");
	}
	$gNoD  = $mRo[23];
	...
	imprtKib($gUpb,$gRin17,$gRin108,$gNmB,$ReGA,$gTKT,$gBTN,$gTgD,$gNoD,$gLti,$gSTT,$gKOT,$gLKS,$gKND,$gGna,$gKTR,$gTgl,$gTgp,$gTglM,$gMLK,$gAUS,$gHRG,$gHRP,$gSP2,$gIDT,$iG,$conn);
	$iG++;
}

function imprtKib($gUpb,$gRin17,$gRin108,$gNmB,$ReGA,$gTKT,$gBTN,$gTgD,$gNoD,$gLti,$gSTT,$gKOT,$gLKS,$gKND,$gGna,$gKTR,$gTgl,$gTgp,$gTgM,$gMLK,$gAUS,$gHRG,$gHRP,$gSP2,$gIDT,$iG,$conn)
{
	...
	if ($CekK=='')
	{
		...
		Bertingkat='".$gTKT."',
		Beton='".$gBTN."',
		Dokumen_Tanggal='".$gTgD."',
		Dokumen_Nomor='".mysql_real_escape_string($gNoD)."',
		Luas_Lantai='".$gLti."',
		Status_Tanah='".mysql_real_escape_string($gSTT)."',
		Kode_Tanah='".$gKOT."',
		Lokasi='".mysql_real_escape_string($gLKS)."',
		Kondisi='".$gKND."',
		...
		#LOOPING
		$SQ = "SELECT Tgl_Perolehan as A0, Keterangan as A1, Harga as A2, No_SP2D as A3 FROM Ta_KIBCR WHERE IDPemda='".$gIDT."' AND harga<>'".$gHRG."' ORDER BY Tahun";
		$rQ = sqlsrv_query($conn,$SQ);
		while($mQ = sqlsrv_fetch_array($rQ))
		{
			$gTG = date_format($mQ[0],"Y-m-d");
			...
			Crit='INV',
			Tanggal='".$gTG."',
			Uraian='".mysql_real_escape_string($mQ[1])."',
			DK='D',
			Debet='".$mQ[2]."',
			Kredit='0',
			Keterangan='".mysql_real_escape_string($mQ[3])."',
			IdTabelHarga='-',
			Recorded=now(),
			Pencatat='$gPCT'";
			mysql_query($SW);

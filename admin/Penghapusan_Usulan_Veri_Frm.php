...
<form name="myfrm" method="POST" action="<?="Penghapusan_Usulan_Veri_Frm_.php?IdL=".$_GET['IdL']?>">
<input type="hidden" name="fSave" style="width:50px" />
<table border="0" class="table-link" cellspacing="0" cellpadding="0" align="center" style="width:99%; color:#fff; background:#729d65">
  <tr>
    <td width="20">&nbsp;</td>
    <td width="91">&nbsp;</td>
    <td width="16">&nbsp;</td>
  ...
  <tr height="25">
    ...
    <td align="center">&nbsp;</td>
    <td>
	<input name="fJNS" type="text" readonly value="<?=$gJNS?>" style=" width:25px; border: 1px solid #C0C0C0"/>
	<input name="dJNS" type="text" readonly value="<?=$dJNS?>" style=" width:171px; border: 1px solid #C0C0C0"/>
	</td>
    <td>FIND</td>
...
<table border="0" cellspacing="0" class="table-link" cellpadding="0" align="center" style="width:99%; height:20px; background: #C9DCD8; font-weight:bold">
  <tr>
    ...
    <td width="300" align="left">SKPD</td>
    <td width="160" align="left">NOMOR DOKUMEN</td>
    <td width="70" align="left">TGL.DOC</td>
    <td align="left">USULAN</td>
    <td width="100" align="right">NILAI</td>
...
<script languange="javascript">
	...
	function RefreshDATA(IdL)
	{
		...
		*/
		...
		{
			$("#ViewDATA").load('Penghapusan_Usulan_Veri_Frm_Data.php?gUnT='+gUnT+'&gHR='+gHR+'&gBL='+gBL+'&gTH='+gTH+'&gHRd='+gHRd+'&gBLd='+gBLd+'&gTHd='+gTHd+'&gFnD='+gFnD+'&gJNS='+gJNS+'&IdL='+IdL);
		});
		*/
		$(document).ready(function()
		{
			$.ajax({
				url:"Penghapusan_Usulan_Veri_Frm_Data.php",
	...
	function showFORM(ReO,crt,PgE,IdT,IdL)
	{
		...
		if (crt=='refr')
		{
			$(document).ready(function()
			{
				$("#formDiv2Veri").load('Penghapusan_Usulan_Veri_Frm_Veri_Mid.php?PgE='+PgE+'&IdT='+IdT+'&IdL='+IdL);
				$("#formDiv3Veri").load('Penghapusan_Usulan_Veri_Frm_Veri_Mid_Pages.php?ReO='+ReO+'&PgE='+PgE+'&IdT='+IdT+'&IdL='+IdL);
			});
			//$(document).ready(function()
			//{
			//	$("#formDiv3Veri").load('Penghapusan_Usulan_Veri_Frm_Veri_Mid_Pages.php?PgE='+PgE+'&IdT='+IdT+'&IdL='+IdL);
			//});
		}
		else
		{
			...
			$(document).ready(function()
			{
				$("#formDiv1Veri").load('Penghapusan_Usulan_Veri_Frm_Veri_Top.php?IdL='+IdL);
			});
	...
	function showEXEC_FIND(crt,PgE,IdT,IdL)
	{
		...
		$(document).ready(function()
		{
			$("#formDiv2Exec").load('Penghapusan_Usulan_Exec_Frm_Mid.php?gFnD='+gFnD+'&PgE='+PgE+'&IdT='+IdT+'&IdL='+IdL);
		});
		$(document).ready(function()
		{
			$("#formDiv3Exec").load('Penghapusan_Usulan_Exec_Frm_Mid_Pages.php?gFnD='+gFnD+'&PgE='+PgE+'&IdT='+IdT+'&IdL='+IdL);
		});
	}
	*/
	...
	function showEXEC_RefR(crt,PgE,IdT,IdL)
	{
		...
		{
			$.ajax({
				url:"Penghapusan_Usulan_Exec_Frm_Mid.php",
	...
	function showEXEC(ReO,crt,PgE,IdT,IdL)
	{
		...
		$(document).ready(function()
		{
			$("#formDiv1Exec").load('Penghapusan_Usulan_Exec_Frm_Top.php?crt='+crt+'&gFnD='+gFnD+'&PgE='+PgE+'&IdT='+IdT+'&IdL='+IdL);
		});
	...
	function SaveDATA(ReO,PgE,rID,IdT,gIdL)
	{
		...
		{
			$.ajax({
				url:"Penghapusan_Usulan_Veri_Frm_Veri_Save.php",
				data: {ReO:ReO,PgE:PgE,gNm:gNm,gJb:gJb,gNi:gNi,nO:nO,mE:mE,gH:gH,gB:gB,gT:gT,rID:rID,IdT:IdT,IdL:gIdL},
				type:"get",
				beforeSend:function()
				{
					$("#loadingImg").show();
	...
	function unexecFORMRepairKib(MdL,PgE,tID,IdT,IdL)
	{
		...
		$(document).ready(function()
		{
			$("#ViewDELL").load('Penghapusan_Usulan_Exec_Frm_Save_RepairKib.php?MdL='+MdL+'&PgE='+PgE+'&tID='+tID+'&IdT='+IdT+'&IdL='+IdL);
			//$("#formDiv2Exec").load('Penghapusan_Usulan_Exec_Frm_Save_RepairKib.php?MdL='+MdL+'&PgE='+PgE+'&tID='+tID+'&IdT='+IdT+'&IdL='+IdL);
		});
	}
	...
	function removeFORM(PgE,tID,IdT,IdL)
	{
		...
		if (AN)
		{
			...
			{
				//$("#ViewDELL").load('Penghapusan_Usulan_Exec_Frm_Save_RepairKib.php?MdL='+MdL+'&PgE='+PgE+'&tID='+tID+'&IdT='+IdT+'&IdL='+IdL);
				$("#formDiv2Exec").load('Penghapusan_Usulan_Exec_Frm_Save_RemoveRec.php?PgE='+PgE+'&tID='+tID+'&IdT='+IdT+'&IdL='+IdL);
			});
		}
	}
	...
	function unexecFORM(DeM,WrM,MdL,PgE,tID,IdT,IdL)
	{
		...
		if (AN)
		{
			...
			$(document).ready(function()
			{
				//$("#ViewDELL").load('Penghapusan_Usulan_Exec_Frm_Save.php?rBatal='+rBatal+'&PgE='+PgE+'&tID='+tID+'&IdT='+IdT+'&IdL='+IdL);
				$("#formDiv2Exec").load('Penghapusan_Usulan_Exec_Frm_Save.php?rBatal='+rBatal+'&PgE='+PgE+'&tID='+tID+'&IdT='+IdT+'&IdL='+IdL);
			});
			*/
			...
			{
				$.ajax({
					url:"Penghapusan_Usulan_Exec_Frm_Save.php",
	...
	function editFORM(crt,mS,tID,gIdL)
	{
		...
		if (crt=='refr')
		{
			$(document).ready(function()
			{
				$("#editDiv2Veri").load('Penghapusan_Usulan_Veri_Frm_Veri_Mid_Edit_Mid.php?tID='+tID+'&IdL='+gIdL);
			});
		}
		else
		{
			...
			$(document).ready(function()
			{
				$("#editDiv1Veri").load('Penghapusan_Usulan_Veri_Frm_Veri_Mid_Edit_Top.php?IdL='+gIdL);
			});
	...
	function saveRECO(fLD,rVal,mID,tID,gIdL)
	{
		...
		$(document).ready(function()
		{
			$("#editDiv2Veri").load('Penghapusan_Usulan_Veri_Frm_Veri_Mid_Edit_Save.php?gVer='+gVer+'&fLD='+fLD+'&rVal='+rVal+'&mID='+mID+'&tID='+tID+'&IdL='+gIdL);
		});
	}
	...
	function saveVERI(fLD,JnS,rVal,tID,gIdL)
	{
		...
		$(document).ready(function()
		{
			$("#editDiv2Veri").load('Penghapusan_Usulan_Veri_Frm_Veri_Mid_Edit_Save.php?gVer='+gVer+'&fLD='+fLD+'&rVal='+rVal+'&tID='+tID+'&IdL='+gIdL);
		});
	}
	...
	function showIMG(CrT,tID,gIdL)
	{
		...
		$(document).ready(function()
		{
			$("#imgDiv1Cri").load('Penghapusan_Usulan_Veri_Frm_Data_Img_Top.php?CrT='+CrT+'&IdL='+gIdL);
		});
		
		$(document).ready(function()
		{
			$("#imgDiv2Cri").load('Penghapusan_Usulan_Veri_Frm_Data_Img_Mid.php?CrT='+CrT+'&tID='+tID+'&IdL='+gIdL);
		});
	}
	...
	function repairDATA(CrT,TbS,TbT,PgE,tID,IdT,IdL)
	{
		$(document).ready(function()
		{
			$("#formDiv2Exec").load('Penghapusan_Usulan_Exec_Frm_Save_RepairNew.php?CrT='+CrT+'&TbS='+TbS+'&TbT='+TbT+'&PgE='+PgE+'&tID='+tID+'&IdT='+IdT+'&IdL='+IdL);
		});
	}
	...
	function creatorEXE(Ref,IdL)
	{
		...
		$(document).ready(function()
		{
			$("#exceDiv1Veri").load('Penghapusan_Usulan_Veri_Frm_Veri_Crea_Top.php?IdL='+IdL);
		});
		
		$(document).ready(function()
		{
			$("#exceDiv2Veri").load('Penghapusan_Usulan_Veri_Frm_Veri_Crea_Mid.php?Ref='+Ref+'&IdL='+IdL);
		});
	...
	function exeALL(Ref,IdL)
	{
		...
		if (AN)
		{
			...
			$(document).ready(function()
			{
				$("#ViewDELL").load('Penghapusan_Usulan_Veri_Frm_Veri_Crea_Save.php?ProT='+ProT+'&Ref='+Ref+'&mHri='+mHri+'&mBln='+mBln+'&mThn='+mThn+'&IdL='+IdL);
			...
			*/
			...
			{
				$.ajax({
					url:"Penghapusan_Usulan_Veri_Frm_Veri_Crea_Save.php",
	...
	function execFORM_All(eMA,ExecAll,RefU,PgE,IdT,IdL)
	{
		gFnD = ReplaceText(objfrm.fFnDT.value);
		
		if (ExecAll!=''){
			alert(ExecAll+'->Proses tidak bisa dilanjutkan, karena ada transaksi yg belum difasilitasi ..!!'); return false;
		}
		var AN = confirm(eMA+" semua sesuai record halaman ini..?!!");
		if (AN)
		{
			ProsT=2;
			if (ProsT==1)
			{
				$(document).ready(function()
				{
					$("#formDiv2Exec").load('Penghapusan_Usulan_Exec_Frm_Exe_All_Save.php?eMA='+eMA+'&gFnD='+gFnD+'&PgE='+PgE+'&RefU='+RefU+'&IdT='+IdT+'&IdL='+IdL);
				});
			}
			else
			{
				...
				{
					$.ajax({
						url:"Penghapusan_Usulan_Exec_Frm_Exe_All_Save.php",
						data: {eMA:eMA,gFnD:gFnD,PgE:PgE,RefU:RefU,IdT:IdT,IdL:IdL},
						type:"get",
						beforeSend:function()
						{
							$("#loadingImg3").show();
	...
	function execFORM(LeNon,DeN,PgE,KdA,JenS,tbl,wrn,exc,tID,IdT,IdL)
	{
		...
		if (AN)
		{
			...
			if (ProsT==1){
				$(document).ready(function()
				{
					$("#formDiv2Exec").load('Penghapusan_Usulan_Exec_Frm_Save.php?PgE='+PgE+'&tID='+tID+'&IdT='+IdT+'&IdL='+IdL);
					//$("#ViewDELL").load('Penghapusan_Usulan_Exec_Frm_Save.php?PgE='+PgE+'&tID='+tID+'&IdT='+IdT+'&IdL='+IdL);
				});
			}
			else
			{

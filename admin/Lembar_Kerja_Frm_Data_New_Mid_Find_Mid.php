...
<table align="center" cellpadding="0" class="table-listpop" cellspacing="0" width="100%" height="240" border="0">
	<?php
	$iG=1;
	$nSQ = "SELECT Kd_Aset, Nm_Aset FROM ref_rek_aset108_7 WHERE (Kd_Aset LIKE '1.3.%' OR Kd_Aset LIKE '1.5.3.%' OR Kd_Aset LIKE '1.5.4.%') $CrT ORDER BY Kd_Aset LIMIT 0,200";
	#echo $nSQ;
	$nRs = mysql_query($nSQ);
	while ($mRo = mysql_fetch_array($nRs, MYSQL_BOTH))
	{
		...
		$gNm = $mRo[1];
		?>
		<tr height="20" onclick="closeCLICK('rekn','<?=$gKd?>','<?=$gNm?>','<?=ReO?>','<?=$IdT?>','<?=$IdL?>'); return false;">
			<td width="10" style="border-bottom:1px dotted #CCCCCC">&nbsp;</td>
			<td width="105" style="border-bottom:1px dotted #CCCCCC"><?=$gKd?></td>

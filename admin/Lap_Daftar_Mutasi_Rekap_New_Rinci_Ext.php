<?php
	function ViewRciObjek($KdRCI,$mUpb,$mThn,$nThn,$mG,$LoadMutasi,$LoadExtrac)
	{
		...
		while ($mRoE = mysql_fetch_array($nRsE, MYSQL_BOTH))
		{
			...
			#if (fNmHuruf($mG)=="f"){
			#	if ($nThn<=2016) {
			#		$Col[5] = fGlobal("IfNull(count(*),0)", "ta_kib_1018","Kd_Aset_108:Kd_Upb:KdpToAset:Tgl_Perolehan",$KdAsT."%:".$mUpb."%:N:".$mThn."-12-31","LIKE:LIKE:=:<=","","");
			...
			#	}
			#	else{
			#		$Col[5] = fGlobal("unitAkhir", "ta_kib_post_saldo_mutasi","Kd_Unit:Kd_Aset_108:Tahun",$mUpb.":".$KdAsT.":".$mThn,"=:=:=","","");
			#		$Col[6] = fGlobal("saldoAkhir", "ta_kib_post_saldo_mutasi","Kd_Unit:Kd_Aset_108:Tahun",$mUpb.":".$KdAsT.":".$mThn,"=:=:=","","");
			#	}
			#}
			#else{
				if ($nThn<=2026) {

...
<form name="myfrm" method="post" action="<?="Ref_Kode_Aset108_3_.php?FrmG=".$_GET['FrmG']."&IdL=".$_GET['IdL']."&Page=".$_GET['Page']."&iG=".$_GET['iG']."&KdAst1=".$KdAst1."&KdAst2=".$KdAst2 ?>">
  ...
  <table align="center" border="0" cellspacing="0" cellpadding="0" class="table-list" style="width:99%">
      <tr> 
        ...
        <th width="170" class="ac"><?php if ($KdAst1=="02" || $KdAst1=="03" || $KdAst1=="04") {echo "Umur Ekonomis";}?></th>
        <th width="138" class="ac"><?php if ($KdAst1=="02" || $KdAst1=="03" || $KdAst1=="04") {echo "Tabel Atribusi";}?></th>
      ...
      <?php
		...
		$nSQL= "SELECT * FROM ref_rek_aset108_4 WHERE Kd_Aset LIKE '".$KdAst2.".__' ORDER BY Kd_Aset LIMIT $Offset, $DataPerPage";
		...
		do
			...
			{
				$mSQL= "SELECT * FROM ref_rek_aset108_5 WHERE Kd_Aset LIKE '".$mRo['Kd_Aset'].".__' ORDER BY Kd_Aset";
      ...
      <tr height="27" style="cursor: pointer" title="clik disini untuk melihat rincian...!" onmouseover="this.style.cursor=&#39;pointer&#39"> 
        ...
        <td style="border-bottom: 1px dotted #CCCCCC" class="ac" onclick="$(&#39;#detail<?=$iG?>&#39;).toggle(&#39;past&#39;)" <?=fBackCLR($iG)?>><?php if ($mRo['Ms_Manfaat']>0) {echo $mRo['Ms_Manfaat']." Tahun";}?></td>
        <td style="border-bottom: 1px dotted #CCCCCC" class="ac" onclick="$(&#39;#detail<?=$iG?>&#39;).toggle(&#39;past&#39;)" <?=fBackCLR($iG)?>><?=$gADA?></td>
        <td style="border-bottom: 1px dotted #CCCCCC" class="ac" <?=fBackCLR($iG)?>>
		[&nbsp;<a href="<?="Ref_Kode_Aset108_4.php?FrmG=".$_GET['FrmG']."&IdL=".$_GET['IdL']."&KdAst1=".$_GET['KdAst1']."&KdAst2=".$_GET['KdAst2']."&KdAst3=".$mRo['Kd_Aset']?>" class="ico prev">VIEW</a>&nbsp;]&nbsp;&nbsp;&nbsp;&nbsp;
      ...
      <tr height="30">
		...
		[&nbsp;<a href="<?=$_SERVER['PHP_SELF']."?FrmG=".$_GET['FrmG']."&IdL=".$_GET['IdL']."&Page=".$_GET['Page']."&iG=".$_GET['iG']."&KdAst1=".$KdAst1."&KdAst2=".$KdAst2?>" class="ico reff">&nbsp;REFRESH</a>&nbsp;]&nbsp;&nbsp;&nbsp;[&nbsp;
		<a href="#" onclick="P_Document('800','400','center','<?=$KdAst2?>');return false" class="ico docu">&nbsp;DOKUMENT</a>&nbsp;]		</td>
        <td style="border-top:1px solid #CCCCCC">&nbsp;</td>
      </tr>
    </table>
	  <?php
		$nSQL= "SELECT COUNT(*) AS JmlRc FROM ref_rek_aset108_4 WHERE Kd_Aset LIKE '".$KdAst2.".__'";
		$fUlrR= "FrmG=".$_GET['FrmG']."&IdL=".$_GET['IdL']."&KdAst1=".$KdAst1."&KdAst2=".$KdAst2."&";
...
<script language="javascript">
			...
			if (win!=null)
				{
					...
					<?php
						$URL_Top = "Form_Kode_Aset108_3_Top.php?FrmG=".$_GET['FrmG']."&IdL=".$_GET['IdL'];
						$URL_Mid = "Form_Kode_Aset108_3_Mid.php?KdAst2=".$KdAst2."&IdL=".$_GET['IdL'];
						$URL_Bot = "Form_Kode_Aset108_3_Bot.php";
					?>       			
	...
	function EditData(w,h,Lev,IDT)
	{
		...
		{
			win.window.document.open()       			
			URL_Top = "Form_Kode_Aset108_3_Top.php?"+"<?="FrmG=".$_GET['FrmG']?>";
			URL_Mid = "Form_Kode_Aset108_3_Mid.php?rIDT="+IDT+"<?="&IdL=".$_GET['IdL']?>";
			URL_Bot = "Form_Kode_Aset108_3_Bot.php";
			txtHTML="<html><head><title><?php require_once 'AppTitle.php'; echo APP_TITLE; ?></title></head><frameset framespacing='0' border='0' rows='45,*,30' frameborder='0'><frame name='WinFormKIB_Top' noresize src='"+URL_Top+"' scrolling='no'><frame name='WinFormKIB_Mid' src='"+URL_Mid+"' scrolling='auto'><frame name='WinFormKIB_Bot' src= '"+URL_Bot+"' scrolling='no'><noframes><body><p>=>.............??!</p></body></noframes></frameset></html>"            
			win.focus()
			win.window.document.clear()
	...
	function P_Document(w,h,pos,gKD)
	{
		...
		LeftPosition=(screen.width)?(screen.width-w)/2:100; 
		TopPosition=(screen.height)?(screen.height-h)/2:100;
		URL = "Ref_Kode_Aset108_Doc.php?CrT=JNS&gKD="+gKD+"<?="&IdL=".$_GET['IdL']?>";

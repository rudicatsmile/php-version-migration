...
<table border="0" class="table-link" cellspacing="0" cellpadding="0" align="center" style="width:99%; background:#CCCCCC">
  ...
  <tr height="25">
	...
	<input name="dUNT" type="text" value="<?=$dUNT?>" <? if (!$IdT) {?> onClick="showUNIT('<?=$stLOCK?>','','<?=$_GET['IdL']?>')" onkeypress="if (event.keyCode==13) {showUNIT('<?=$stLOCK?>','find','<?=$_GET['IdL']?>'); return false;} else if (event.keyCode==27) {closeCLICK('unit'); return false;}" <? } else {echo "readonly";}?> style="padding-left:5px; width:372px; border: 1px solid #C0C0C0; text-transform:uppercase"/>

...
while($mRo = sqlsrv_fetch_array($rst))
{
	...
	$SQa = "SELECT sum(harga) FROM Ta_KIBCR WHERE IDPemda='".$mRo[0]."' AND harga<>'".$mRo[27]."'";
	#echo $SQa."<br>";
	$rsa = sqlsrv_query($conn,$SQa);
	$mRa = sqlsrv_fetch_array($rsa);
	
	$SQb = "SELECT harga FROM ta_fn_kib_c WHERE IDPemda='".$mRo[0]."' AND Tahun='2025'";
	#echo $SQb."<br>";
	...
	<tr height="20">
	  ...
	  <td style="border-right:1px solid #ccc; border-bottom:1px solid #ccc; padding-left:2px"><?=$mRo[58].":".$mRo[59].":".$mRo[60]?></td>
	  <td style="border-right:1px solid #ccc; border-bottom:1px solid #ccc; text-align:right; padding-right:2px"><?=fConvertToRupiahBulat($mRo[27])?></td>
	  ...
	  <td style="border-bottom:1px solid #ccc; text-align:right; padding-right:2px"><?=fConvertToRupiahBulat($mRb[0])?></td>
	</tr>
	<?
	}
	?>

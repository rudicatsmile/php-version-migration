...
if ($AmBiL=="ALL")
{
	...
	if ($gTHN==""){$gTHN="____";}
	#$nSQ = "SELECT Referensi,Kd_UPB,Kd_Aset,No_Register,Nm_Aset,Tgl_Perolehan,Keterangan,Harga,IDT,Ref_Mutasi FROM ta_kib_".$rTbL." 
	#WHERE Kd_UPB LIKE '$gUNT%' AND Tgl_Perolehan LIKE '$gTHN-%-%' AND MasukKeUsulan='belum' $CrT $nMB ORDER BY Kd_Aset, No_Register, Tgl_Perolehan LIMIT 0,$gLST";
	...
	WHERE referensi LIKE '$eRf%' AND Kd_UPB LIKE '$gUNT%' AND Tgl_Perolehan LIKE '$gTHN-%-%' AND MasukKeUsulan LIKE '%' $CrT $nMB ORDER BY Kd_Aset_108, No_Register, Tgl_Perolehan LIMIT 0,$gLST";
	#echo $nSQ;
	#return false;
}
else
{
	...
	$nSQ = "SELECT Referensi,Kd_UPB,Kd_Aset_108,No_Register,Nm_Aset,Tgl_Perolehan,Keterangan,Harga,IDT,Ref_Mutasi,Ref_Usulan,extracom FROM ta_kib_108 WHERE (".$SyT.") ORDER BY IDT";
}
$nRs = mysql_query($nSQ);
while ($mRo = mysql_fetch_array($nRs, MYSQL_BOTH))
{
	...
	if (!$CeK)
	{
		...
		$eSQL= "UPDATE ta_kib_".$rTbL." SET MasukKeUsulan='sudah' WHERE IDT='".$eIdT."'";

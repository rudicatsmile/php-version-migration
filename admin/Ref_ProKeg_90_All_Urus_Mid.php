...
if ($FnD)
{
	$CrT ="WHERE (Kode LIKE '%$FnD%' OR Deskripsi LIKE '%$FnD%')";
}
?>
<table align="center" cellpadding="0" class="table-listpop" cellspacing="0" width="100%" height="240" border="0">
	<?php
	$iG=1;
	$nSQ = "SELECT Kode, Deskripsi FROM ref_keg_90_1 $CrT ORDER BY Kode";
	#echo $nSQ;
	$nRs = mysql_query($nSQ);
	while ($mRo = mysql_fetch_array($nRs, MYSQL_BOTH))
	{
		...
		$gBG  = fBackCLR($iG);
		?>
		<tr height="22" onclick="showCLICK('urus','','','<?=$gKd?>','<?=$gNm?>','<?=$IdL?>'); return false;">
			<td width="10" <?=$gBG?> style="border-bottom:1px dotted #CCCCCC">&nbsp;</td>
			<td width="30" <?=$gBG?> style="border-bottom:1px dotted #CCCCCC"><?=$gKd?></td>

...
//if (isset($_GET['gThn2'])) {$gThn2  = $_GET['gThn2'];} else {$gThn2  ="2018";}
if (isset($_GET['gBid'])) {
	$gBid  = $_GET['gBid'];
} else {
	$gBid  = "";
...
<body>
	...
	<form name="myfrm" method="post" action="<?php echo "KIB-E_.php?FrmG=" . $_GET['FrmG'] . "&IdL=" . $_GET['IdL'] ?>">
		...
		<table border="0" align="center" style="width:99%">
			...
			<tr>
				...
				<td>&nbsp;</td>
				<td align="right"><input type="button" name="B37" disabled value="EXPORT DATA" onclick="P_ToExcel('800','400','center','ToExcel/KIB_E_ToExcel')" style="width: 90px; height: 24px; padding-left: 0px; padding-right: 0px; padding-top: 0px; padding-bottom: 0px" /></td>
			</tr>
			<tr>
				<td>TAHUN</td>
				<td valign="middle">
			...
			?>
							...
							<td width="8%">FIND</td>
							<td width="35%"> <input class="text" type="text" name="fFind" id="fFind" value="<? echo $gFin ?>" placeholder='Search' style=" width:200px; font-family: Calibri; font-size: 10pt; padding-left: 4px; padding-right: 4px; padding-top: 1px; padding-bottom: 1px; background-color: #E1F986" /></td>
							<td width="1%">&nbsp;</td>
			...
			<tr>
				<td>&nbsp;</td>
				<td>&nbsp;</td>
				<td>ASET / EXTRACOM<div id="loadingImg" style="width:40px;height:10px;display:none"><img src="Images/loading3.gif" alt="" width="40" height="40"></div>
				</td>
				<td>
		...
		<table border="0" align="center" id="tbViewData" style="width:99%">
			<tr>
				<td>
					<div id="ViewDATA" style="height:100px; width:500px; overflow:auto; display:none"></div>
				</td>
			</tr>
		</table>
		<table border="0" align="center" class="table-list" cellspacing="0" cellpadding="0" style="width:99%">
			<tr>
				<th width="33" class="ac">No</th>
				<th width="100" class="ac">Referensi</th>
				<th width="60" class="ac">Register</th>
				<th width="280" align="left">Nama Barang</th>
				<th align="left">Uraian</th>
				<th width="72" class="ac">Perolehan</th>
			...
			if ($gFin != "") {
				$fFindSy = "AND (No_Register LIKE '%" . $gFin . "%' OR Nm_Aset LIKE '%" . $gFin . "%' OR Harga LIKE '%" . $gFin . "%' OR Referensi LIKE '%" . $gFin . "%' OR Kd_Aset_108 LIKE '%" . $gFin . "%' OR Keterangan LIKE '%" . $gFin . "%' OR Ref_Temp LIKE '%" . $gFin . "%')";
			} else {
				$fFindSy = "";
			}
			...
			###################
			/*
		$nSQA= "SELECT Referensi, Ref_Group, No_Register, Kd_UPB, Kd_Aset_108, Tgl_Perolehan, Harga, No_Pengadaan FROM ta_kib_e WHERE Kd_UPB LIKE '".substr($zUpb,0,11)."%' AND Tgl_Perolehan LIKE '2017-__-__' ORDER BY Kd_Aset_108, No_Register";
		...
		*/
			...
			$nSQL = "SELECT * FROM ta_kib_108" . $tMuTZ . " WHERE referensi LIKE '$gRf%' AND extracom LIKE '" . $zExt . "' AND Kd_UPB LIKE '" . $zUpb . "' AND Kd_Aset_108 LIKE '" . $rBid . "." . $rKel . "." . $rOBJ . "." . $rRin . "' AND Tgl_Perolehan LIKE '" . $rThn . "-__-__' " . $fFindSy . $tKdBar . " ORDER BY Kd_Aset_108, IDT, No_Register LIMIT $Offset, $DataPerPage";
			#echo $nSQL;
			...
			if ($tRo > 0) {
				do {
					...
					$gHRG = $mRo['Harga'];
					$gKDA = $mRo['Kd_Aset_108'];
					$gREG = $mRo['No_Register'];
					$gNIa = fGlobal("IfNull(sum(Debet),0)", "Ta_KIB_Post_108" . $tMuTZ, "Referensi:Kd_UPB:Kd_Aset_108:No_Register", $mRo['Referensi'] . ":" . substr($mRo['Kd_UPB'], 0, 11) . "%:" . $gKDA . ":" . $gREG, "=:LIKE:=:=", "", "");
					$gNIb = 0;
					if ($gNIb != 0) {
						$gNIL = $gNIa - $gNIb;
					} else {
			...
			if ($ImPORT=='YA')
			{
				$aTR = fGlobal("count(*)","atribusi_e","kode_skpd:register_new",$gSK.":".$mRo['No_Register'],"=:=","","");
				$aTP = fGlobal("count(*)","ta_kib_post_108","referensi:kd_upb",$mRo['Referensi'].":".$mRo['Kd_UPB'],"=:=","","");
				$aHR = fGlobal("IfNull(sum(Harga_Bertambah),0)","atribusi_e","kode_skpd:register_new",$gSK.":".$mRo['No_Register'],"=:=","","");
				
				if ($aTR > 1 && $aTR > $aTP && $aHR==$gNIL)
				{
					callImportAtribusi_CDE('atribusi_e',$mRo['No_Register'],$mRo['Referensi'],$mRo['Kd_Aset_108'],$mRo['Kd_UPB'],$Dipla,$Exec);
				}
			}
			*/
			...
			?>
					<tr height="40" style="cursor: pointer <?= $Blnk ?>" onmouseover="this.style.cursor=&#39;pointer&#39" <?= fBackCLR($iG) ?>>
						...
						<td style="border-bottom: #999999 dotted 1px; border-right:#999999 dotted 1px; text-align:center"><? echo $mRo['Referensi'] . $kdUPB . $RfPTA ?></td>
						<td style="border-bottom: #999999 dotted 1px; border-right:#999999 dotted 1px; text-align:center"><? echo $mRo['No_Register'] . "<br>" . $aTR ?></td>
						<td style="border-bottom: #999999 dotted 1px; border-right:#999999 dotted 1px; padding-left:2px; padding-right:2px"><? echo $NmB . $TtK ?></td>
		...
		#echo $nSQL;
		$fUlrR = "FrmG=" . $_GET['FrmG'] . "&IdL=" . $_GET['IdL'] . "&gMuT=" . $gMuT . "&eMuT=" . $eMuT . "&gExt=" . $gExt . "&gFin=" . $gFin . "&gUnt=" . $zUnt . "&gSub=" . $gSub . "&gUpb=" . $gUpb . "&gThn=" . $gThn . "&gBid=" . $gBid . "&gKel=" . $gKel . "&gOBJ=" . $gOBJ . "&gRin=" . $gRin . "&gKdBar=" . $gKdBar . "&";
...
<script language="javascript">
	...
	function InputData(w, h, pos) {
		...
		if (win != null) {
			...
			<?
			$URL_Top = "Form_Asset_E_Top.php?FrmG=" . $_GET['FrmG'] . "&IdL=" . $_GET['IdL'];
			$URL_Mid = "Form_Asset_E_Mid.php?gUnt=" . $zUnt . "&gSub=" . $zSub . "&gUpb=" . $zUpb . "&gBid=" . $zBid . "&gKel=" . $zKel . "&gOBJ=" . $zOBJ . "&gRin=" . $zRin . "&IdL=" . $_GET['IdL'];
			$URL_Bot = "Form_Asset_E_Bot.php";
			?>
	...
	function OpenKIB(w, h, pos) {
		...
		if (win != null) {
			...
			$URL_Mid = "Open_KIB_Mid.php?CrKIB=KIB_E&gUnt=" . $zUnt . "&gSub=" . $zSub . "&gUpb=" . $zUpb . "&IdL=" . $_GET['IdL'];
	...
	function OpenAccNumb(w, h, pos) {
		...
		if (win != null) {
			win.window.document.open()
			<?
			$URL_Top = "Find_Acc_Top.php?CrAcc=KIB-E&FrmG=" . $_GET['FrmG'] . "&gUnt=" . $zUnt . "&gSub=" . $zSub . "&gUpb=" . $zUpb . "&IdL=" . $_GET['IdL'];
			$URL_Mid = "Find_Acc_Mid.php?CrAcc=KIB-E&FrmG=" . $_GET['FrmG'] . "&gUnt=" . $zUnt . "&gSub=" . $zSub . "&gUpb=" . $zUpb . "&IdL=" . $_GET['IdL'];
	...
	function EditData(w, h, pos, IDT) {
		...
		if (win != null) {
			win.window.document.open()
			URL_Top = "Form_Asset_E_Top.php?" + "<?= "FrmG=" . $_GET['FrmG'] ?>";
			URL_Mid = "Form_Asset_E_Mid.php?rIDT=" + IDT + "<?= "&IdL=" . $_GET['IdL'] ?>";
			URL_Bot = "Form_Asset_E_Bot.php";
	...
	function P_OpenDoc(w, h, IdL) {
		...
		LeftPosition = (screen.width) ? (screen.width - w) / 2 : 100;
		TopPosition = (screen.height) ? (screen.height - h) / 2 : 100;
		URL = 'KIB_E_Count.php?CriT=BPK&gUnt=' + gUnt + '&gSub=' + gSub + '&gUpb=' + gUpb + '&gBid=' + gBid + '&gKel=' + gKel + '&gOBJ=' + gOBJ + '&gRin=' + gRin + '&gExt=' + gExt + '&gFin=' + gFin + '&gThn=' + gThn + '&IdL=' + IdL;
		settings = 'width=' + w + ',height=' + h + ',top=' + TopPosition + ',left=' + LeftPosition + ',location=no,directories=no,status=no,menubar=yes,toolbar=yes,resizable=ya,maximize=yes,scrollbars=yes,navigation=no';
		window.open(URL, '', settings);
	}

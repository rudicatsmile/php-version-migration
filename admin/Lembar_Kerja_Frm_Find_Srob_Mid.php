...
<table align="center" cellpadding="0" class="table-listpop" cellspacing="0" width="100%" height="240" border="0">
	<tr height="18" onclick="showCLICK('srob','0.0.0.00.00.00','ALL','<?=$IdL?>'); return false;">
		<td>&nbsp;</td>
	...
	<?php
	$iG=1;
	$nSQ = "SELECT Kd_Aset, Nm_Aset FROM ref_rek_aset108_6 WHERE Kd_Aset LIKE '".$MsT."%' $CrT ORDER BY Kd_Aset";
	#echo $nSQ;
	$nRs = mysql_query($nSQ);
	while ($mRo = mysql_fetch_array($nRs, MYSQL_BOTH))
	{
		...
		$gNm = $mRo[1];
		?>
		<tr height="20" onclick="showCLICK('srob','<?=$gKd?>','<?=$gNm?>','<?=$IdL?>'); return false;">
			<td width="10" style="border-bottom:1px dotted #CCCCCC">&nbsp;</td>
			<td width="90" style="border-bottom:1px dotted #CCCCCC"><?=$gKd?></td>

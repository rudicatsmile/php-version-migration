...
if ($gLmT=="YA")
{
	$nSQL= "SELECT * FROM ta_kib_c WHERE Kd_UPB LIKE '".$gUpb."' AND Kd_Aset LIKE '".$gAss."' AND (Tgl_Perolehan BETWEEN '".$gTgA."' AND '".$gTgB."') ".$fFindSy." ORDER BY Tgl_Perolehan, Kd_Aset, No_Register LIMIT ".$gReC.",10000";
}
else
{
	$nSQL= "SELECT * FROM ta_kib_c WHERE Kd_UPB LIKE '".$gUpb."' AND Kd_Aset LIKE '".$gAss."' AND (Tgl_Perolehan BETWEEN '".$gTgA."' AND '".$gTgB."') ".$fFindSy." ORDER BY Tgl_Perolehan, Kd_Aset, No_Register";
}
...
header("Content-Type: application/x-msexcel; name=\"KiB-C Choise.xls\"");
header("Content-Disposition: inline; filename=\"KiB-C Choise.xls\"");
$fh=fopen($fname, "rb");
fpassthru($fh);
unlink($fname);
?>
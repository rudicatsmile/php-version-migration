...
if ($Smp=="Save")
	{
			...
			if ($gGRP!="")
			{
				if ($gChoiceGRP=="ON")		//PILIHAN EDIT GROUP ATAU TIDAK
				{
					...
					else if ($gSTN > $gSTN_Old)		//JIKA JUMLAH ITEM YANG BARU LEBIH BESAR MAKA TAMBAH RECORD BARU
					{
						...
						$NewG = fGlobal("max(no_register)","ta_kib_108","referensi:no_register NOT","ATB%:%HEX%","LIKE:LIKE","","");
		...
		else
		{
			...
			{
				//CARI REGISTER RERAKHIR BARANG DI TABEL KIB
				$NewReGAset = MakeNewNoRegiterAuto('ATB','');
...
$URL="Form_Asset_J_Mid.php?rIDT=".$rIDT."&IdL=".$IdL."&gUnt=".$gUnt."&gSub=".$gSub."&gUpb=".$gUpb."&gThn=".$gThn."&gBid=".$gBid."&gKel=".$gKel."&gOBJ=".$gOBJ."&gRin=".$gRin."&gFin=".$gFin;
header("Location: ".$URL);
?>

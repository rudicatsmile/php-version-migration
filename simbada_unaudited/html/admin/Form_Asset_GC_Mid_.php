				...
				if ($gChoiceGRP=="ON")		//PILIHAN EDIT GROUP ATAU TIDAK
				{
					...
					else if ($gSTN < $gSTN_Old)		//JIKA JUMLAH ITEM YANG BARU LEBIH KECIL, MAKA ADA PENGHAPUSAN RECORD SEJUMLAH PERBEDAANYA
					{
						...
						if ($SisaRec > 1)			//MASIH DALAM KORIDOR ENTRY GROUP
						{
							...
							{
								//$DeLIDT = fGlobal("IDT","ta_kib_g","Ref_Group",$gGRP,"=","IDT desc limit 1","");
								//$SQL = "delete from ta_kib_g where IDT='".$DeLIDT."'";
								//$rst = mysql_query($SQL) or die(mysql_error());
...
$URL="Form_Asset_GC_Mid.php?rIDT=".$rIDT."&IdL=".$IdL."&gUnt=".$gUnt."&gSub=".$gSub."&gUpb=".$gUpb."&gThn=".$gThn."&gBid=".$gBid."&gKel=".$gKel."&gOBJ=".$gOBJ."&gRin=".$gRin."&gFin=".$gFin;
header("Location: ".$URL);
?>

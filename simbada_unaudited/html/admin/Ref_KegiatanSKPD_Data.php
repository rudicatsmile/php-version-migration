...
$rToT = 0;
if ($gPRG){
	$nSQ="SELECT idt,idKegiatan,idReferensi,nmKegiatan FROM ta_apbd_kegiatan_skpd 
	WHERE kdUnit='".$gUNT."' AND idKegiatan LIKE '$gPRG%' AND periode='$gTHN' AND apbd='$gAPB' ORDER BY idKegiatan";
}
else{
	$nSQ="SELECT idt,idKegiatan,idReferensi,nmKegiatan FROM ta_apbd_kegiatan_skpd 
	WHERE kdUnit='".$gUNT."' AND idKegiatan LIKE 'XXXXXXXXXXXXXXX' AND periode='$gTHN' ORDER BY idKegiatan";
}
$nRs = mysql_query($nSQ);
#echo $nSQ;

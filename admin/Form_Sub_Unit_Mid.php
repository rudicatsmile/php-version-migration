...
if ($rIDT!="")
{
	$nSQ = "SELECT * FROM ref_sub_unit WHERE IDT='".$rIDT."'";
	...
	if ($tRo > 0)
	{
		...
		$gUnt = substr($gCod,0,11);
	}
}
else
{
	if ($gUnt=="") {$gUnt=$KdProp.".".$KdKabK.".XX";}
	$gCod  = $gUnt.".XX";
...
?>
<body>
<form name="myfrm" method="post" action="<?php echo "Form_Sub_Unit_Mid_.php?IdL=".$_REQUEST['IdL']."&rIDT=".$rIDT ?>">
  ...
  <table border="0" width="663" cellspacing="1" style="font-family: Calibri; font-size: 10pt; border-collapse: collapse">
    ...
    <tr>
      ...
      <td>:</td>
      <td>
	  <select class="boxs" name="fBid" tabindex="0" style="width: 480px" <?php echo $Sbmt?>>
        <?php
		if ($Lev > 1)
    ...
    <tr>
      ...
      <td>UNIT KERJA </td>
      <td>&nbsp;</td>
      <td><select class="boxs" name="fUnt" tabindex="0" style="width: 480px" <?php echo $Sbmt?>>
	  <option value=""></option>
        <?php
		if ($Lev > 1)
		{$nSQ = "SELECT * FROM ref_unit where Kd_Unit like '".substr($SkP,0,11)."' ORDER BY Kd_Unit";}
		else
		{$nSQ = "SELECT * FROM ref_unit where Kd_Unit like '".$zBid.".__' ORDER BY Kd_Unit";}
		$nRs = mysql_query($nSQ) or die(mysql_error());
		$mRo = mysql_fetch_assoc($nRs);
		$tRo = mysql_num_rows($nRs);
		if ($tRo > 0)
    ...
    <tr> 
      <td width="24">&nbsp;</td>
      <td width="111">KODE SUB UNIT </td>
      <td width="25">:</td>
      <td><input name="fKode" type="text" class="text" id="fKode" style="font-family: Calibri; font-size: 11pt; border: 1px solid #C0C0C0; padding-left: 4px; padding-right: 4px; padding-top: 1px; padding-bottom: 1px; background-color: #E1F986" value="<?php echo $gCod?>" size="11" maxlength="14" readonly /></td>
    </tr>
    <tr> 
      <td width="24">&nbsp;</td>
      <td width="111">NAMA SUB UNIT </td>
      <td width="25">:</td>
